...
**Phase 1: production prose at scale.** In progress. Volumes I through V are drafted in full at first-draft density; Volume VI is partially drafted (Ch 1 through Ch 10 of 13); Volumes VII and VIII are drafted in full (19 of 19 and 14 of 14 chapters respectively). All 109 drafted chapters compile cleanly in `main.pdf`. The discipline guards (`make check`, `make audit-docs`, `make accidents`, `make exercise-counts`) report PASS across the current state.
...
- **Volumes IX through XII** — 65 chapters at **Stage 1** (scaffolding shells with dossier-derived metadata; no prose).

Two lanes remain open:

- Continue forward into Volume VI Ch 11-13 and Volume IX prose.
- Codex review for Volumes II through VIII (100 chapters awaiting review on the same three-role protocol that carried Volume I from Stage 4 to Stage 5).
- Volume I-VIII production-density pass: figures, project artifacts, expanded solution sets, case-study expansions, code/data co-located with each chapter. Q57 (page density vs target) is the open question that gates whether this lane runs at all; with seven full volumes plus Vol VI partial drafted at consistent ~14-31% of target density, the empirical case is now strong.
...
| `Project track: TBD` remaining | 68 (Vol VI Ch 11-13 plus all of Vols IX-XII) |
| TODO markers in TeX | ~700 |
| Empty epigraphs in chapter shells | 66 |
| Section headings | ~1650 |
| Bibliography entries | 261 |
| Named-case registry entries | 23 |
| `acc:` keys cited in chapter prose | 22 |
| Research seeds on disk | 1 |
| Chapters with prose (Stage 4 or 5) | 109 |
| Vol I chapters at Stage 5 | 9 of 9 |
| Vols II-VIII chapters at Stage 4 | 100 awaiting review |
| Chapters at Stage 1 | 65 |
| `main.pdf` page count (last build) | 2343 |
...
| Prefix | Count | Use |
| --- | --- | --- |
| `text:` | ~135 | textbooks, monographs, working handbooks |
| `hist:` | ~32 | historical and biographical primary/secondary sources |
| `paper:` | ~28 | peer-reviewed papers |
| `acc:` | 23 | accident investigation reports and the registry's primary keys |
| `web:` | ~13 | tertiary explainers and reference web content (non-load-bearing) |
| `std:` | ~14 | standards documents |
...
- 109 chapters with first-draft prose across Volumes I-VIII (Vol VI partial).
...
- A working PDF build at 2343 pages. Volume I prose complete and reviewed at first-draft density; Volumes II-V prose complete at first-draft density awaiting review; Volume VI 10 of 13 chapters drafted at first-draft density awaiting review; Volumes VII and VIII prose complete at first-draft density awaiting review.
- Named-cases registry with 23 entries, schema-conformant, audited by `make accidents`.
...
- **Q55 per-chapter project track (68 chapters remaining)**: Volumes I-V are complete; Volume VI is partially complete (Ch 1-10 settled, Ch 11-13 still TBD); Volumes VII and VIII are complete; Volumes IX-XII still carry `Project track: TBD`. Editorial pass needed per volume as that volume comes into prose drafting.
- **Q57 page density vs target**: Seven full volumes plus partial Vol VI are now drafted at consistent first-draft density (~14-31\% of target across the eight volumes). Volume I 220 of 720; Vol II 490 of 1800; Vol III 274 of 1300; Vol IV 234 of 1400; Vol V 238 of 1200; Vol VI (partial) 178 of 1300; Vol VII 264 of 1980; Vol VIII 342 of 1610. The pattern is consistent: first-draft prose lands at roughly one-third to one-fifth of the planned page budget. The two readings (chapters dense vs artifacts missing) are now testable against eight volumes of evidence.
...
- **Q56 named-cases registry**: settled 2026-04-29; registry now holds 23 entries (Pentium FDIV bug, Mars Pathfinder priority inversion added during Vol VII drafting; de Havilland Comet added during Vol VIII drafting), all `acc:` keys cited in prose resolve to a registry entry, and every named accident in prose carries at least one of its registry's citation keys.
...
2. **Scale honesty for the reader.** **Open.** Q57 is the live form; eight volumes of evidence now sit under the question.
3. **Verification (citation discipline, reproducible examples, table provenance).** **Substantially mitigated for Volume I; carried forward into Vols II-VIII as first-draft discipline awaiting Codex review.** Bibliography grew from 86 to 261 entries across Vols II-VIII drafting. The Stage 4 review pass for Vols II-VIII is the discipline carrier for the rest of the project.
...
Produces `main.pdf` at 2343 pages. `make check`, `make audit-docs`, `make accidents`, and `make exercise-counts` all report `PASS`. `make stats` reports the at-a-glance counts above.
...
3. Start Volume IX prose (Systems) once Vol VI is complete. The bridge from Vol VIII (Machines) is direct: the integration discipline of Vol VIII Ch 14 carries forward to systems-of-systems work.
4. Settle Q57 with eight volumes of evidence behind the decision. The two readings imply different forward sequences (forward to Vol IX vs back to Vols I-VIII production-density pass).
5. Settle Q54 (companion-note architecture) before any production-density lane runs.
6. Continue closing the 68 remaining Q55 project-track decisions, one volume at a time as that volume comes into prose drafting.
7. Apply or formally defer the queued III-IV and IV-V interlude revisions.

Seven-and-a-half volumes of first-draft prose are now on disk at consistent density. The pipeline has been exercised at scale (parallel-agent passes for Vol II ran cleanly; single-thread main-thread passes for Vols III through VIII all ran cleanly). The next gating decisions are review throughput and page-density target, not whether the project can produce defensible chapters.
