...
Vol I Ch 4 project track: **household + analysis (hybrid)**. Hazard class: **low** (outdoor street-level measurement; no climbing, no working at height, no instruments above battery voltage). Tools: smartphone (clinometer or theodolite app, compass, GPS, camera with EXIF or known focal length), tape measure (preferably $\geq 10 \,\si{\meter}$), a stopwatch, a meter rule, and a published reference for the chosen building (Wikipedia, OpenStreetMap, building permit records, or the building owner's documentation). Artifact: three independent estimates of one building's height, each with an explicit error bar derived from the underlying measurement uncertainties via the algebraic propagation rules introduced in this chapter, plus a 1500-word reflection comparing the three results, identifying the dominant uncertainty source in each method, and discussing where the methods agreed within their stated error bars and where they did not. Solutions take the form of a published reference set of three-method computations from the general editor, not a single answer key.

### Settled 2026-04-29 (Q55-ch05)

Vol I Ch 5 project track: **physical with simulation alternative (hybrid)**. The physical track is Volume I's first hands-on hardware project; the simulation alternative is provided for readers without electronics access. Hazard class: **low** (battery-voltage circuits only; no mains-voltage work; thermistor in air or warm water, no soldering required if breadboard-only). Tools (physical track): an NTC thermistor (e.g., $10 \,\si{\kilo\ohm}$ at $25 \,\si{\degreeCelsius}$), a fixed reference resistor of similar resistance, a breadboard, jumper wires, a multimeter, a microcontroller with at least one analog-to-digital input (Arduino Uno, Raspberry Pi Pico, or equivalent), a kitchen thermometer or other reference temperature instrument, optionally a beaker or insulated container with warm and cool water for stepped reference temperatures. Tools (simulation track): a Steinhart-Hart thermistor model, a voltage-divider model, simulated ADC noise and quantisation, plotting environment. Artifact: a characterisation curve mapping thermistor resistance and divider output voltage to temperature across the operating range, with explicit error bars from the chapter's algebraic propagation rules, plus a 1500-word reflection on linearity, drift, the dominant uncertainty source, and what the reader's confidence in the sensor would change for a real engineering deployment. Solutions take the form of a published reference characterisation from the general editor.

### Settled 2026-04-29 (Q55-ch06)

Vol I Ch 6 project track: **household + analysis (hybrid)**. Hazard class: **low** (no direct mains-voltage work; the reader uses a battery-isolated optical or audio coupling to the mains lighting, never a galvanic connection). Tools: a mains-powered LED lamp or fluorescent bulb whose flicker can be observed by a photodiode or a smartphone camera in slow-motion mode; OR an inexpensive battery-powered audio adapter with a low-voltage transformer providing galvanic isolation; a smartphone or laptop with audio input; software to detect zero crossings or peak frequencies (a free audio analysis tool, a short Python script, or a spreadsheet); optionally an oscilloscope. Artifact: a 24-hour record of the mains frequency at the reader's location at one-minute resolution or finer, with an estimated standard uncertainty per sample, compared to the grid operator's published frequency record where one is available, plus a 1500-word reflection on the dominant uncertainty source, the time-of-day frequency variation observed, and the engineering implications of grid-frequency control.

### Settled 2026-04-29 (Q55-ch07)

Vol I Ch 7 project track: **household + analysis (hybrid)**. Hazard class: **none**. Tools: a kitchen scale or postal scale; a ruler or tape measure; a beaker, measuring cup, or graduated cylinder for water displacement; water; a smartphone camera (for image-based 3D approximation, optional); paper and pencil for the geometric approximation. Artifact: three independent volume measurements of one irregular household object (a stone, a small tool, a piece of fruit, a piece of jewellery) using (i) water displacement (Archimedes), (ii) geometric approximation (decomposition into a sum of simple shapes whose volumes the reader can compute), and (iii) image-based or contact-based dimensional reconstruction. Each measurement carries an uncertainty bar derived from the chapter's algebraic propagation rules. Plus a 1500-word reflection on which method gave the smallest error bar, where the methods agreed within their combined uncertainty, and where they did not.

### Settled 2026-04-29 (Q55-ch08)

Vol I Ch 8 project track: **analysis with optional household component (hybrid)**. Hazard class: **none** for the analysis track; **low** if the chosen experiment uses simple household instruments. Tools: paper, pencil, a spreadsheet or programming environment for the statistical analysis; optionally household instruments for the chosen experiment. Artifact: a 30-trial experimental record of the reader's choice (cooking-time variability, daily commute time, smartphone-step-counter accuracy, household appliance energy use, or any equivalent), with summary statistics per the chapter's section 8.1, an inferential analysis appropriate to the experiment, an explicit pre-registration of the hypothesis tested, and a 1500-word reflection on what the reader's confidence in the result would change if the experiment were repeated by a colleague.

### Settled 2026-04-29 (Q55-ch09)
...
### Settled 2026-04-29 (Q55-vol02-ch10)

Vol II Ch 10 project track: **simulation with analysis**. Hazard class: **none**. Tools: paper, pencil, programming environment (Python+NumPy, MATLAB, Julia, or equivalent) with SVD primitives. Artifact: a from-scratch implementation of Principal Component Analysis (no `sklearn.decomposition.PCA` or equivalent library shortcut) applied to a real dataset of the reader's choice (a Vol I project dataset, a public dataset such as a UCI archive set, or a synthesised dataset with known principal directions). The implementation must compute the SVD by an explicit method (eigendecomposition of $A^{T}A$, or power iteration, or a library SVD primitive) and apply the projection by hand. Compare the principal directions and explained-variance ratios against a library implementation. Plus a 1500-word reflection on what the SVD reveals about the data and on the numerical conditioning differences between the three SVD routes.

### Settled 2026-04-29 (Q55-vol02-ch11)

Vol II Ch 11 project track: **analysis only**. Hazard class: **none**. Tools: paper, pencil, calculator. Artifact: a hand-derived solution to a three-constraint optimisation problem of the reader's choice (canonical examples: minimise surface area at fixed volume and fixed aspect-ratio range; minimise power at fixed throughput and fixed budget; fit a quadratic through three points with a derivative constraint at one of them; minimise a quadratic cost subject to two equality constraints and one inequality). The reader writes out the Lagrangian, applies the KKT conditions explicitly, identifies which constraints are active, and verifies the second-order sufficient conditions where they apply. Plus a 1500-word reflection on which constraint was active and why, and on what the multiplier values mean operationally.

### Settled 2026-04-29 (Q55-vol02-ch12)

Vol II Ch 12 project track: **simulation with analysis**. Hazard class: **none**. Tools: paper, pencil, programming environment with array primitives (Python+NumPy, MATLAB, Julia, or equivalent). Artifact: a numerical solution of the 1D heat equation $u_{t} = \alpha u_{xx}$ on a rod of stated length with explicit initial and boundary conditions, computed with both an explicit forward-Euler finite-difference scheme and an implicit backward-Euler scheme; compared against the separation-of-variables analytical solution at three times. The reader documents the CFL stability condition for the explicit scheme, demonstrates instability at $\Delta t$ above the CFL bound, and explains why the implicit scheme can step through that bound. Plus a 1500-word reflection on the cost of bad boundary conditions and on the engineer's choice between explicit and implicit time-stepping.

### Settled 2026-04-29 (Q55-vol02-ch13)

Vol II Ch 13 project track: **simulation with analysis**. Hazard class: **none**. Tools: paper, pencil, programming environment with random-number primitives. Artifact: a simulation of a Markov chain of the reader's choice (a random walk on a small graph of the reader's design, a two-state weather model, a queueing model, or a board-game position chain) for at least $10\,000$ steps. The reader computes the empirical state-occupancy distribution, compares it to the analytical stationary distribution (the leading left eigenvector of the transition matrix), reports the rate of convergence, and demonstrates the Law of Large Numbers and the Central Limit Theorem on a chain-derived statistic of their choice. Plus a 1500-word reflection on what the limit theorems guarantee and what they do not, and on when a Markov assumption is and is not safe.

### Settled 2026-04-29 (Q55-vol02-ch14)

Vol II Ch 14 project track: **simulation with analysis**. Hazard class: **none**. Tools: paper, pencil, programming environment with regression primitives. Artifact: a model fit (linear regression, logistic regression, or a regularised variant) on a real dataset of the reader's choice (could be Vol I project data); k-fold cross-validation with $k = 5$ or $k = 10$; reported with proper error bars (cross-validated, not training-set), a confusion matrix or residual diagnostic, and an honest description of what the model can and cannot defend. The reader must include at least one variable selection or regularisation choice and explain why, and report what changes if the choice is reversed. Plus a 1500-word reflection on how cross-validation differs from training-error reporting and on why the latter overstates model quality.

### Settled 2026-04-29 (Q55-vol02-ch15)

Vol II Ch 15 project track: **simulation with analysis**. Hazard class: **none**. Tools: paper, pencil, programming environment. Artifact: gradient descent implemented from scratch (vanilla, plus at least one of: momentum, Nesterov, RMSProp, Adam) on a chosen objective (suggested: the polynomial-fit problem from Vol II Ch 9 project, a Rosenbrock function, or a small neural-network loss surface); convergence trajectory plotted; learning-rate sensitivity demonstrated; compared against a library optimiser. Plus a 1500-word reflection on local minima, learning-rate sensitivity, when stochastic methods help, and how the engineer should choose between methods given a real-world objective.

### Settled 2026-04-29 (Q55-vol02-ch16)

Vol II Ch 16 project track: **simulation with analysis**. Hazard class: **none**. Tools: paper, pencil, programming environment. Artifact: a reproduction of one numerical result from a published paper or canonical numerical-textbook exercise of the reader's choice (suggestions: a chapter exercise from Trefethen-Bau on conditioning of a Hilbert matrix; a numerical integration from a published heat-transfer paper; a published ODE integration of a stiff system; a published root-finding example with a documented failure mode). The reader documents the original method, reproduces the result on a modern floating-point system, accounts for any drift between the published value and the reader's, and identifies the dominant numerical error source. Plus a 1500-word reflection on what reproducibility costs in real engineering and on which floating-point or algorithmic choices were the most consequential.

### Settled 2026-04-29 (Q55-vol02-ch17)

Vol II Ch 17 project track: **analysis with optional simulation**. Hazard class: **none**. Tools: paper, pencil, optionally a programming environment with graph-algorithm primitives (NetworkX, igraph, or equivalent). Artifact: a graph model of a real network of the reader's choice (city transit network, electrical grid section, water distribution network, social network, supply chain, software dependency graph, or biological network). The reader documents the data source, the abstraction choices (what counts as a node, what counts as an edge, what is left out), and computes the diameter, clustering coefficient, and degree distribution. Identification of the highest-betweenness or highest-degree nodes and discussion of what their removal would do to network connectivity. Plus a 1500-word reflection on what the graph metrics reveal about the network's resilience and on the abstraction choices' effect on the conclusions.

### Settled 2026-04-29 (Q55-vol02-ch18)

Vol II Ch 18 project track: **analysis only**. Hazard class: **none**. Tools: paper, pencil, calculator, the reader's own Volume I measurement notebook. Artifact: a 5-page modelling write-up that takes one quantity from the reader's Vol I capstone notebook and turns it into a model: a hypothesis about the underlying mechanism, a parameter to estimate, a validation procedure, an honest accounting of what the model leaves out, and a sensitivity analysis. The write-up must include a clear distinction between what is data, what is assumption, what is fit, and what is residual. Plus a 1500-word reflection on which modelling decisions were the most consequential, which would change the engineering conclusion if reversed, and on the difference between a model the reader can defend and a model the reader cannot.
