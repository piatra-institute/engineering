...
- **Phase 1** (production prose at scale) **In progress**. Volumes I-V drafted in full at first-draft density; Volume VI 10 of 13 drafted; Volume VII drafted in full (19 of 19). Volume I is at Stage 5 (reviewed and resolved); Volumes II-VII are at Stage 4 awaiting Codex review. Codex-review lane for Vols II-VII and forward-prose lane into Vol VI Ch 11-13 and Vol VIII both open; Q57 (page density vs target) gates the production-density choice and now has seven volumes of evidence behind it.

Verified at last edit (2026-05-15):

- 12 volumes / 174 chapters / 174 dossiers.
- 2201-page PDF.
- 95 chapters with first-draft prose. Per-volume status:
  ...
  - **Vols VIII-XII** 76 chapters at Stage 1 (scaffolding shells only).
- 252 bibliography entries (131 text, 31 hist, 27 paper, 22 acc, 13 web, 12 std, 8 gen, 5 method, 2 data, 1 law).
- 22 named-case registry entries; 22 `acc:` keys cited in prose; `make accidents` PASS. (Pentium FDIV bug and Mars Pathfinder priority inversion added during Vol VII drafting.)
- 1 research seed on disk under `docs/research/seeds/`.
- 79 chapters still carry `Project track: TBD` (Vol VI Ch 11-13 plus all of Vols VIII-XII). Q55 settled per-chapter for Vols I-V, Vol VI Ch 1-10, and Vol VII during drafting.
- Vol I review log complete: `docs/reviews/ch01-pilot-review.md` plus `docs/reviews/vol01-ch02-review.md` through `docs/reviews/vol01-ch09-review.md`, each with a `Resolved:` banner. Vols II-VII reviews not yet started; the same three-role protocol carries forward.
...
- Math operators in `eng-macros.sty`: linear-algebra (`\spn`, `\col`, `\nul`, `\rank`, `\sgn`, `\proj`); trace and diagonal (`\tr`, `\diag`); optimisation (`\argmin`, `\argmax` via `\DeclareMathOperator*`). Added during Vol II drafting; carried forward unchanged through Vols III-VII. Vol VII drafting added the `\usepackage[inline]{enumitem}` option in `preamble.tex` to support `enumerate*` inline lists.
- `make check`, `make audit-docs`, `make accidents`, `make exercise-counts` all PASS. `make distclean && make` produces a clean 2201-page PDF.
