...
- 3698-page PDF. Vol I at Tier 3 second pass (9-agent parallel git-worktree rollout 2026-05-31, plus three main-thread deepening turns), 620/720 pp = 86.1% target density. Vol II at Tier 1+2 across 17 of 18 chapters, with Ch 2 still at first-draft pending re-run.
- 171 chapters with first-draft prose. Per-volume status:
  - **Vol I** 9/9 at Stage 5 and at **Tier 3 second pass**, pp 1-618 (618pp; target 720; 85.8% of target). Tier 3 second pass 2026-05-31 via nine parallel git-worktree agents (one per chapter, each closing the dossier gap with long-form case-study expansions, multi-page worked examples, historical sketches, dossier-promised subsection deepening, and ~10 exercise-solution promotions); eight agents merged, the Ch 8 agent hit a session rate limit before chapter.tex edits landed (only 2 figures + 32 bib entries from that agent). Two main-thread deepening turns followed, closing Ch 8 multiple-comparisons (Tukey HSD / Scheffé / Dunnett), Ch 8 Fisher→NP→ASA-2016 historical sketch, Ch 8 tolerance-interval worked procedure (Howe 1969), Ch 8 OPERA + carbon-nanotube replication cases, Ch 4 multivariate Monte Carlo with Zernike-coefficient optical-aberration example, and Ch 4 spreadsheet floating-point case study. Per-chapter spans: Ch 1 56pp (93%), Ch 2 58pp (83%), Ch 3 62pp (78%), Ch 4 76pp (76%), Ch 5 80pp (89%), Ch 6 74pp (93%), Ch 7 66pp (94%), Ch 8 68pp (76%), Ch 9 72pp (90%). Remaining gap concentrated in Ch 4 (+24pp) and Ch 8 (+22pp). Bibliography grew 498→702 entries; registry held at 39 entries (canonical 787-battery key resolved during merge). Tier 3 plan recorded in `docs/vol01-tier3-plan.md`.
...
- `make check`, `make audit-docs`, `make accidents`, `make exercise-counts` all PASS. `make distclean && make` produces a clean 3698-page PDF.
