...
\input{volumes/02-form/ch15-optimisation/figures/fig-jensen-inequality.tex}

\paragraph{Worked example: convexity by the sublevel-set and
epigraph tests.}
The Hessian test needs a twice-differentiable function; the two
geometric tests do not, and they settle convexity for functions the
Hessian cannot reach. The \engterm{epigraph} of $f$ is the set of
points on or above its graph, $\operatorname{epi} f = \set{(\vect{x},
t) : t \geq f(\vect{x})}$, and $f$ is convex exactly when
$\operatorname{epi} f$ is a convex set.
Take $f(x) = \max(x^{2} - 1, \; 2x - 2)$, a pointwise maximum of a
convex parabola and an affine function, with a kink where the two
pieces meet. The Hessian does not exist at the kink, so the
second-order test is silent, but the epigraph is the intersection of
the two epigraphs $\set{t \geq x^{2} - 1}$ and $\set{t \geq 2x - 2}$,
each convex, and an intersection of convex sets is convex, so $f$ is
convex. The \engterm{sublevel set} $\set{\vect{x} : f(\vect{x}) \leq
\alpha}$ of a convex $f$ is convex for every threshold $\alpha$, which
is why an inequality constraint $f_{i}(\vect{x}) \leq 0$ built from a
convex $f_{i}$ carves out a convex feasible region. The converse
fails, and the failure is worth seeing: $g(x) = \sqrt{\abs{x}}$ has
convex sublevel sets (each is an interval symmetric about the origin)
yet is not convex (its chord from $-1$ to $1$ lies below the graph at
the origin). A function all of whose sublevel sets are convex is
\engterm{quasiconvex}, a strictly weaker property than convexity, and
the engineer who checks only the sublevel sets has verified
quasiconvexity, which guarantees a connected set of minimisers but not
the local-implies-global theorem that convexity delivers. The three
tests form a ladder: the epigraph test is equivalent to convexity, the
sublevel-set test is necessary but not sufficient, and the Hessian
test is the computational shortcut when the function is smooth.
...
\input{volumes/02-form/ch15-optimisation/figures/fig-convex-nonconvex.tex}
...
\input{volumes/02-form/ch15-optimisation/figures/fig-contour-descent.tex}
...
\input{volumes/02-form/ch15-optimisation/figures/fig-feasible-polytope.tex}
...
\input{volumes/02-form/ch15-optimisation/figures/fig-lp-graphical.tex}
...
\input{volumes/02-form/ch15-optimisation/figures/fig-efficient-frontier.tex}
...
\input{volumes/02-form/ch15-optimisation/figures/fig-central-path.tex}

\subsection{Applied vignette: economic dispatch of a power system}

A grid operator must meet an electrical demand $D$ at every instant by
choosing how hard to run each generator, and the choice is a convex QP
solved thousands of times a day in every wholesale electricity market.
The setting shows the chapter's apparatus carrying a decision with real
money and real physics attached, and it shows the shadow price
reappearing as a quantity the market actually pays.

\paragraph{The brief.}
Three generators serve a demand of $D = 500\,\si{\mega\watt}$. Each
generator $i$ has a quadratic running cost $C_{i}(P_{i}) = a_{i} +
b_{i} P_{i} + c_{i} P_{i}^{2}$ in dollars per hour, with the quadratic
term capturing the rising marginal cost of pushing a unit toward its
ceiling, and each has an output range $[P_{i}^{\min}, P_{i}^{\max}]$.
Take the coefficients
\[
\begin{array}{c|ccc}
 & b_{i}\ (\text{USD/MWh}) & c_{i}\ (\text{USD/MWh}^{2})
 & [P_{i}^{\min}, P_{i}^{\max}]\ (\si{\mega\watt}) \\ \hline
\text{unit 1} & 20 & 0.020 & [50, 300] \\
\text{unit 2} & 25 & 0.010 & [50, 300] \\
\text{unit 3} & 40 & 0.005 & [50, 300]
\end{array}
\]
The fixed terms $a_{i}$ drop out of the optimisation because the units
are committed and running. The dispatch problem in plain English:
split $500\,\si{\mega\watt}$ across the three units to meet demand at
least total cost.

\paragraph{The brief becomes a program.}
The decision variable is $\vect{P} = (P_{1}, P_{2}, P_{3})$, the
objective is $\sum_{i}(b_{i} P_{i} + c_{i} P_{i}^{2})$, and the
constraints are the demand balance $\sum_{i} P_{i} = D$ and the box
$P_{i}^{\min} \leq P_{i} \leq P_{i}^{\max}$. The Hessian is
$\diag(2c_{1}, 2c_{2}, 2c_{3}) \succ 0$, so the problem is a strictly
convex QP with a unique global optimum.

\paragraph{The equal-incremental-cost rule.}
Ignore the box bounds first and attach a multiplier $\lambda$ to the
demand balance. Stationarity of the Lagrangian $\sum_{i}(b_{i} P_{i} +
c_{i} P_{i}^{2}) + \lambda(D - \sum_{i} P_{i})$ gives $b_{i} + 2 c_{i}
P_{i} = \lambda$ for every unit: at the optimum every generator runs at
the same \engterm{incremental cost} $\lambda$, the classical
equal-lambda dispatch rule. Solving each for $P_{i} = (\lambda -
b_{i})/(2 c_{i})$ and substituting into $\sum_{i} P_{i} = 500$,
\[
\frac{\lambda - 20}{0.04} + \frac{\lambda - 25}{0.02} +
\frac{\lambda - 40}{0.01} = 500.
\]
Multiplying through, $25(\lambda - 20) + 50(\lambda - 25) + 100(\lambda
- 40) = 500$, so $175 \lambda - 500 - 1250 - 4000 = 500$, giving
$175 \lambda = 6250$ and $\lambda^{*} \approx 35.7\,\text{USD/MWh}$.
The dispatched outputs are $P_{1} = (35.7 - 20)/0.04 \approx 393$,
$P_{2} = (35.7 - 25)/0.02 \approx 536$, $P_{3} = (35.7 - 40)/0.01
\approx -430$. The last two violate the box: unit 2 exceeds its
$300\,\si{\mega\watt}$ ceiling and unit 3 falls below its
$50\,\si{\mega\watt}$ floor.

\paragraph{The active set fixes the dispatch.}
The box violations are the KKT complementary-slackness conditions
speaking. Pin the violated units at their nearest bound: $P_{2} = 300$
(ceiling) and $P_{3} = 50$ (floor). The remaining demand $500 - 300 -
50 = 150\,\si{\mega\watt}$ falls to unit 1, which is interior, so
$P_{1} = 150$, well inside $[50, 300]$. The optimal dispatch is
$\vect{P}^{*} = (150, 300, 50)\,\si{\mega\watt}$, and the system
incremental cost is set by the one interior (marginal) unit: $\lambda^{*}
= b_{1} + 2 c_{1} P_{1} = 20 + 0.04 \times 150 = 26\,\text{USD/MWh}$.

\paragraph{What the multiplier pays for.}
The multiplier $\lambda^{*} = 26\,\text{USD/MWh}$ is the shadow price
of the demand-balance constraint: the marginal cost of serving one more
megawatt-hour of load, met by the marginal unit. In an electricity
market this number is the \engterm{locational marginal price}, the
price every generator is paid and every consumer charged for energy at
that node in that interval, and it falls directly out of the QP dual.
The two boundary units carry their own nonzero multipliers on the
active box constraints: unit 2's ceiling multiplier is the amount by
which system cost would fall per megawatt of extra capacity on unit 2,
which quantifies the value of an upgrade to that generator; unit 3's
floor multiplier is the cost the minimum-run constraint imposes, which
quantifies what the operator pays to keep an expensive unit spinning
for reliability. The QP thus returns not one number but a full price
signal: the energy price, the value of relieving each binding limit,
and the merit order of the fleet. The same problem with transmission
limits and multiple nodes stays a convex QP with more constraint rows
and is the optimal-power-flow computation at the centre of Volume X's
treatment of the grid; the three-unit case here is that computation
stripped to the one insight that survives at every scale, which is that
the balance multiplier is the price and the active set is the merit
order.

\subsection{Applied vignette: minimum-cost flow through a network}

A large family of engineering allocation problems live on a graph:
water through a pipe network, current through a distribution grid,
packets through a router mesh, product through a supply chain. Each asks
the same question, how to route a required amount of some commodity from
sources to sinks at least total cost while respecting the capacity of
every link, and each is a linear program with a structure that hands the
engineer an integer answer for free. The vignette works a small
instance end to end and shows the shadow price returning as a congestion
toll (\cref{fig:vol02:ch15:network-flow}).

\input{volumes/02-form/ch15-optimisation/figures/fig-network-flow.tex}

\paragraph{The brief.}
A network has nodes joined by directed edges. Each node carries a
\engterm{supply} $b_{v}$ (positive at a source, negative at a sink, zero
at a pass-through node), and the supplies sum to zero so the network is
balanced. Each edge $(u, v)$ carries a per-unit cost $c_{uv}$ and a
capacity $\kappa_{uv}$. The decision is the flow $x_{uv} \geq 0$ on each
edge, and two rules bind: no edge carries more than its capacity, and
flow is conserved at every node, meaning the flow out minus the flow in
equals the node's supply. The four-node instance of
\cref{fig:vol02:ch15:network-flow} sends five units from source $s$
(supply $+5$) to sink $t$ (supply $-5$) across two intermediate nodes,
with the cost and capacity of each edge shown on the figure.

\paragraph{The brief becomes a program.}
The objective is the total routing cost $\sum_{(u,v)} c_{uv} x_{uv}$.
The conservation rule at each node $v$ is the linear equality
$\sum_{u} x_{vu} - \sum_{u} x_{uv} = b_{v}$, and the capacity rule is the
bound $0 \leq x_{uv} \leq \kappa_{uv}$. The program
\[
\min_{\vect{x} \geq \vect{0}} \; \sum_{(u,v)} c_{uv} x_{uv}
\quad \text{subject to} \quad
N \vect{x} = \vect{b}, \;\; \vect{x} \leq \boldsymbol{\kappa},
\]
where $N$ is the node-edge incidence matrix (one row per node, a $+1$
and a $-1$ per edge column), is a linear program. Working the instance:
the two source edges are the cheapest way out of $s$, so the optimum
saturates $s\!\to\!a$ (cost $2$, three units) and $s\!\to\!b$ (cost $1$,
two units) to push all five units forward. At the sink side, edge
$a\!\to\!t$ (cost $1$) takes its three units cheaply, and edge
$b\!\to\!t$ (cost $3$, capacity $2$) is forced to carry its two units
because $b$ has no cheaper exit; the cross-link $a\!\to\!b$ stays idle
because rerouting through it would only add cost. The optimal flow is
$(x_{sa}, x_{sb}, x_{at}, x_{bt}, x_{ab}) = (3, 2, 3, 2, 0)$ at total
cost $2\cdot 3 + 1\cdot 2 + 1\cdot 3 + 3\cdot 2 + 0 = 17$.

\paragraph{Integrality for free.}
The instance was posed with integer supplies and capacities, and the
optimal flow came out integer with no rounding. This is not luck. The
incidence matrix $N$ of a directed graph is totally unimodular, the same
structural property that made the assignment problem of Section 15.7
solve as an LP, so every vertex of the flow polytope is integer whenever
the supplies and capacities are integer. A minimum-cost flow is
therefore an integer program that the polynomial-time LP solver already
solves exactly, and the specialised network-simplex and
successive-shortest-path algorithms exploit the graph structure to solve
it faster still, in time that scales with the number of edges rather
than with a general LP's dimension.

\paragraph{What the dual prices.}
The dual assigns a scalar \engterm{potential} $\pi_{v}$ to each node
(the multiplier on that node's conservation equality) and a nonnegative
price $\lambda_{uv}$ to each capacity bound. Complementary slackness
gives the routing rule directly: on an edge carrying flow strictly
between zero and capacity, the reduced cost $c_{uv} - \pi_{u} + \pi_{v}$
is zero, so the potential difference along a used, unsaturated edge
equals its cost. On a saturated edge the capacity price $\lambda_{uv} >
0$ measures the cost reduction that one more unit of capacity would buy,
which is the value of widening that pipe or upgrading that line. In the
instance, edge $b\!\to\!t$ is saturated and expensive, so its capacity
price is positive and flags it as the network's bottleneck: the engineer
who can add one unit of capacity anywhere should add it there, because
relieving $b\!\to\!t$ lets a unit reroute onto a cheaper path and lowers
total cost by exactly the capacity price. The node potentials are the
prices a market would post at each location (the locational prices of the
economic-dispatch vignette, now on a transport network), and their
differences are the tolls that make every routing decision individually
rational. The same LP, scaled to thousands of nodes and edges, is the
transshipment model behind logistics planning, the DC-power-flow
approximation behind grid dispatch, and the traffic-assignment model
behind road-network pricing; the four-node case is that model stripped
to the two facts that survive at every scale, that the flow is integer
because the incidence matrix is unimodular and the capacity price is the
value of relieving the bottleneck.
...
\input{volumes/02-form/ch15-optimisation/figures/fig-kkt-geometry.tex}

\input{volumes/02-form/ch15-optimisation/figures/fig-lagrange-geometry.tex}
...
\subsection{Worked KKT example: water-filling power allocation}

The KKT conditions read off a closed-form allocation for a problem an
engineer meets in communications, energy, and any setting where a fixed
budget is split across resources of unequal quality. A transmitter has a
total power budget $P$ to split across $n$ parallel channels; channel
$i$ has gain $g_{i} > 0$, and allocating power $p_{i} \geq 0$ to it
returns a throughput $\log(1 + g_{i} p_{i})$ (the concave return of an
additive-Gaussian-noise channel). The allocation maximises total
throughput subject to the budget:
\[
\max_{\vect{p} \geq \vect{0}} \; \sum_{i=1}^{n} \log(1 + g_{i} p_{i})
\quad \text{subject to} \quad \sum_{i=1}^{n} p_{i} = P.
\]
The objective is concave and the constraints affine, so the problem is
convex and the KKT conditions are sufficient. Attach a multiplier $\nu$
to the budget equality and multipliers $\mu_{i} \geq 0$ to the
nonnegativity bounds $-p_{i} \leq 0$. Stationarity of the Lagrangian
$-\sum_{i} \log(1 + g_{i} p_{i}) + \nu(\sum_{i} p_{i} - P) - \sum_{i}
\mu_{i} p_{i}$ gives, for each channel,
\[
-\frac{g_{i}}{1 + g_{i} p_{i}} + \nu - \mu_{i} = 0.
\]
Complementary slackness $\mu_{i} p_{i} = 0$ splits the channels. Where a
channel is used ($p_{i} > 0$) its bound multiplier vanishes, $\mu_{i} =
0$, and stationarity gives $1/g_{i} + p_{i} = 1/\nu$, so $p_{i} = 1/\nu
- 1/g_{i}$. Where the bound binds ($p_{i} = 0$) the same relation would
demand $1/\nu \leq 1/g_{i}$, so the channel is too weak to be worth any
power. Writing $1/\nu$ as a single scalar level, the two cases collapse
into
\[
p_{i}^{*} = \max\!\left(\frac{1}{\nu} - \frac{1}{g_{i}}, \; 0\right),
\]
with the level $1/\nu$ fixed by the budget $\sum_{i} p_{i}^{*} = P$. The
solution is the \engterm{water-filling} allocation
(\cref{fig:vol02:ch15:water-filling}): picture each channel as a vessel
whose floor sits at height $1/g_{i}$, pour a volume $P$ of water in, and
the depth over each floor is the power allocated. Strong channels have
low floors and take the most power; a channel whose floor rises above
the water surface is left dry. A three-channel instance makes the
arithmetic concrete. Gains $g = (10, 2, 0.5)$ give floors $1/g = (0.1,
0.5, 2.0)$, and a budget $P = 1$ is met by raising the level until the
wet depths sum to one: at level $1/\nu = 0.7$ the depths are $(0.6, 0.2,
0)$ summing to $0.8$, at $1/\nu = 0.75$ they are $(0.65, 0.25, 0)$
summing to $0.9$, and the exact budget lands at $1/\nu \approx 0.767$
with channel 3 dry throughout because its floor $2.0$ never reaches the
surface. The multiplier $\nu$ is the shadow price of power: the marginal
throughput per extra watt of budget is exactly $\nu$, so an engineer
deciding whether to buy a larger amplifier reads the value of the
purchase straight off the water level.

\input{volumes/02-form/ch15-optimisation/figures/fig-water-filling.tex}
...
\input{volumes/02-form/ch15-optimisation/figures/fig-gradient-zigzag.tex}
...
\Cref{fig:vol02:ch15:newton-vs-gd} sets the same two-coordinate
quadratic against Newton's one-step finish, the picture the method
comparison of \cref{fig:vol02:ch15:method-comparison} summarises
asymptotically.

\input{volumes/02-form/ch15-optimisation/figures/fig-newton-vs-gd.tex}

\paragraph{Worked example: preconditioning collapses the condition
number.}
The zigzag of the elongated quadratic is a coordinate-scaling
artifact, and a change of variables removes it. Take again
$f(x_{1}, x_{2}) = \tfrac{1}{2}(x_{1}^{2} + 100 x_{2}^{2})$ with
Hessian $P = \diag(1, 100)$ and condition number $\kappa = 100$.
Rescale the variables by $\vect{y} = D \vect{x}$ with the diagonal
$D = \diag(1, 10)$, so that $x_{2}$ is measured in units ten times
coarser. In the new coordinates the objective is $\tilde{f}(\vect{y})
= \tfrac{1}{2} \vect{y}^{\transpose} D^{-1} P D^{-1} \vect{y} =
\tfrac{1}{2}(y_{1}^{2} + y_{2}^{2})$, a perfectly round bowl with
Hessian $I$ and condition number $\kappa = 1$. Gradient descent on
$\tilde{f}$ reaches the optimum in a single step from any start,
because the negative gradient of an isotropic quadratic points
straight at the centre. Undoing the change of variables, the update
$\vect{y}_{k+1} = \vect{y}_{k} - \eta \grad \tilde{f}$ becomes
$\vect{x}_{k+1} = \vect{x}_{k} - \eta \, D^{-2} \grad f(\vect{x}_{k})$
in the original coordinates: gradient descent with the gradient
premultiplied by the fixed matrix $M = D^{-2} = \diag(1, 1/100)$. That
matrix is a \engterm{preconditioner}, and the perfect choice here is
$M = P^{-1}$, which turns the preconditioned gradient step into exactly
the Newton step $-P^{-1} \grad f$. The lesson generalises past the
diagonal case: any positive-definite $M$ that approximates $P^{-1}$
shrinks the effective condition number $\kappa(M^{1/2} P M^{1/2})$
toward $1$, and the whole art of practical first-order optimisation,
from diagonal Jacobi scaling through the adaptive per-coordinate steps
of Adam to the limited-memory inverse-Hessian estimate of L-BFGS, is
the search for a cheap $M$ that does the work an exact Newton solve
would do at $O(n^{3})$ cost.
...
\input{volumes/02-form/ch15-optimisation/figures/fig-trust-region.tex}
...
\begin{mastery}
\textbf{Mirror descent and the geometry of the update.} Gradient
descent measures distance in the Euclidean norm: the step
$\vect{x}_{k+1} = \argmin_{\vect{x}} \big( \grad f(\vect{x}_{k})^{
\transpose} \vect{x} + \tfrac{1}{2\eta} \norm{\vect{x} -
\vect{x}_{k}}_{2}^{2} \big)$ trades progress on the linearised objective
against a squared-Euclidean penalty for moving, and solving it recovers
the familiar $\vect{x}_{k} - \eta \grad f$. \engterm{Mirror descent}
replaces the Euclidean penalty with a \engterm{Bregman divergence}
$D_{\psi}(\vect{x}, \vect{x}_{k}) = \psi(\vect{x}) - \psi(\vect{x}_{k})
- \grad \psi(\vect{x}_{k})^{\transpose}(\vect{x} - \vect{x}_{k})$
generated by a strictly convex \engterm{mirror map} $\psi$, and the
choice of $\psi$ tunes the geometry of the step to the geometry of the
feasible set. On the probability simplex the natural mirror map is the
negative entropy $\psi(\vect{x}) = \sum_{i} x_{i} \log x_{i}$, and the
resulting update is \engterm{exponentiated gradient}: $x_{k+1,i} \propto
x_{k,i} \exp(-\eta [\grad f(\vect{x}_{k})]_{i})$, a multiplicative
update that keeps the iterate on the simplex automatically and needs no
projection. The payoff is the dependence of the convergence rate on the
dimension. Euclidean gradient descent on the simplex carries a
worst-case rate scaling as $\sqrt{n}$ in the dimension, while
exponentiated gradient scales as $\sqrt{\log n}$, an exponential
improvement that matters when the decision is a distribution over many
alternatives, as in online portfolio selection, boosting, and the
weighted-majority algorithm for combining expert predictions. The
unifying reading: the right first-order method matches its distance
measure to the constraint set, Euclidean geometry for a box or a ball
and entropic geometry for a simplex, and mirror descent is the single
template from which projected gradient and exponentiated gradient both
fall out by choosing $\psi$. The engineer facing a large simplex-
constrained problem should reach for the multiplicative update, not the
projected Euclidean step, because the $\sqrt{\log n}$ against $\sqrt{n}$
gap is the difference between a method that scales to a million
alternatives and one that does not.
\end{mastery}

\subsection{Constrained descent: the projected gradient}

The gradient methods above solve unconstrained problems, and the
chapter reduces a constrained problem to that case through penalty,
barrier, or projection. The projection route is the most transparent
of the three and deserves to be developed, because it keeps every
iterate feasible and reuses the whole first-order apparatus unchanged.

\begin{definition}[Projected gradient descent]
For the problem $\min_{\vect{x} \in \mathcal{C}} f(\vect{x})$ over a
closed convex feasible set $\mathcal{C}$, \engterm{projected gradient
descent} alternates an unconstrained gradient step with a projection
back onto $\mathcal{C}$:
\[
\vect{x}_{k+1} = \Pi_{\mathcal{C}}\!\left(\vect{x}_{k} - \eta_{k}
\grad f(\vect{x}_{k})\right),
\qquad
\Pi_{\mathcal{C}}(\vect{z}) = \argmin_{\vect{u} \in \mathcal{C}}
\norm{\vect{u} - \vect{z}}_{2},
\]
where $\Pi_{\mathcal{C}}$ is the Euclidean projection onto
$\mathcal{C}$.
\end{definition}

The projection is itself a convex optimisation (minimise distance to
$\vect{z}$ over $\mathcal{C}$), so the method is only cheap when the
projection is cheap, and for the feasible sets an engineer meets most
often it is. Projection onto a box $[\vect{l}, \vect{u}]$ clips each
coordinate to its bounds; projection onto a Euclidean ball of radius
$r$ scales a point outside the ball back to the surface; projection
onto a half-space moves a violating point orthogonally onto the
bounding hyperplane; projection onto the probability simplex is a
sort-and-threshold that costs $O(n \log n)$. The method inherits the
convergence rate of unconstrained gradient descent, geometric on a
strongly convex objective, because the projection onto a convex set is
\engterm{nonexpansive}: it never increases the distance between two
points, so it cannot spoil the contraction the gradient step provides
(\cref{fig:vol02:ch15:projected-gradient}).

\input{volumes/02-form/ch15-optimisation/figures/fig-projected-gradient.tex}

\paragraph{Worked example: projection onto the simplex.}
A resource-allocation problem constrains the decision to be a
distribution: $\vect{x} \geq \vect{0}$ and $\vect{1}^{\transpose}
\vect{x} = 1$. Take the gradient step's output $\vect{z} = (0.8, 0.1,
0.6, -0.3)$ and project it onto the simplex. The projection has the
form $x_{i} = \max(z_{i} - \tau, \; 0)$ for a single scalar threshold
$\tau$ fixed by the requirement $\sum_{i} \max(z_{i} - \tau, 0) = 1$.
Sorting $\vect{z}$ descending gives $(0.8, 0.6, 0.1, -0.3)$. Try
keeping the top two active: $\tau = (0.8 + 0.6 - 1)/2 = 0.2$, which
gives $x = (0.6, 0, 0.4, 0)$ after checking that the third-largest
entry $0.1$ falls below $\tau = 0.2$ and is correctly zeroed. The
projected point $(0.6, 0, 0.4, 0)$ sums to one, is nonnegative, and is
the closest simplex point to $\vect{z}$. The soft-threshold shape of
the answer is not a coincidence; it is the same shrink-and-clip
structure that soft-thresholding produces for the $\ell_{1}$ proximal
operator, because both are projections and proximal maps of piecewise-
linear convex functions, and the two constructions are the same
machinery seen from two sides.

\subsection{Two more first-order workhorses: coordinate descent and
Frank-Wolfe}

Two methods sit alongside the gradient family and win on problems where
the full gradient step is either too expensive or lands outside the
feasible set. Both reduce each iteration to a subproblem cheaper than a
gradient evaluation, and both are the default in a corner of practice
the plain gradient method serves poorly.

\engterm{Coordinate descent} updates one coordinate at a time, holding
the rest fixed, cycling through the coordinates or picking them at
random. For a separable-plus-quadratic objective the per-coordinate
minimisation is a scalar solve in closed form, so a full sweep costs
one gradient evaluation's worth of arithmetic while making progress on
every coordinate as it goes. Take the quadratic $f(x_{1}, x_{2}) =
x_{1}^{2} + x_{2}^{2} + x_{1} x_{2} - x_{1}$, whose exact minimum solves
$\grad f = (2 x_{1} + x_{2} - 1, \; 2 x_{2} + x_{1}) = \vect{0}$ at
$(x_{1}^{*}, x_{2}^{*}) = (2/3, -1/3)$. Coordinate descent from
$(0, 0)$ minimises over $x_{1}$ first: $\partial f / \partial x_{1} =
2 x_{1} + x_{2} - 1 = 0$ at fixed $x_{2} = 0$ gives $x_{1} = 1/2$. Then
over $x_{2}$: $2 x_{2} + x_{1} = 0$ at $x_{1} = 1/2$ gives $x_{2} =
-1/4$. The next sweep gives $x_{1} = (1 - x_{2})/2 = 5/8$, $x_{2} =
-x_{1}/2 = -5/16$, already close to $(0.667, -0.333)$, and the sweeps
contract geometrically at a rate set by the off-diagonal coupling. When
the objective is fully separable (no cross terms) a single sweep is
exact; the coupling is what forces iteration, and a strongly coupled
objective is where coordinate descent slows and a full-gradient method
regains the advantage. Coordinate descent is the workhorse behind the
fastest LASSO solvers (each coordinate's soft-threshold update is a
scalar closed form) and behind the coordinate-wise updates in many
large-scale statistical fitting routines, precisely because it never
forms or stores a full gradient.

\engterm{Frank-Wolfe} (the conditional-gradient method) solves a
constrained problem $\min_{\vect{x} \in \mathcal{C}} f(\vect{x})$ over a
convex feasible set $\mathcal{C}$ without any projection, which is its
advantage when the projection of the projected-gradient method is
expensive but linear minimisation over $\mathcal{C}$ is cheap. Each
iteration linearises the objective at the current point and minimises
the linear model over $\mathcal{C}$, which lands at a vertex:
\[
\vect{s}_{k} = \argmin_{\vect{s} \in \mathcal{C}} \grad
f(\vect{x}_{k})^{\transpose} \vect{s}, \qquad
\vect{x}_{k+1} = (1 - \gamma_{k}) \vect{x}_{k} + \gamma_{k}
\vect{s}_{k},
\]
with step $\gamma_{k} = 2/(k+2)$. The next iterate is a convex
combination of the current point and the vertex, so it stays feasible
by construction with no projection. Over the probability simplex the
linear subproblem is trivial: the minimising vertex is the unit vector
$\vect{e}_{j}$ on the most negative gradient coordinate $j =
\argmin_{i} [\grad f(\vect{x}_{k})]_{i}$. A single step on $f(\vect{x})
= \tfrac{1}{2}\norm{\vect{x} - \vect{c}}^{2}$ with $\vect{c} = (0.2,
0.5, 0.3)$ from the simplex centre $\vect{x}_{0} = (1/3, 1/3, 1/3)$: the
gradient is $\vect{x}_{0} - \vect{c} = (0.133, -0.167, 0.033)$, its most
negative entry is coordinate 2, the vertex is $\vect{s}_{0} = (0, 1, 0)$,
and with $\gamma_{0} = 2/2 = 1$ the first Frank-Wolfe step jumps
straight to that vertex before the shrinking steps pull the iterate back
toward the interior optimum. Frank-Wolfe's iterates are sparse convex
combinations of vertices, which is why it is the method of choice when
the feasible set is a simplex, a trace-norm ball, or a flow polytope
and a sparse or low-rank solution is wanted; it converges at the
$O(1/k)$ rate, slower than projected gradient's geometric rate on a
strongly convex problem but bought at the price of a linear solve rather
than a projection.
...
\input{volumes/02-form/ch15-optimisation/figures/fig-soft-threshold.tex}
...
\input{volumes/02-form/ch15-optimisation/figures/fig-method-comparison.tex}
...
\Cref{fig:vol02:ch15:sgd-variance} plots the two curves the choice
balances.

\input{volumes/02-form/ch15-optimisation/figures/fig-sgd-variance.tex}

\paragraph{Worked example: the noise ball of a constant step.}
A constant step size does not let SGD converge to a point; it lets
SGD converge to a neighbourhood, and the size of that neighbourhood is
worth computing because it sets the floor on the achievable loss. On a
$\mu$-strongly convex quadratic with minibatch gradient variance
$\sigma_{b}^{2} = \sigma^{2}/b$, the expected squared distance to the
optimum under a constant step $\eta$ obeys the recursion
\[
\E \norm{\boldsymbol{\theta}_{k+1} - \boldsymbol{\theta}^{*}}^{2}
\leq (1 - \eta \mu) \, \E \norm{\boldsymbol{\theta}_{k} -
\boldsymbol{\theta}^{*}}^{2} + \eta^{2} \sigma_{b}^{2},
\]
a contraction toward the optimum fighting a constant noise injection.
The fixed point of the recursion is the equilibrium error
\[
\E \norm{\boldsymbol{\theta}_{\infty} - \boldsymbol{\theta}^{*}}^{2}
= \frac{\eta \sigma_{b}^{2}}{\mu},
\]
the radius of the \engterm{noise ball} the iterate rattles around in
once the transient has decayed. Two readings follow directly. Halving
the step halves the equilibrium error but also halves the contraction
rate $\eta \mu$, so a smaller step buys a tighter ball at the cost of a
slower approach, the same trade the diminishing schedule
$\eta_{k} \to 0$ resolves by shrinking the ball to a point over time.
Doubling the batch halves $\sigma_{b}^{2}$ and halves the ball at fixed
step, which is the convergence-side reason to enlarge a batch late in
training. With $\sigma^{2} = 4$, $b = 64$, $\mu = 1$, and $\eta = 0.1$,
the equilibrium error is $0.1 \times (4/64) / 1 = 6.25 \times 10^{-3}$,
a distance of about $0.08$ from the optimum that no number of further
constant-step iterations will improve; reaching a tighter answer
requires dropping the step, growing the batch, or switching to a
variance-reduced method. The engineer who sees an SGD loss curve
flatten well above the known noise floor should suspect the noise ball
before suspecting the model, and the fix is a step-size decay, not more
epochs at the same step.
...
\Cref{fig:vol02:ch15:branch-bound-tree} draws the search tree, the
incumbent's rise from $200$ to $220$, and the pruning that closes it.

\input{volumes/02-form/ch15-optimisation/figures/fig-branch-bound-tree.tex}
...
\input{volumes/02-form/ch15-optimisation/figures/fig-truss-sizing.tex}
...
\input{volumes/02-form/ch15-optimisation/figures/fig-gauss-newton-fit.tex}
...
\subsection{Applied vignette: fuel-optimal powered descent}

A lander approaching a planetary surface must fire its engine to arrive
at the target with zero velocity while spending the least propellant,
and the thrust it can command is bounded below (an engine that cannot
throttle to zero) and above (a structural limit) and points within a
cone about the vertical. The naive formulation is nonconvex, and the
result that made autonomous rocket landing routine is that the problem
convexifies exactly into an SOCP. The vignette shows a hard-looking
control problem collapsing onto the conic apparatus of this section.

\paragraph{The brief.}
Over a fixed descent time discretised into $H$ steps of length
$\Delta t$, the lander's position $\vect{p}_{k} \in \R^{3}$ and
velocity $\vect{v}_{k} \in \R^{3}$ evolve under gravity $\vect{g}$ and
a commanded thrust acceleration $\vect{u}_{k}$:
\[
\vect{v}_{k+1} = \vect{v}_{k} + \Delta t\,(\vect{u}_{k} + \vect{g}),
\qquad
\vect{p}_{k+1} = \vect{p}_{k} + \Delta t\, \vect{v}_{k}
+ \tfrac{1}{2}\Delta t^{2}\,(\vect{u}_{k} + \vect{g}).
\]
The boundary conditions fix the initial state $(\vect{p}_{0},
\vect{v}_{0})$ and demand a soft touchdown $\vect{p}_{H} =
\vect{p}_{\text{target}}$, $\vect{v}_{H} = \vect{0}$. The engine
imposes three limits at every step: a magnitude ceiling
$\norm{\vect{u}_{k}}_{2} \leq u_{\max}$, a magnitude floor
$\norm{\vect{u}_{k}}_{2} \geq u_{\min} > 0$, and a pointing constraint
that the thrust stay within an angle $\theta$ of vertical. The
objective minimises total propellant, proportional to $\sum_{k}
\norm{\vect{u}_{k}}_{2} \Delta t$, the accumulated thrust magnitude.

\paragraph{Where the nonconvexity lives.}
The dynamics are affine in the decision variables $\set{\vect{u}_{k}}$,
so they contribute only linear equality constraints. The ceiling
$\norm{\vect{u}_{k}}_{2} \leq u_{\max}$ is a second-order cone
constraint, convex. The pointing constraint $\vect{u}_{k}$ within angle
$\theta$ of vertical is $\norm{\vect{u}_{k}}_{2} \cos\theta \leq
u_{k,z}$, again a second-order cone constraint, convex. The objective,
a sum of norms, is convex. The single nonconvex element is the thrust
floor $\norm{\vect{u}_{k}}_{2} \geq u_{\min}$: a lower bound on a norm
carves out the region \emph{outside} a ball, which is not convex, and a
naive solver stalls on it.

\paragraph{The convexification.}
Introduce a slack variable $\Gamma_{k}$ that stands in for the thrust
magnitude, replace the objective with $\sum_{k} \Gamma_{k} \Delta t$,
and replace the three thrust constraints with
\[
\norm{\vect{u}_{k}}_{2} \leq \Gamma_{k}, \qquad
u_{\min} \leq \Gamma_{k} \leq u_{\max}, \qquad
\Gamma_{k} \cos\theta \leq u_{k,z}.
\]
The first is a second-order cone constraint tying the true magnitude to
its surrogate; the second is now a pair of plain linear bounds on the
scalar $\Gamma_{k}$, convex; the third is linear. The nonconvex floor
has been lifted into a linear bound on the slack, and the whole program
is an SOCP. The subtlety that makes this exact rather than a relaxation:
at the optimum the surrogate constraint $\norm{\vect{u}_{k}}_{2} \leq
\Gamma_{k}$ is always tight, $\norm{\vect{u}_{k}}_{2} = \Gamma_{k}$,
because the objective drives $\Gamma_{k}$ down until it meets the norm.
The relaxation therefore recovers the true optimum of the nonconvex
problem, a property called \engterm{lossless convexification}, proved
for exactly this powered-descent problem and the reason a spacecraft
can solve a guaranteed-optimal landing trajectory onboard in
milliseconds with a general SOCP solver.

\paragraph{What the vignette teaches.}
The powered-descent problem is the second-order-cone counterpart of the
truss LP and the battery calibration: a plain-English brief carried
into a formal program whose structural class decides its tractability.
The recognition that the dynamics are affine, the ceiling and pointing
limits are cones, and the troublesome floor lifts to a linear bound on
a slack is the entire difficulty; once the class is named as SOCP the
solver is a library call. The lesson restates the chapter's first
diagnostic in a control setting: identify the structural class before
choosing a method, and a problem that looks nonconvex may convexify
exactly when the one nonconvex constraint can be traded for a slack
that the objective keeps tight.

\subsection{Geometric programming}

A separate convex class hides inside a wide family of engineering cost
and sizing models that look thoroughly nonconvex in the design
variables and become convex under a single change of variables. A
\engterm{monomial} is a function $c\, x_{1}^{a_{1}} x_{2}^{a_{2}} \cdots
x_{n}^{a_{n}}$ with a positive coefficient $c > 0$ and any real
exponents $a_{j}$, on the positive orthant $\vect{x} > \vect{0}$. A
\engterm{posynomial} is a sum of monomials. Areas, volumes, stresses,
pressure drops, RC delays, and material costs are overwhelmingly
monomials or posynomials in their design variables, because physical
scaling laws are power laws.

\begin{definition}[Geometric program]
A \engterm{geometric program} (GP) minimises a posynomial objective
subject to posynomial inequality constraints (each upper-bounded by
one) and monomial equality constraints:
\[
\min_{\vect{x} > \vect{0}} \; f_{0}(\vect{x})
\quad \text{subject to} \quad
f_{i}(\vect{x}) \leq 1, \;\; h_{j}(\vect{x}) = 1,
\]
with $f_{0}, f_{i}$ posynomials and $h_{j}$ monomials.
\end{definition}

A GP is not convex in $\vect{x}$. The transform that convexifies it is
the substitution $y_{j} = \log x_{j}$, so $x_{j} = e^{y_{j}}$. Under it
a monomial $c \prod_{j} x_{j}^{a_{j}}$ becomes $e^{\vect{a}^{\transpose}
\vect{y} + \log c}$, an exponential of an affine function, and a
posynomial becomes a sum of such exponentials. Taking the logarithm of
the objective and each constraint turns the whole program into
minimising a $\log$-$\sum$-$\exp$ function subject to $\log$-$\sum$-$\exp$
constraints and affine equalities, and $\log \sum_{k} e^{\vect{a}_{k}^{
\transpose} \vect{y} + b_{k}}$ is convex (a standard entry of the convex-
function library). The GP is therefore a convex program in $\vect{y}$
and solves in polynomial time by interior-point methods, however
nonconvex it looked in $\vect{x}$ (\cref{fig:vol02:ch15:geometric-program}).

\paragraph{Worked GP: the cheapest open-top box.}
Size an open-top rectangular storage box of width $w$, depth $d$, and
height $h$ to enclose a required volume while spending the least on
material. The volume requirement is $w d h \geq V$, and the material
cost is proportional to the surface area of the base and four sides,
$S = w d + 2 w h + 2 d h$ (no top). The problem
\[
\min_{w, d, h > 0} \; w d + 2 w h + 2 d h
\quad \text{subject to} \quad V\, w^{-1} d^{-1} h^{-1} \leq 1
\]
has a posynomial objective and a single monomial-in-the-constraint of
the posynomial-$\leq 1$ form, so it is a GP. The symmetry of the two
side pairs suggests $w = d$ at the optimum, and the KKT conditions
confirm it: the base contributes area proportional to $w d$ and each
side pair to $w h$, so balancing marginal cost against the volume
multiplier gives $w = d$ and, working the stationarity, $h = w/2$. The
volume constraint binds, $w \cdot w \cdot (w/2) = V$, so $w^{3} = 2V$
and $w^{*} = d^{*} = (2V)^{1/3}$, $h^{*} = (2V)^{1/3}/2$. For $V =
1\,\si{\cubic\meter}$ this is $w^{*} = d^{*} \approx 1.26\,\si{\meter}$
and $h^{*} \approx 0.63\,\si{\meter}$, a base twice as wide as the box
is tall, the standard proportion for a material-minimal open box. The
point of the example is not the box but the recognition: a designer who
sees a cost that is a sum of power-law terms and constraints that are
power-law inequalities has a GP, and a GP is a convex program wearing a
nonconvex costume, solvable to global optimality by a library call once
the log-transform is named.

\input{volumes/02-form/ch15-optimisation/figures/fig-geometric-program.tex}
...
\input{volumes/02-form/ch15-optimisation/figures/fig-pareto-front.tex}
...
\Cref{fig:vol02:ch15:nonconvex-pareto} shows the blind spot and the
repair on a re-entrant front.

\input{volumes/02-form/ch15-optimisation/figures/fig-nonconvex-pareto.tex}

\paragraph{Worked example: a re-entrant front the weighted sum
misses.}
A concrete two-objective problem exhibits the blind spot in closed
form. Minimise the pair $f_{1}(x) = x$ and $f_{2}(x) = (x - 1)^{2}(x -
3) + 4$ over $x \in [0, 3]$, a design parameter traded against a
performance measure whose curve dips, rises, and dips again. Sweeping
a weight ratio $w = w_{2}/w_{1}$ and minimising $x + w \, f_{2}(x)$
finds a stationary point wherever $1 + w f_{2}'(x) = 0$, with
$f_{2}'(x) = 3(x - 1)(x - 7/3)$. For a range of small positive weights
the scalarised objective has two competing local minima, one on each
convex arc of the front, and the global minimiser jumps discontinuously
from the left arc to the right arc as $w$ crosses a threshold, never
resting on the middle arc between $x \approx 1.6$ and $x \approx 2.1$.
Those middle designs are Pareto optimal: no feasible $x$ improves both
$f_{1}$ and $f_{2}$ against them. The weighted-sum sweep skips them
entirely, leaving a visible gap in the reported front exactly where the
front is re-entrant. The $\epsilon$-constraint method recovers them by
solving $\min_{x} f_{2}(x)$ subject to $f_{1}(x) = x \leq \epsilon$ and
sweeping $\epsilon$ across $[0, 3]$: at each $\epsilon$ the constraint
pins $x$ and the middle-arc designs appear one by one. The example
turns the geometric statement of the figure into arithmetic: the
weighted sum sees the convex hull of the front, and the parts of the
front strictly inside that hull are invisible to it.
