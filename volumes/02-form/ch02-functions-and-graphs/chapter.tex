...
\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-function-zoo.tex}
...
\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-composition-inverse.tex}
...
\paragraph{Worked example: domain bookkeeping through a composition
chain.}
A composition is only defined where the output of each stage lands
inside the domain of the next, and tracking that condition through a
chain is the bookkeeping a draft calculation most often skips. Consider
the chain $h \circ g \circ f$ with $f(x) = x - 3$, $g(u) = \sqrt{u}$,
and $h(w) = \ln(w)$, evaluated on a real input $x$. The inner stage
$f$ accepts all real $x$. The middle stage $g = \sqrt{\cdot}$ requires
its input non-negative, so it accepts $f(x) = x - 3 \geq 0$, that is,
$x \geq 3$. The outer stage $h = \ln(\cdot)$ requires its input
strictly positive, so it needs $g(f(x)) = \sqrt{x - 3} > 0$, that is,
$x - 3 > 0$, that is, $x > 3$. The strict inequality at the last stage
overrides the non-strict one at the middle stage: the point $x = 3$
passes the square root (which returns zero there) but fails the
logarithm (which diverges at zero), so the domain of the full chain is
the open ray $x > 3$, not the closed ray $x \geq 3$ the middle stage
alone would allow. The rule the example teaches is to propagate the
domain forward stage by stage, intersecting the constraint each stage
imposes on the running output, and to let the strictest constraint
win. An engineer who reads the domain off the inner function alone, or
off the outer function alone, gets a domain that is wrong at exactly
the boundary points where calculations break, which is where a program
calls $\ln$ on zero and returns negative infinity into the rest of the
pipeline.
...
\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-even-odd-symmetry.tex}
...
\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-transformations.tex}
...
\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-family-fit.tex}
...
\paragraph{Worked example: evaluating a polynomial without waste.}
A polynomial written in the form $a_n x^n + \cdots + a_1 x + a_0$ and
evaluated term by term recomputes each power of $x$ from scratch,
costing roughly $n^2/2$ multiplications for a degree-$n$ polynomial and
accumulating rounding error in every separate power. The rewrite that
removes the waste is \engterm{Horner's method}, the nested form
\[
  p(x) = a_0 + x\bigl(a_1 + x\bigl(a_2 + \cdots + x(a_{n-1} +
    x\,a_n)\bigr)\bigr),
\]
evaluated from the innermost bracket outward. Take
$p(x) = 2x^3 - 6x^2 + 2x - 1$ at $x = 3$. The nested form is
$((2 \cdot 3 - 6) \cdot 3 + 2) \cdot 3 - 1$: the inner bracket is
$2 \cdot 3 - 6 = 0$, the next is $0 \cdot 3 + 2 = 2$, the outer is
$2 \cdot 3 - 1 = 5$, so $p(3) = 5$ in three multiplications and three
additions rather than the six multiplications the term-by-term form
would use. Horner's method is the algorithm every numerical library
uses to evaluate a polynomial, and the same nested sweep, carried one
step further, is the synthetic division that factored the cubic two
examples back. Fewer operations means less accumulated rounding, so
the nested form is both the faster and the more accurate evaluation,
which is why it is the default rather than an optimisation.

\begin{mastery}
The way a function is written changes how accurately a computer
evaluates it, even when the algebra is identical, because floating-point
arithmetic rounds at every step and some arrangements of the same
expression cancel large nearly-equal numbers where others do not. The
polynomial $p(x) = (x - 1)^6$ evaluated near $x = 1$ from its expanded
form $x^6 - 6x^5 + 15x^4 - 20x^3 + 15x^2 - 6x + 1$ subtracts terms of
order one to leave a result of order $10^{-12}$, and the rounding error
in the large terms swamps the tiny true value, so the expanded form
returns visible noise where the factored form returns a clean near-zero.
The general name for this hazard is \engterm{catastrophic
cancellation}: subtracting two numbers that agree in their leading
digits discards those digits and promotes the rounding error in the
trailing digits to the leading role. The engineering habit that follows
is to prefer the arrangement that avoids differencing nearly equal
quantities, to use the factored form near a root, to replace
$\sqrt{1 + x} - 1$ by $x/(\sqrt{1 + x} + 1)$ for small $x$, and to
reach for the quadratic formula's numerically stable variant when
$b^2 \gg 4ac$. The function on paper and the function in the machine
are the same map; the arithmetic that connects them is not exact, and
carrying that distinction is the difference between a formula that
computes and one that merely reads correctly. The subject is developed
in Volume II Chapter 16 on numerical methods.
\end{mastery}

\paragraph{Worked example: interpolating a device curve, and why a
high-degree polynomial fails.}
A manufacturer supplies a sensor characteristic as a short table of
calibration points, and the engineer needs the value between the
tabulated points. The first instinct is to fit one polynomial through
all the nodes, since a polynomial of degree $n - 1$ passes exactly
through any $n$ points. The instinct is a trap. A single high-degree
polynomial forced through equally spaced nodes oscillates wildly
between them, the swings growing toward the ends of the range, an
effect named the \engterm{Runge phenomenon} after the function
$1/(1 + 25x^{2})$ on which it was first shown.
\Cref{fig:vol02:ch02:spline-vs-polynomial} sets the global polynomial
beside the working alternative. The polynomial in panel (a) hits every
node and is useless between them, dipping below zero where the device
curve is strictly positive. The remedy in panel (b) is the
\engterm{cubic spline}, a piecewise function built from a separate
cubic on each interval between nodes, with the cubics joined so that
the value, the first derivative, and the second derivative all match
where two pieces meet. The matching conditions buy a curve that is
smooth to the eye and to two derivatives, stays close to the
underlying characteristic, and has no oscillation budget to spend
between nodes. The engineering rule that follows is sharp: interpolate
a tabulated device curve with a low-degree piecewise fit, almost
always a cubic spline, and reserve a single global polynomial for the
two or three nodes where its degree is too low to oscillate. The
spline is the standard interpolant in instrument firmware,
finite-element shape functions, and computer-graphics curve drawing
precisely because it interpolates without inventing structure the data
never had.

\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-spline-vs-polynomial.tex}
...
\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-rational-asymptotes.tex}
...
\paragraph{Worked example: a partial-fraction decomposition, worked.}
The decomposition the box names is worth carrying out once by hand, so
the algebra is not a black box when Chapter 3 leans on it. Take
$r(x) = 1/[(x - 1)(x + 2)]$, a rational function with two simple poles
at $x = 1$ and $x = -2$ and a numerator of degree zero, so the
degree condition $\deg p < \deg q$ holds. Write the target form with
one unknown constant per pole,
\[
  \frac{1}{(x - 1)(x + 2)} = \frac{A}{x - 1} + \frac{B}{x + 2},
\]
and recover the constants by the cover-up rule: multiply through by
$(x - 1)$ and set $x = 1$ to isolate $A = 1/(1 + 2) = 1/3$; multiply
through by $(x + 2)$ and set $x = -2$ to isolate $B = 1/(-2 - 1) =
-1/3$. The cover-up rule works because each multiplication cancels the
pole whose constant is wanted and every other term carries a factor
that vanishes at the substituted value. The decomposition is therefore
$r(x) = \tfrac{1/3}{x - 1} - \tfrac{1/3}{x + 2}$, and
\Cref{fig:vol02:ch02:partial-fraction} shows the full curve as the sum
of the two simple-pole terms on a window between the poles. The
engineering payoff arrives in the next chapter: a transfer function
that factors into simple poles inverts term by term, and each term
$\tfrac{A_i}{s - s_i}$ inverts to a single exponential mode
$A_i e^{s_i t}$, so a two-pole system rings as the sum of two
exponentials whose rates are the two pole locations. The partial
fraction is the bridge from the pole map to the time response.

\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-partial-fraction.tex}

\paragraph{Worked example: a repeated pole and the cover-up rule
extended.}
The cover-up rule as stated handles distinct simple poles; a repeated
pole needs one more step, and working it once shows where the extra term
comes from. Take $r(x) = 1/[(x - 1)^{2}(x + 1)]$, with a double pole at
$x = 1$ and a simple pole at $x = -1$. A repeated pole of multiplicity
two contributes two terms, one for each power of the offending factor,
so the target form is
\[
  \frac{1}{(x - 1)^{2}(x + 1)}
  = \frac{A}{x + 1} + \frac{B}{x - 1} + \frac{C}{(x - 1)^{2}}.
\]
The constant $A$ on the simple pole and the constant $C$ on the highest
power of the repeated pole both come from the plain cover-up rule.
Multiply by $(x + 1)$ and set $x = -1$: $A = 1/(-1 - 1)^{2} = 1/4$.
Multiply by $(x - 1)^{2}$ and set $x = 1$: $C = 1/(1 + 1) = 1/2$. The
middle constant $B$, on the lower power of the repeated factor, does not
survive the cover-up because setting $x = 1$ after multiplying by
$(x - 1)^{2}$ kills its term; the standard recovery is to differentiate
$(x - 1)^{2} r(x) = 1/(x + 1)$ once with respect to $x$ and then set
$x = 1$, giving $B = -1/(x + 1)^{2}\big|_{x = 1} = -1/4$. The
decomposition is $r(x) = \tfrac{1/4}{x + 1} - \tfrac{1/4}{x - 1} +
\tfrac{1/2}{(x - 1)^{2}}$, and
\Cref{fig:vol02:ch02:repeated-pole} draws the result. The engineering
reading is in the sign behaviour. The simple pole at $x = -1$ carries a
$1/(x + 1)$ term that flips sign across the pole, positive on one side
and negative on the other, the transversal behaviour a simple pole
always shows. The double pole at $x = 1$ is dominated near the pole by
the $1/(x - 1)^{2}$ term, whose squared denominator stays positive on
both sides, so the curve diverges to the same sign from left and right
and does not flip. Pole multiplicity read off the factored denominator
therefore predicts, before any plotting, whether the modelled quantity
reverses sign across a singularity, and in the Laplace setting of the
next chapter a repeated pole is the algebraic marker of a mode that
rings with a $t\,e^{s_i t}$ envelope rather than a plain exponential,
the signature of critical damping.

\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-repeated-pole.tex}
...
\paragraph{Worked example: reading a system off its pole-zero map.}
A rational transfer function $H(s) = N(s)/D(s)$ carries its dynamics
in the locations of the roots of $D$ (the poles) and the roots of $N$
(the zeros), plotted as points in the complex plane.
\Cref{fig:vol02:ch02:pole-zero-map} draws the map for a representative
system and reads the dynamics off the geometry without solving a
single differential equation. The vertical (imaginary) axis is the
stability boundary. A pole in the left half-plane, with negative real
part, is a mode that decays; a pole in the right half-plane, with
positive real part, is a mode that grows without bound and renders the
system unstable; a pole exactly on the imaginary axis is a sustained
oscillation that neither grows nor decays. For a conjugate pole pair
at $s = \sigma \pm \mathrm{j}\omega_{d}$ the geometry splits into two
readings: the real part $\sigma$ sets the decay rate of the mode, so
the time constant of the ringing is $\tau = 1/|\sigma|$, and the
imaginary part $\omega_{d}$ sets the frequency at which the mode rings.
A pole pair close to the imaginary axis is lightly damped and rings for
a long time; the same pair pushed left into the half-plane is heavily
damped and settles quickly. The zeros do not set stability but shape
the response, blocking the input frequencies at which $N(s)$ vanishes.
The whole reading is the system preview the chapter promised: an
engineer who can find the roots of a rational function's numerator and
denominator can read stability, decay rate, and ringing frequency off
their positions, which is the entry move of the control engineering
that Volume IX develops in full.

\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-pole-zero-map.tex}

\paragraph{Worked example: reading the quality factor off a resonance.}
A resonant rational function has a magnitude response that rises to a
peak near its natural frequency and falls away on both sides, and the
sharpness of that peak is the single number a designer most often
wants. \Cref{fig:vol02:ch02:resonance-q} plots the magnitude of a
second-order bandpass, normalised so the peak is one, and two readings
fix the response. The peak frequency is $f_0$, read off the top of the
curve. The two frequencies at which the magnitude has fallen to
$1/\sqrt{2} \approx 0.707$ of the peak, the half-power points, bracket
the bandwidth $\Delta f$, and the \engterm{quality factor} is their
ratio $Q = f_0/\Delta f$. The half-power level is $1/\sqrt{2}$ and not
one half because the labels are amplitudes while ``half power'' is a
statement about the square of the amplitude: power scales as amplitude
squared, so half power is amplitude $1/\sqrt{2}$, which in decibels is
$20\log_{10}(1/\sqrt{2}) = -3\,\si{\deci\bel}$, the origin of the
ubiquitous $-3\,\si{\deci\bel}$ corner. For the curve drawn, the peak
sits at $f/f_0 = 1$ and the half-power crossings sit at about
$0.95 f_0$ and $1.05 f_0$, so $\Delta f \approx 0.1 f_0$ and $Q
\approx 10$. A high-$Q$ circuit, a quartz crystal reaching $Q$ in the
tens of thousands, has a needle-sharp peak and rings for many cycles;
a low-$Q$ circuit has a broad peak and settles fast. The same two
readings, peak location and half-power width, characterise a tuned
antenna, a laser cavity, a struck bell, and a vibration-isolation
mount, and each is the resonant cousin of the pole pair
\Cref{fig:vol02:ch02:pole-zero-map} drew: a high $Q$ is a pole pair
close to the imaginary axis.

\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-resonance-q.tex}
...
\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-piecewise-step.tex}
...
\paragraph{Worked example: a lookup table and its interpolation error.}
Firmware often stores a sensor characteristic as a table of points and
computes intermediate values by joining adjacent entries with straight
segments, which is a piecewise-linear function on the domain, one
segment per table interval. The engineering question is how coarse the
table can be before the straight-line joins depart too far from the
true curve. For a function with bounded curvature, the largest error of
linear interpolation across an interval of width $h$ is at most
$\tfrac{1}{8} h^2 \max|f''|$, the error peaking in the middle of the
interval where the chord sits farthest from the arc. Take a temperature
sensor whose characteristic curves with $\max|f''| = 0.02\,\si
{\per\kelvin}$ over the working range, and suppose the design allows a
$0.01$ interpolation error. Solving $\tfrac{1}{8} h^2 (0.02) \leq 0.01$
gives $h^2 \leq 4$, that is, $h \leq 2\,\si{\kelvin}$, so table entries
every two kelvin hold the interpolation error inside budget, and a
coarser table would not. The example turns a vague ``how many table
points'' into a computed spacing, and the same bound sets the node
count for a piecewise-linear approximation of any function whose second
derivative the engineer can bound, which is the memory-versus-accuracy
trade every embedded lookup table makes. Halving the error needs the
spacing cut by $\sqrt{2}$, since the error scales as $h^2$, the
quadratic-convergence signature that separates piecewise-linear
interpolation from the cubic spline's faster $h^4$ approach.
...
\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-hysteresis-deadband.tex}
...
\begin{mastery}
The argument of an exponential, a logarithm, or a trigonometric
function must be dimensionless, and checking that it is is one of the
fastest audits of a written formula. The reason is the series
definition: $e^{x} = 1 + x + x^2/2 + \cdots$ adds $x$ to $x^2$ to $1$,
and terms of different dimension cannot be added, so $x$ can only be a
pure number. The same argument bars a logarithm of a dimensioned
quantity, because $\ln x$ inherits the same series through
$\ln(1 + u)$, and it explains why every physical exponential in the
chapter carries its argument as a ratio: the decay $e^{-t/\tau}$
divides time by a time constant, the Arrhenius factor
$e^{-E_a/k_B T}$ divides an energy by an energy, the sinusoid
$\sin(\omega t)$ multiplies an angular frequency in radians per second
by a time to leave radians, which are dimensionless by construction. A
formula that writes $\ln(p)$ for a pressure $p$ is therefore
incomplete: what is meant is $\ln(p/p_{\text{ref}})$ for some reference
pressure, and the reference has been dropped, which is the same
unstated-reference-level defect the graph-reading section names for
decibel and pH axes. When a logarithm of an apparently dimensioned
quantity appears, the missing reference is hiding in the additive
constant, since $\ln(p/p_{\text{ref}}) = \ln p - \ln p_{\text{ref}}$,
and the engineer who restores it recovers a formula whose every
argument is honestly dimensionless. The habit is Volume I's
unit-carrying discipline applied inside the transcendental functions,
where a dimension error does not announce itself as a mismatched unit
in the answer but as a silently meaningless argument.
\end{mastery}
...
\paragraph{Worked example: fitting a saturating-growth model.}
Pure exponential growth is the small-population limit of a process
that eventually runs into a resource ceiling, and the engineer who
fits an exponential to data that will saturate is choosing a family
the physics outgrows. The \engterm{logistic} model
$N(t) = K/\bigl(1 + ((K - N_{0})/N_{0}) e^{-rt}\bigr)$ carries the
ceiling explicitly through the \engterm{carrying capacity} $K$, with
$r$ the small-population rate constant and $N_{0}$ the initial count.
\Cref{fig:vol02:ch02:logistic-fit} fits the model to a population that
grows then levels off, and three readings recover the parameters from
features of the curve. The upper asymptote reads off $K$, the level
the data approaches. The inflection, the point of steepest rise, sits
at $N = K/2$ and its time fixes the combination of $r$ and $N_{0}$.
The early-time slope matches the pure exponential $N_{0} e^{rt}$, so
$r$ reads off the initial rise on a semilog plot of $N$ against $t$
while $N \ll K$, before the saturation term bends the curve. The
logistic linearises under the logit transform of Section~2.6: plotting
$\ln\bigl(N/(K - N)\bigr)$ against $t$ gives a straight line of slope
$r$ once $K$ is known, which is the working route to $r$ from the full
dataset rather than the early points alone. The same curve, under
other names, describes an autocatalytic reaction, a technology
adoption curve, a tumour-volume model, and the charge of a
self-limiting battery; recognising the saturating-S shape is
recognising that a resource ceiling has entered the mechanism. The
logistic equation is also the entry point to nonlinear dynamics, where
its discrete cousin, the logistic map, is the standard first example of
a route to chaos \cite{v2c02-text:strogatz1994}, a reminder that the
same functional form the engineer fits as a growth curve carries
behaviour far richer than the smooth S once its parameters are pushed.

\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-logistic-fit.tex}

\paragraph{Worked example: why a distribution is log-normal.}
A quantity built by multiplying many independent random factors ends up
distributed log-normally, and understanding why is understanding what
the logarithm does to a product. If a particle's final size is set by a
sequence of independent fragmentation steps, each removing a random
fraction, then the size is a product of random factors, and the
logarithm of the size is a sum of the logarithms of those factors. A
sum of many independent contributions tends to a normal distribution
by the central-limit result of Volume I Chapter 6, so $\ln(\text{size})$
is approximately normal, which is the definition of the size itself
being log-normal. The engineering consequences are concrete. Plotting
the size on a linear axis gives a lopsided distribution with a long
upper tail that hides its structure; plotting the logarithm of the size,
or using a log axis, straightens the tail into the symmetric bell the
central-limit result predicts, and a log-probability plot straightens
it further into a line whose slope reads the spread. The log-normal
appears wherever a quantity is a product of proportional effects:
crushed-rock and aerosol particle sizes, the strength of brittle
materials set by the largest flaw, income distributions, and the
time-to-failure of components under multiplicative wear. Recognising
that a right-skewed quantity may be log-normal is recognising that its
natural axis is logarithmic and its natural model is a normal
distribution on that axis, which turns an awkward skewed fit into a
routine two-parameter one.
...
\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-semilog-decay.tex}
...
\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-thermistor-fit.tex}
...
\paragraph{Worked example: how transformations act on log axes.}
The shift-and-scale vocabulary of Section~2.1 changes character on a
logarithmic axis, and reading the change correctly is what lets an
engineer compare two curves on the same log plot. Multiplying the
plotted quantity by a constant, $y \mapsto c y$, becomes a vertical
\emph{shift} on a log axis, because $\log(c y) = \log c + \log y$, so
two power laws that differ only in their prefactor $A$ appear as
parallel lines on log-log axes, offset vertically by $\log(A_{2}/
A_{1})$ and never crossing. Two exponential decays that differ only in
their initial value appear the same way on semilog axes, parallel
lines with the same slope. The diagnostic follows at once: parallel
lines on a log plot are the same mechanism at different scale, while
lines of different slope are different mechanisms, a different exponent
in the power-law case or a different rate constant in the exponential
case. Scaling the \emph{input}, $x \mapsto bx$, behaves differently on
the two axis types. On a log-log plot, $y = A(bx)^{n} = (A b^{n})
x^{n}$ keeps the slope $n$ and shifts the line vertically, so a change
of input units (metres to millimetres, say) slides a power law up or
down without tilting it. On a semilog plot a change of input scale
\emph{does} change the slope, because the horizontal axis is still
linear and stretching it stretches the apparent rate. The working
habit is to read a log plot for slope first, since slope carries the
mechanism, and for vertical offset second, since offset carries the
scale, and to remember that a unit change on the input slides a
power-law line but tilts an exponential one.

\paragraph{Worked example: the factor of ten and the factor of twenty.}
The decibel is defined on a power ratio as $10\log_{10}(P/P_{\text{ref}})$
and on an amplitude ratio as $20\log_{10}(A/A_{\text{ref}})$, and the
two prefactors are the single most common source of decibel error. The
$20$ is not a different definition; it is the $10$ carried through the
fact that power scales as amplitude squared, so
$10\log_{10}(A^2/A_{\text{ref}}^2) = 20\log_{10}(A/A_{\text{ref}})$. A
voltage that doubles is $20\log_{10} 2 \approx 6\,\si{\deci\bel}$ up as
an amplitude, and delivers four times the power into a fixed resistance,
which is $10\log_{10} 4 \approx 6\,\si{\deci\bel}$ up as a power: the
same $6\,\si{\deci\bel}$, reached by two routes, which is the check that
the prefactor was chosen correctly. The trap is to report a voltage
gain with the power prefactor or the reverse: a tenfold voltage gain is
$20\,\si{\deci\bel}$, and calling it $10\,\si{\deci\bel}$ understates it
by a factor of $\sqrt{10} \approx 3.16$ in the quantity that matters.
The working rule is to name the quantity before the prefactor: power,
intensity, and mean-square quantities take the $10$; voltage, current,
pressure, and other amplitudes take the $20$.

\begin{mastery}
The slope of a curve on log-log axes has a name and a meaning beyond
``the exponent of the local power law.'' It is the \engterm{elasticity}
of the quantity, the fractional change in the output per fractional
change in the input, and it is the natural sensitivity measure whenever
both quantities are positive and span a wide range. Formally the
elasticity is $E = (\dd y / y)/(\dd x / x) = (x/y)\,\dd y/\dd x =
\dd(\ln y)/\dd(\ln x)$, which is exactly the slope on a plot of $\ln y$
against $\ln x$, so the log-log slope the engineer reads by eye is a
point elasticity even where the curve is not a global power law. For a
true power law $y = A x^{n}$ the elasticity is the constant $n$
everywhere, which is why a power law plots straight on log-log axes: a
constant elasticity is a straight log-log line. Where the elasticity
varies, the log-log curve bends, and the local slope reports the local
sensitivity: an elasticity of one is proportional response, an
elasticity above one is an amplifying regime where the output outpaces
the input, and an elasticity between zero and one is a diminishing
regime where the output lags. The measure recurs wherever a design
trades one wide-range quantity against another. The lift-to-drag scaling
of an aircraft, the throughput-to-core scaling of a parallel program,
the strength-to-mass scaling of a structural member, and the cost-to-
capacity scaling of a plant are all read as elasticities off log-log
plots, and an engineer who names the log-log slope as an elasticity
carries a single sensitivity number that is dimensionless, scale-free,
and directly comparable across quantities that share no units.
\end{mastery}

\subsection{Applied vignette: a radio link budget in decibels}

A radio link connects a transmitter to a receiver across a path that
attenuates the signal, and the engineer's design question is whether
enough power arrives to be demodulated. The whole calculation is a
sum of logarithmic terms, which is why radio engineers work it in
decibels and never leave the log domain until the last line. The tool
is the material of this chapter: an exponential and a power law read
on log axes, and a chain of gains that adds because logarithms turn
the product of ratios into a sum of decibels.

The received power in free space follows the inverse-square law. A
transmitter radiating isotropically spreads its power over a sphere
whose area grows as $r^2$, so the power crossing a fixed collecting
area falls as $r^{-2}$. Writing the received power as
$P_r = P_t \cdot G \cdot r^{-2} \cdot k$ for a lumped constant $k$ and
antenna gain $G$, the power law $P_r \propto r^{-2}$ is a straight line
of slope $-2$ on log-log axes, or equivalently, since decibels already
take the log of the power, a line of slope $-20\,\si{\deci\bel}$ per
decade on a plot of power in decibels against log range.
\Cref{fig:vol02:ch02:friis-linkbudget} draws that line together with
the horizontal floor set by the receiver's sensitivity, the smallest
power it can still recover. The two cross at the maximum usable range,
and reading that crossing off the plot answers the design question with
no further algebra.

Working the budget in decibels turns every physical effect into an
additive term. Start from the transmitted power in \si{\deci\bel}
relative to a milliwatt (dBm), add the transmit antenna gain in
decibels, subtract the free-space path loss in decibels, subtract the
cable and connector losses, add the receive antenna gain. The result
is the received power in dBm, compared directly against the receiver
sensitivity in dBm. Suppose a transmitter puts out $20\,\si{\deci\bel}$
relative to a milliwatt, the two antennas contribute $+6$ and
$+3\,\si{\deci\bel}$, the cables cost $-4\,\si{\deci\bel}$, and the
path loss at the design range is $-113\,\si{\deci\bel}$. The received
power is $20 + 6 + 3 - 4 - 113 = -88\,\si{\deci\bel}$ relative to a
milliwatt, computed by addition alone. Against a receiver sensitivity
of $-100\,\si{\deci\bel}$ relative to a milliwatt, the link closes with
a $12\,\si{\deci\bel}$ margin, meaning the arriving signal is about
sixteen times the minimum the receiver needs. The margin is the
engineer's headroom against rain, fading, and pointing error, and it
is read off the same additive ledger. Had the sum come out below the
sensitivity, the link would not close and the engineer would raise
transmit power, add antenna gain, or shorten the range, each a term
that slides the sloped line of
\Cref{fig:vol02:ch02:friis-linkbudget} up or bends the range at which
it meets the floor. The vignette shows the payoff of the log domain in
one line: a chain of multiplications in the linear world is a column of
additions in the decibel world, and the design closes or fails on the
sign of a single sum.

\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-friis-linkbudget.tex}

\subsection{Applied vignette: reading a titration curve}

An acid-base titration adds a measured base to an acid and tracks the
pH, and the resulting curve is a compact demonstration of a logarithmic
axis, a sigmoidal shape, and a high-gain region all in one plot. The pH
is $-\log_{10}[\mathrm{H}^{+}]$, the negative base-ten logarithm of the
hydrogen-ion concentration, so the vertical axis is a logarithmic
transform of the quantity that chemistry actually changes. A shift of
one pH unit is a tenfold change in concentration, and the entire
working range of aqueous chemistry, fourteen orders of magnitude in
$[\mathrm{H}^{+}]$, compresses onto the fourteen-unit pH scale exactly
as the decibel compressed the dynamic range of a signal.

\Cref{fig:vol02:ch02:titration-curve} plots pH against the volume of
base added for a weak acid titrated by a strong base, and the curve has
three regions the engineer reads by shape. The flat \engterm{buffer
region} at the start is where added base is absorbed with little change
in pH, because the acid and its conjugate base together resist the
change; the pH there equals the acid's $\mathrm{p}K_a$ at the
half-equivalence point, the volume at which half the acid has been
neutralised, which is how a titration measures a $\mathrm{p}K_a$
directly off the curve. The steep jump at the \engterm{equivalence
point} is where the last of the acid is consumed and a small further
addition swings the pH by several units; it is the point of maximum
slope, the high-gain region of the input-output map. The second flat
region past the equivalence point is where excess strong base sets the
pH on its own.

Each region is an engineering statement. A chemical engineer designing
a buffer for a process reads the target pH off the flat region and the
buffer's capacity off its width, since a wider flat region absorbs more
acid or base before the pH breaks. A control engineer running an
automated titration reads the steep region as the zone where the
measurement is most sensitive and the control loop hardest to keep
stable, because the same high slope that makes the endpoint easy to
detect also makes the pH swing wildly for a small overshoot in
reagent addition, the reason automated titrators slow their dosing
near the equivalence point. The sigmoidal shape is the same one the
logistic fit drew for a saturating population and the logit transform
straightened for an ion-channel activation curve; a titration is
another system whose input-output curve is an S, and the reading
disciplines of this chapter carry across unchanged. The logarithmic
axis is doing quiet work throughout: without it the buffer region and
the equivalence jump would sit at concentrations differing by orders
of magnitude and no single linear plot could show both.

\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-titration-curve.tex}

\subsection{Applied vignette: the photographic exposure triangle}

A camera sets the light reaching its sensor with three controls, and
the working system that ties them together is a base-2 logarithm, which
turns a three-way multiplicative balance into a three-way sum. The
material is this chapter's: an exponential relation read on a log axis,
and a chain of factors that adds because the logarithm turns the product
of ratios into a sum of steps. The example is worth carrying because it
shows the same log-domain reasoning the radio link budget used, applied
to a device most readers hold in a pocket.

The light on the sensor is a product of three factors. The
\engterm{aperture} sets how wide the lens opens, and its area scales as
the square of the diameter, so the light scales as the square of the
aperture ratio. The \engterm{shutter time} sets how long the sensor
collects, and the light scales linearly with it. The \engterm{sensor
gain} (the ISO setting) scales the recorded signal linearly. The total
exposure $H$ is the product $H \propto (\text{aperture area}) \cdot
(\text{shutter time}) \cdot (\text{gain})$, and a photographer who
worked this as a product of three numbers would be doing arithmetic in
the wrong domain. The system in use takes the base-2 logarithm of each
factor and counts in \engterm{stops}, where one stop is a factor of two
in light. The exposure value is $\mathrm{EV} = \log_{2}(N^{2}/t)$ for
aperture number $N$ and shutter time $t$, and a change of one stop in
any control is a change of one unit in $\mathrm{EV}$.
\Cref{fig:vol02:ch02:exposure-stops} plots the shutter-time control on a
base-2 log axis, where the halving sequence $1, 1/2, 1/4, \dots$ falls
on a straight line whose equal spacing marks equal ratios, the signature
of a logarithmic axis this chapter named for every log plot.

Working the balance in stops turns every adjustment into an addition.
A photographer who opens the aperture by one stop (letting in twice the
light) and wants to hold the total exposure fixed shortens the shutter
time by one stop (halving the collection time), so the two changes sum
to zero on the stop scale and the exposure is unchanged. This is the
\engterm{reciprocity} rule, and it is a statement that the exposure is a
sum of logarithmic terms exactly as the link budget was: aperture stops
plus shutter stops plus gain stops equals the exposure in stops, and any
redistribution that keeps the sum fixed keeps the light fixed. The
reason a photographer trades these three controls rather than picking
one is that each carries a side effect the sum does not see: a wider
aperture narrows the depth of field, a longer shutter blurs motion, a
higher gain raises noise. The engineer's move is to name the constraint
the sum must satisfy, then spend the remaining freedom on the side
effect that matters least for the shot, which is the same
constrained-tradeoff reasoning a link budget applies when it chooses
between raising transmit power and adding antenna gain. The logarithmic
control system is doing the quiet work throughout: without it the three
factors would multiply across several decades of light and no linear
control could hold them in a balance the photographer adjusts by
counting on the fingers of one hand.

\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-exposure-stops.tex}
...
The three functions carry small-argument behaviours the engineer uses
constantly to linearise a problem near a reference point. For small
$x$ in radians, $\sin x \approx x$, $\tan x \approx x$, and
$\cos x \approx 1 - x^2/2$, the leading terms of the Taylor series
Chapter 4 develops. The approximations are not vague; each carries a
known error. The next term in $\sin x$ is $-x^3/6$, so the fractional
error in $\sin x \approx x$ is about $x^2/6$, which the reader can turn
into a working tolerance rather than a rule of thumb.

\paragraph{Worked example: how small is a small angle.}
An engineer wants the largest angle at which $\sin\theta \approx
\theta$ holds to one percent. The fractional error of the
approximation is approximately $\theta^2/6$, so the tolerance
$\theta^2/6 \leq 0.01$ gives $\theta \leq \sqrt{0.06} \approx
0.245\,\si{\radian}$, about $14^{\circ}$. At $10^{\circ} =
0.1745\,\si{\radian}$ the error is $0.1745^2/6 \approx 0.0051$, half a
percent, which is where the chapter's earlier claim that
$\tan x \approx x$ is good to one percent below $10^{\circ}$ comes from.
The lesson is that ``small angle'' is a quantitative statement with a
bound attached, not a qualitative licence: a pendulum swung to
$30^{\circ}$ carries a $\sin\theta \approx \theta$ error near four
percent, enough to shift its period visibly, which is why the
simple-pendulum period formula is quoted with the small-amplitude
caveat that Volume II Chapter 7 makes exact through the elliptic
integral. Carrying the error bound alongside the approximation is the
same validity-domain discipline the chapter opened with, applied to a
truncated series rather than a fitted formula.
...
\subsection{Combining sinusoids of different frequency: beats}

Two sinusoids of the \emph{same} frequency add to one sinusoid of that
frequency. Two sinusoids of \emph{different} frequency do not; their
sum is no longer a single sinusoid, and the way it fails to be one is
the phenomenon of beats. The product-to-sum identity runs the algebra:
\[
  \cos(2\pi f_1 t) + \cos(2\pi f_2 t)
  = 2 \cos\!\Bigl(2\pi \tfrac{f_2 - f_1}{2} t\Bigr)
      \cos\!\Bigl(2\pi \tfrac{f_1 + f_2}{2} t\Bigr).
\]
The right side reads as a fast oscillation at the mean frequency
$(f_1 + f_2)/2$ multiplied by a slow one at half the difference
frequency $(f_2 - f_1)/2$. When the two frequencies are close the mean
is near either one and the difference is small, so a listener hears a
tone at roughly the common pitch whose loudness swells and fades. The
loudness follows the magnitude of the slow factor, which completes a
full cycle of rise and fall twice per period of the half-difference
term, so the audible beat frequency is the full difference $f_2 - f_1$,
not half of it.

\paragraph{Worked example: a piano tuner counting beats.}
A tuner sounds a reference fork at $440\,\si{\hertz}$ against a piano
string and hears the combined tone swell and fade three times per
second. The beat frequency is the difference $|f_2 - f_1| = 3\,\si
{\hertz}$, so the string is at either $443\,\si{\hertz}$ or
$437\,\si{\hertz}$; the beat count alone does not resolve the sign,
exactly as $\mathrm{atan2}$ was needed to resolve a quadrant earlier.
The tuner resolves the sign by tightening the string slightly: if the
beats speed up the string was already sharp (above the fork) and moved
further away, if they slow the string was flat and is approaching.
Tuning to zero beats drives $f_2 - f_1 \to 0$, at which point the sum
collapses back to the single-frequency case of the previous subsection.
\Cref{fig:vol02:ch02:beat-envelope} draws the pattern for two nearby
frequencies, the fast carrier inside its slow envelope. The same
beating governs the interference of two lasers in a heterodyne
detector, the roughness of two mistuned engine cylinders, and the
moir\'e pattern of two overlaid gratings; each is two nearby
frequencies read through the product identity.

\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-beat-envelope.tex}

\subsection{Two sinusoids on two axes: Lissajous figures}

Beats came from adding two sinusoids on the same axis. Putting one
sinusoid on the horizontal axis and another on the vertical axis instead
of adding them traces a parametric curve whose shape encodes the
frequency ratio and the phase between the two, and reading that shape is
a fast measurement the engineer once made on an oscilloscope and still
meets in any two-signal comparison. The curve is
$\bigl(x(t), y(t)\bigr) = \bigl(A\sin(\omega_{x} t),\,
B\sin(\omega_{y} t + \varphi)\bigr)$, and its name is the
\engterm{Lissajous figure}.

When the two frequencies are equal, $\omega_{x} = \omega_{y}$, the curve
closes into an ellipse whose shape reads the phase. At zero phase the
ellipse collapses to a straight diagonal line, because $x$ and $y$ rise
and fall together; at a quarter-cycle phase of $\pi/2$ with equal
amplitudes the ellipse opens into a circle, because $x$ leads $y$ by
exactly the amount that makes the pair trace a rotation; intermediate
phases give a tilted ellipse whose openness grows with the phase.
\Cref{fig:vol02:ch02:lissajous} draws the $\pi/4$ case in panel (a), a
tilted ellipse partway between the line and the circle. Reading the
phase off the tilt and openness of the ellipse is the two-signal
version of the single-signal phase reading the chapter has used
throughout.

When the two frequencies stand in a ratio of small integers, the curve
still closes, but into a multi-lobe pattern whose lobe counts read the
ratio directly. A ratio $\omega_{x} : \omega_{y} = m : n$ gives a figure
with $n$ lobes along the horizontal extent and $m$ along the vertical,
so counting the tangency points on two adjacent edges recovers the ratio
without any calculation. Panel (b) of \Cref{fig:vol02:ch02:lissajous}
shows a $3 : 2$ figure with three lobes across and two up. The reading
is exact: a Lissajous figure that closes into a stable, stationary
pattern certifies that the two frequencies are in a rational ratio, and
a pattern that drifts and never closes signals a small irrational offset
between them, which is the same swelling mismatch the piano tuner heard
as a beat, now seen as a slow rotation of the figure rather than heard
as a slow throb.

\paragraph{Worked example: reading a frequency ratio off a Lissajous
figure.}
An oscilloscope in $x$-$y$ mode displays a stable, closed Lissajous
figure with two lobes touching the top edge of the screen and three
lobes touching the right edge. The frequency ratio reads directly off
the tangency counts: the vertical lobe count sets the horizontal-drive
frequency and the horizontal lobe count sets the vertical-drive
frequency, so with two top tangencies and three right tangencies the
ratio is $\omega_{x} : \omega_{y} = 2 : 3$. If the vertical channel
carries a known reference at $\SI{600}{\hertz}$, the horizontal
unknown is $\omega_{x} = (2/3) \cdot \SI{600}{\hertz} =
\SI{400}{\hertz}$. Had the figure been slowly rotating rather than
stationary, the two frequencies would not be in an exact $2 : 3$ ratio;
the rotation rate would give the small offset, one full rotation of the
figure per cycle of the difference between the actual frequency and the
exact $2 : 3$ value, which is the beat frequency of the two-axis display
and the same difference-frequency reading the beat envelope carried on a
single axis. The Lissajous method turns a frequency comparison into a
count of lobes and a rotation rate, both read by eye, which is why it
survived as a laboratory technique long after it could have been
replaced by arithmetic.

\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-lissajous.tex}
...
\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-inverse-mirror.tex}
...
\paragraph{Worked example: inverting a function by Newton iteration.}
When a function has no closed-form inverse, the engineer recovers the
input from an output by solving $f(x) - y = 0$ numerically, and
Newton's method is the fast standard tool where the derivative is
available. To invert $y = x + \sin x$ at $y = 1$, define $g(x) =
x + \sin x - 1$ and iterate $x_{k+1} = x_k - g(x_k)/g'(x_k)$ with
$g'(x) = 1 + \cos x$. Starting from $x_0 = 0.5$: $g(0.5) = 0.5 +
0.479 - 1 = -0.021$, $g'(0.5) = 1 + 0.878 = 1.878$, so $x_1 = 0.5 -
(-0.021)/1.878 = 0.511$. The next step gives $g(0.511) \approx
-0.0002$ and $x_2 \approx 0.5110$, already correct to four decimals.
Newton's method roughly doubles the number of correct digits per step
near the root, which is why two or three iterations suffice once the
start is close. The precondition is the one the boxes named:
$f(x) = x + \sin x$ has derivative $1 + \cos x \geq 0$, vanishing only
at isolated points, so it is monotone increasing and its inverse is
single-valued, which is what lets a single Newton sequence converge to
the one right answer rather than wandering between branches. A function
whose derivative changed sign would need the iteration confined to a
monotone segment, the same restriction that gave the square root and
the arcsine their principal branches.

\begin{mastery}
Monotonicity and convexity are the two structural properties an
engineer most often needs a function to have, because each carries a
guarantee the unstructured case cannot offer. A \engterm{monotone}
function (one that keeps a single direction across an interval)
guarantees a single-valued inverse, which is the property the previous
boxes turned into a sensor-design rule. A \engterm{convex} function
(one whose graph curves upward, so every chord lies on or above the
graph and every tangent on or below it) guarantees something stronger
about its minimum.
\Cref{fig:vol02:ch02:convex-classes} draws the chord and tangent
properties and the gap that names Jensen's inequality, that the
average of a convex function exceeds the function of the average. The
working payoff is the minimisation guarantee: a convex function on an
interval has at most one local minimum, and that minimum is also the
global minimum, so a search that finds a point where the slope
vanishes has found the answer and need not worry that a deeper
minimum hides elsewhere. A non-convex objective offers no such
guarantee, and an optimiser may settle into a local minimum far worse
than the global one. This single difference is why an engineer setting
up an optimisation works to cast the objective in convex form when the
physics allows, and the formal development of that craft is the
subject of convex optimisation
\cite{text:boyd-vandenberghe2004}. The two classes compose
predictably: a monotone function of a monotone function is monotone,
and a convex increasing function of a convex function is convex, so
the engineer who tracks these labels through a composition chain knows
which guarantees survive to the output. Carrying the convexity label on
a function is carrying the promise that its minimum is findable, the
same way carrying the monotonicity label is carrying the promise that
its inverse is single-valued.
\end{mastery}

\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-convex-classes.tex}
...
and the source. Only then does the engineer read the data. The
discipline of choosing axes and transforms that let the data show its
own structure, rather than axes that flatter a conclusion, is the
subject Cleveland treats as the craft of visualising data
\cite{v2c02-text:cleveland1993}, and the reading discipline here is its
mirror image: the same choices the honest maker of a plot weighs are
the ones the careful reader reverse-engineers.
...
\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-log-axis-reading.tex}
...
\paragraph{Worked example: the trap of reading area under a log axis.}
A histogram drawn with a logarithmic horizontal axis carries a hazard
the linear case does not, because the eye reads a bar's importance from
its area and a log axis distorts the widths. Suppose particle counts are
binned into equal ratio intervals, one decade per bin, and plotted with
the bin edges at $1, 10, 100, 1000$ on a log axis so the three bars
occupy equal widths on the page. The equal page widths hide unequal true
widths: the first bin spans a range of $9$ units, the second $90$, the
third $900$. A bar in the third bin drawn to the same height as one in
the first therefore packs the same count into a range a hundred times as
wide, so its true \engterm{density}, count per unit of the underlying
quantity, is a hundredth of what the equal-height bar suggests. Reading
the three bars as equally important because they are equally tall and
equally wide on the page is the log-axis version of reading a suppressed
origin: the frame has rescaled the quantity the eye is integrating. The
correct plot for a density on a log axis divides each bin's count by the
true width of its interval before setting the bar height, so that the
bar area, height times true width, recovers the count and the eye reads
importance honestly. The general rule is that area under a curve or
across a set of bars is only meaningful on a linear axis; on a log axis
the area on the page is not the integral of the quantity, and any
reading that depends on area, a total, a fraction of the whole, a
center of mass, must be done on the linear values or with an explicit
density correction, never off the log-axis picture directly.
...
Slope carries units, and the units are the first check on whether a
slope has been read correctly. On a plot of distance in metres against
time in seconds the slope is a velocity in metres per second; on a plot
of voltage against temperature the slope is a sensitivity in volts per
kelvin, the very number a sensor datasheet quotes. A slope read off a
plot and reported without its units is as incomplete as a bare number
without a unit anywhere else in engineering, and a slope whose units do
not match the quantity it is supposed to represent is a signal that an
axis was misread. On transformed axes the slope's meaning transforms
with them, a fact the chapter has used repeatedly and states here as a
rule: the slope on linear-linear axes is a rate in the ratio of the
axis units, the slope on semilog axes is a rate constant scaled by
$\log_{10} e$, and the slope on log-log axes is a dimensionless
exponent. Reading a semilog slope as if it were a linear rate, or a
log-log slope as if it carried units, is the same category error as
mixing units at an interface, and it produces answers wrong by the
conversion factor between the two readings.

A second reading the slope supports is sensitivity analysis: the slope
at an operating point is the factor by which an error in the input
propagates to an error in the output. A steep local slope amplifies
input noise, so a measurement taken where the characteristic is steep
carries more output uncertainty per unit of input uncertainty than one
taken on a flat stretch, which is why an instrument is designed to
operate where its characteristic has the slope that best trades
sensitivity against noise. The surveyor's tower-height example earlier
in the chapter was exactly this calculation: the derivative
$\dd h/\dd\theta$ was the slope that turned an angle error into a height
error, and reading it off the geometry set the precision of the result.

Slope is the first reading a curve gives; the way the slope itself
changes is the second, and it carries information the slope alone hides.
A curve whose slope grows as the input grows is \engterm{concave up},
its graph bending like the inside of a bowl, and every chord between two
of its points lies above the curve. A curve whose slope falls as the
input grows is \engterm{concave down}, bending like a dome, and every
chord lies below. The engineer reads concavity for what it says about
the mechanism: a concave-up cost curve is a system whose marginal cost
rises with scale, a diseconomy, while a concave-down yield curve is a
system of diminishing returns, each added unit of input buying less
output than the one before. A point where the concavity switches is an
\engterm{inflection}, and it marks where a mechanism changes character:
the steepest point of a saturating S-curve, the transition from
accelerating to decelerating growth, the onset of saturation in a
sensor.

\paragraph{Worked example: reading concavity to diagnose a regime.}
A logged reactor conversion rises with residence time along a curve that
is steep at first and flattens toward a ceiling, and an engineer reads
its two shape features to name the regime without fitting anything. The
slope is positive throughout, so conversion increases with residence
time everywhere, which the raw numbers already show. The concavity is
the diagnostic: the curve is concave down across the whole logged range,
its slope falling as residence time grows, which says the process is in
diminishing returns and each added second of residence buys less
conversion than the second before. The absence of any inflection tells
the engineer the reactor is not in an autocatalytic or self-accelerating
regime, where an early concave-up stretch would precede the concave-down
saturation and leave an inflection between them. The design reading
follows at once: because the returns are diminishing everywhere, the
economic operating point sits where the marginal conversion per unit of
residence time has fallen to match the marginal cost of the extra
residence, which is the point where the curve's slope crosses the
cost-per-second line, and pushing residence time past that point spends
reactor volume for conversion the curve has already told the engineer is
getting cheaper to forgo. Two shape readings, slope sign and concavity
sign, and the regime and the operating point are fixed before any model
is chosen.
...
The rate at which a curve approaches an asymptote is itself
engineering information, because it sets how far into the tail the
approximation by the asymptote stays honest. A horizontal asymptote
approached exponentially, as in a first-order thermal or electrical
transient, is within a fixed fraction of its final value after a fixed
number of time constants, so the engineer can name a settling time: the
$99\,\%$ point at about five time constants, the $99.9\,\%$ point at
about seven. A horizontal asymptote approached as a power law, as in a
$1/r^2$ field far from its source, has no such clean settling number
because the approach never becomes exponentially fast; the tail must be
carried explicitly rather than truncated at a fixed multiple of a
scale. Distinguishing the two approaches is the difference between a
transient the engineer can declare finished and a tail that must be
integrated to infinity, and reading which one a curve shows is the same
move as reading whether the data is exponential or a power law, made at
the far end of the range rather than the near one. A vertical asymptote
carries a matched reading: the sign of the function on each side of the
pole, positive-to-positive for an even-order pole and
positive-to-negative for an odd-order one, is the local behaviour that
tells the engineer whether the modelled quantity diverges the same way
from both sides, as a squared resonance denominator does, or flips
sign across the singularity, as a simple pole in a transfer function
does.
...
\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-loadcell-characterisation.tex}
...
\section{Case study: identifying a transfer characteristic from
frequency-response data}\pathtag{mastery}

The load-cell case took a static input-output curve from data to a
usable model. The complementary task takes a \emph{dynamic} curve, the
way a device responds across frequency, from measured data to a
transfer function the engineer can compute with. We work it end to end
on a galvanometer-style moving-coil instrument, the rotating-pointer
movement at the heart of an analogue panel meter, a chart recorder, and
the read head positioner of a disk drive. The device is a mass on a
spring with damping, and its response across frequency is the canonical
second-order curve every engineer meets. The discipline the case shows,
reading a model off the asymptotes and the resonant peak rather than
fitting it blind, transfers to any frequency-domain characterisation.

\subsection{The device and the frequency-sweep measurement}

A moving-coil movement is a coil suspended in a magnetic field on a
torsion spring. A current through the coil produces a torque, the coil
rotates against the spring, and a pointer reads the deflection. The
coil has rotational inertia, the spring provides a restoring torque,
and eddy currents and air provide damping, so the movement is a
second-order mechanical system: inertia, stiffness, and damping, the
same three ingredients as a mass on a spring. The engineer wants to
know how the pointer deflection follows a current that varies in time,
which is the device's frequency response.

The characterisation experiment drives the coil with a sinusoidal
current of fixed amplitude and swept frequency, and at each frequency
records the amplitude of the steady-state pointer oscillation. The raw
data is a column of $(f, |H|)$ pairs: drive frequency against
response amplitude relative to the low-frequency deflection. The frame
check of Section~2.6 applies to the engineer's own sweep before anyone
else sees it. The input is frequency, in hertz, over the swept domain.
The output is response magnitude, a dimensionless ratio because it is
normalised to the low-frequency response, which divides out the static
sensitivity exactly as the load cell normalised to excitation voltage.
The measurement is amplitude only; the phase response is a second
sweep, recorded separately, that the magnitude reading does not need
but that confirms the model.

\subsection{Reading the model off the magnitude curve}

\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-second-order-bode.tex}

\Cref{fig:vol02:ch02:second-order-bode} plots the magnitude data in
decibels against log frequency, and two features fix the two
parameters of a second-order model without any nonlinear fit. The
low-frequency data sits flat at $0\,\si{\deci\bel}$, the passband where
the pointer follows the current faithfully. The high-frequency data
falls along a straight line of slope $-40\,\si{\deci\bel}$ per decade,
the signature of a two-pole roll-off, where the inertia of the coil
cannot keep up with the drive. The two asymptotes, the flat passband
and the $-40\,\si{\deci\bel}$ line, intersect at the
\engterm{natural frequency} $\omega_{n}$, and reading that intersection
off the plot is the first parameter recovered. The second parameter is
the \engterm{damping ratio} $\zeta$, read off the height of the
resonant peak: a lightly damped movement shows a peak above the
passband near $\omega_{n}$, and for small $\zeta$ the peak height is
$M_{p} \approx 1/(2\zeta)$, so a measured peak of $9\,\si{\deci\bel}$,
a factor of about $2.8$ in amplitude, gives $\zeta \approx 1/(2 \cdot
2.8) \approx 0.18$. Two readings off the curve, the corner frequency
and the peak height, and the model is identified.

The identified transfer function is the standard second-order form
\[
  H(s) = \frac{\omega_{n}^{2}}{s^{2} + 2\zeta\omega_{n} s
    + \omega_{n}^{2}},
\]
a rational function whose denominator is a quadratic in the complex
frequency $s$. Everything the chapter said about rational functions
now pays off. The poles are the roots of the denominator,
$s = -\zeta\omega_{n} \pm \mathrm{j}\omega_{n}\sqrt{1 - \zeta^{2}}$, a
conjugate pair in the left half-plane for $0 < \zeta < 1$. The pair is
exactly the kind \Cref{fig:vol02:ch02:pole-zero-map} drew: the real
part $-\zeta\omega_{n}$ sets the decay rate of the movement's ringing,
and the imaginary part $\omega_{d} = \omega_{n}\sqrt{1 - \zeta^{2}}$,
the \engterm{damped natural frequency}, sets the frequency at which it
rings. The magnitude curve and the pole-zero map are two views of the
one model, and the engineer reads between them: the peak in the
frequency response and the closeness of the poles to the imaginary
axis are the same physical fact, light damping, seen in two diagrams.

\subsection{Using the model: the step response and the design fix}

The transfer function predicts how the pointer answers a sudden change
in current, the step response the meter user actually sees. A lightly
damped movement with $\zeta = 0.18$ overshoots its final reading by a
large margin and rings several times before settling, because the
conjugate poles sit close to the imaginary axis and the ringing decays
slowly. The overshoot fraction follows from the damping ratio alone,
$e^{-\pi\zeta/\sqrt{1 - \zeta^{2}}}$, which for $\zeta = 0.18$ is about
$56\,\%$: the pointer swings more than half again past the true value
before coming back. A panel meter that overshoots by half its reading
and rings is unusable, so the characterisation has not only identified
the device, it has diagnosed a defect.

The design fix reads off the model directly. Settling without
objectionable overshoot wants a damping ratio near
$\zeta \approx 0.6$ to $0.7$, the standard target that trades a small
overshoot for a fast settle. Raising $\zeta$ means adding damping,
which for a moving-coil movement means increasing the eddy-current
loss, often by winding the coil on an aluminium former that acts as a
shorted turn. The model says how much: moving from $\zeta = 0.18$ to
$\zeta = 0.65$ pulls the poles left and down, away from the imaginary
axis, cutting the overshoot from $56\,\%$ to about $6\,\%$ and turning
a ringing pointer into one that settles cleanly. The same model, two
parameters read off a magnitude curve, has carried the engineer from
raw frequency-sweep data through an identified transfer function to a
named defect and a quantified design change. One device, two sweeps,
the rational-function and graph-reading vocabulary of the chapter, and
a model that does engineering work.
...
\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-extrapolation-trap.tex}
...
\input{volumes/02-form/ch02-functions-and-graphs/figures/fig-challenger-chart.tex}
...
\begin{mastery}
The chapter has treated a function as a rule for turning one number
into another, which is the working view for most engineering. A deeper
view, the one the later volumes lean on, treats a whole function as a
single object: a point in a space whose other points are also
functions. The shift is the same one that turned a list of three
numbers into a single vector in Chapter 1. Once functions are points
in a space, the operations of linear algebra carry over. A set of
simple functions can serve as a \engterm{basis}, and a complicated
function is written as a weighted sum of those basis functions the way
a vector is written as a weighted sum of coordinate directions. The
Fourier series of the even-odd box is exactly this: the sinusoids of
every harmonic frequency are a basis, and the Fourier coefficients are
the coordinates of a periodic function in that basis. The Taylor series
is the same move with the monomials $1, x, x^{2}, \ldots$ as the basis.
A spline is the same move with a basis of localised piecewise-cubic
bumps. The payoff of the function-as-object view is that approximating
a function becomes choosing how many basis terms to keep, and the error
of the approximation becomes the size of the discarded coordinates, a
quantity the engineer can bound and control. The space of functions
with a notion of length and angle is the subject of functional
analysis, and the inner product that measures the angle between two
functions is the integral of their product, which is why a Fourier
coefficient is computed by integrating a signal against a single
harmonic. The working engineer does not need the full theory to use
its first consequence: a function is a thing that can be built out of
simpler functions, and the art is choosing the basis whose first few
terms already carry most of what the function does.
\end{mastery}
