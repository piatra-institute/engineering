...
chapter's Failure section (section 16.12.2) returns to this constant
...
\begin{example}
\textbf{The quadratic formula rewritten.} The catalogue of cancellation
patterns has a member every engineer has typed without a second thought: the
quadratic formula. To solve $a x^{2} + b x + c = 0$ the textbook writes
$x_{\pm} = (-b \pm \sqrt{b^{2} - 4 a c})/(2 a)$. Take $a = 1$, $b = 10^{8}$,
$c = 1$, whose roots are $x_{+} \approx -10^{8}$ and $x_{-} \approx -10^{-8}$.
The discriminant is $\sqrt{10^{16} - 4} \approx 10^{8} - 2 \times 10^{-8}$,
which rounds in binary64 to $10^{8}$ because the correction $2 \times 10^{-8}$
sits below the spacing of $10^{8}$ (which is $\approx 2^{27 - 52} \times 2^{53}
\approx 1.5 \times 10^{-8} \cdot \ldots$, of order $10^{-8}$). The large root
$x_{+} = (-10^{8} - 10^{8})/2 = -10^{8}$ is fine, an addition of like signs.
The small root $x_{-} = (-10^{8} + 10^{8})/2$ subtracts two numbers that agree
to all their stored digits and returns $0$, which has no correct digits at all
against the true $-10^{-8}$. The problem is well conditioned; the small root
depends smoothly on the coefficients. The algorithm is the buyer, and the
rewrite is the same conjugate trick as the $\sqrt{x+1} - \sqrt{x}$ example.
Multiply numerator and denominator of the small root by the conjugate to get
$x_{-} = 2c/(-b - \sqrt{b^{2} - 4 a c})$, an addition of like-signed numbers
in the denominator, which returns $-10^{-8}$ to full precision. The working
rule is to compute the root that adds by the standard formula, and the other
root from the product identity $x_{+} x_{-} = c/a$, so that each root is
obtained by an addition rather than a cancelling subtraction.
Figure~\ref{fig:vol02ch16:quadratic-cancellation} tracks the digit loss of the
naive formula as the roots separate.
\end{example}

\input{volumes/02-form/ch16-numerical-methods/figures/fig-quadratic-cancellation}

\begin{example}
\textbf{The area of a needle triangle.} Heron's formula computes the area of
a triangle with sides $a \geq b \geq c$ from the semiperimeter $s = (a + b +
c)/2$ as $A = \sqrt{s(s-a)(s-b)(s-c)}$. For a \engterm{needle} triangle, one
that is long and thin so that $a \approx b + c$, the factor $s - a = (-a + b +
c)/2$ subtracts nearly equal numbers and loses digits, and the loss carries
into the area under the square root. Take $a = 100000$, $b = c = 50000.00001$,
a valid triangle by the barest margin. The true $s - a = 10^{-5}$, but in
binary64 the sum $b + c = 100000.00002$ and the subtraction $-a + b + c$
returns a value corrupted in its low digits, so the computed $s - a$ carries
only a few correct figures and the area inherits the loss. Kahan's stable
rearrangement orders the sides $a \geq b \geq c$ and evaluates $A =
\tfrac{1}{4}\sqrt{(a + (b + c))\,(c - (a - b))\,(c + (a - b))\,(a + (b - c))}$
with the parenthesisation exactly as written, which arranges every subtraction
to be between quantities that are not nearly equal and recovers the full
precision. The diagnostic is again the section's: the triangle-area problem is
well conditioned away from the degenerate limit, so a wrong answer indicts the
algorithm, and the cure is the reparenthesisation, not higher precision.
\end{example}

\begin{example}
\textbf{One-pass variance and why textbooks warn against it.} The sample
variance of $x_{1}, \ldots, x_{n}$ is $s^{2} = \tfrac{1}{n-1}\sum(x_{i} -
\bar{x})^{2}$, which needs the mean $\bar{x}$ before the sum, hence two passes
over the data. The tempting one-pass identity $s^{2} = \tfrac{1}{n-1}\big(\sum
x_{i}^{2} - (\sum x_{i})^{2}/n\big)$ accumulates $\sum x_{i}^{2}$ and $\sum
x_{i}$ in a single sweep, and it is a numerical trap. Take three values
$x = (10^{9}, 10^{9} + 1, 10^{9} + 2)$, whose true variance is exactly $1$.
The one-pass form computes $\sum x_{i}^{2} \approx 3 \times 10^{18}$ and
$(\sum x_{i})^{2}/n \approx 3 \times 10^{18}$, two eighteen-digit numbers
whose difference is $2$; binary64 carries about sixteen significant digits, so
the eighteen-digit operands have already lost the units place before the
subtraction, and the computed difference is meaningless, often negative, which
would return an imaginary standard deviation. The two-pass form subtracts the
mean $\bar{x} = 10^{9} + 1$ first, forming residuals $(-1, 0, 1)$ of order
unity whose squares sum to $2$ with no cancellation, and returns the exact
variance. Welford's online algorithm updates a running mean and a running sum
of squared residuals in a single pass with the two-pass accuracy, and is the
form a production statistics routine uses \cite{text:v2ch16-mccullough-vinod-1999}.
The lesson generalises past variance: an algebraic identity that is exact on
the page can be numerically ruinous when it forms a large intermediate whose
cancellation destroys the small quantity of interest.
\end{example}
...
\subsection{Higher order, and the step from scalar to system}

Newton doubles the digit count per step; a method that uses one more
derivative can triple it. \engterm{Halley's method} folds the second
derivative into the iteration,

\[
x_{k+1} = x_{k} - \frac{2 f(x_{k}) f'(x_{k})}
{2 f'(x_{k})^{2} - f(x_{k}) f''(x_{k})},
\]

and converges cubically at a simple root: the error obeys $\abs{e_{k+1}}
\approx C \abs{e_{k}}^{3}$, so the number of correct digits roughly triples
each step rather than doubling. The method is the fixed-point iteration
whose map has both a vanishing first \emph{and} a vanishing second
derivative at the root, which is the structural reason its order climbs one
past Newton's. The cost is one extra derivative per step, and the working
question is whether the third-order rate saves enough iterations to repay the
$f''$ evaluation; when $f''$ is available in closed form and evaluations
dominate the runtime, Halley reaches machine precision in three or four steps
where Newton takes five or six.

\begin{example}
\textbf{Halley against Newton on a cube root.} Solve $f(x) = x^{3} - 2 = 0$
for the real root $x^{*} = 2^{1/3} = 1.2599210498948732$, from $x_{0} = 1$.
Newton uses $f'(x) = 3x^{2}$: $x_{1} = 1 - (-1)/3 = 1.3333333$, error $7.34
\times 10^{-2}$; $x_{2} = 1.2638889$, error $3.97 \times 10^{-3}$; $x_{3} =
1.2599335$, error $1.2 \times 10^{-5}$; $x_{4}$ near $10^{-10}$, $x_{5}$ at the binary64 floor, five
steps to full precision. Halley adds $f''(x) = 6x$: the first step is $x_{1}
= x_{0} - 2 f f' / (2 f'^{2} - f f'')$ with $f(1) = -1$, $f'(1) = 3$, $f''(1)
= 6$, giving $x_{1} = 1 - 2(-1)(3)/(2 \cdot 9 - (-1)(6)) = 1 + 6/24 = 1.25$,
error $9.9 \times 10^{-3}$; $x_{2} = 1.2599211$, error $1.4 \times 10^{-8}$;
$x_{3}$ at the floor, two steps to full precision. The digit count runs $2,
8, 16$ for Halley against $1, 2, 5, 16$ for Newton, the cubic-versus-quadratic
signature that Figure~\ref{fig:vol02ch16:halley-newton} plots. Halley reached
machine precision in one fewer step at the price of one $f''$ evaluation per
step; on this cheap cubic the trade is a wash, and Newton's simplicity wins,
which is why Halley is reserved for the case where $f''$ is cheap and every
saved evaluation of an expensive $f$ pays for itself.
\end{example}

\input{volumes/02-form/ch16-numerical-methods/figures/fig-halley-newton}

\begin{example}
\textbf{Steffensen recovers Newton's rate without a derivative, by the
numbers.} The Aitken-acceleration mastery box promised that folding the
$\Delta^{2}$ extrapolation into a fixed-point iteration recovers quadratic
convergence with no derivative. Carry it on the linearly convergent map
$g(x) = (x + 2)^{1/3}$ for the root of $x^{3} - x - 2$ near $x^{*} =
1.5213797$. From $x_{0} = 1.5$, one Steffensen step evaluates $g$ twice,
$a = g(x_{0}) = (3.5)^{1/3} = 1.5182945$ and $b = g(a) = (3.5182945)^{1/3} =
1.5210313$, then extrapolates $x_{1} = x_{0} - (a - x_{0})^{2}/(b - 2a +
x_{0})$. The numerator is $(a - x_{0})^{2} = (1.8294 \times 10^{-2})^{2} =
3.347 \times 10^{-4}$ and the denominator is $b - 2a + x_{0} = 1.5210313 -
3.0365890 + 1.5 = -1.5254 \times 10^{-2}$, so $x_{1} = 1.5 - (3.347 \times
10^{-4})/(-1.5254 \times 10^{-2}) = 1.5 + 2.1942 \times 10^{-2} = 1.5219420$,
error $5.6 \times 10^{-4}$ against the plain iteration's $3.1 \times 10^{-3}$
after two raw applications. The next Steffensen step drops the error to $1.9
\times 10^{-7}$ and the one after to below $10^{-14}$: the error squares each
accelerated step, the quadratic rate the raw linear map could never reach, and
no derivative of $f$ was formed. The three evaluations of $g$ per accelerated
step (two here, plus the restart) are the price, and the guard against the
denominator $b - 2a + x_{0}$ vanishing near convergence is the one piece of
defensive code the method demands.
\end{example}

\subsection{Newton for systems}

The root-finding of the previous subsections was scalar, and most
engineering root problems are not: an equilibrium of a reaction network, a
power-flow balance, a trajectory constraint, each is a system $\vect{F}
(\vect{x}) = \vect{0}$ with $\vect{F}: \R^{n} \to \R^{n}$. \engterm{Vector
Newton} linearises the whole map at the current iterate. Writing the
\engterm{Jacobian} $\mat{J}(\vect{x})$ with entries $J_{ij} = \partial F_{i}
/ \partial x_{j}$, the step solves the linear system

\[
\mat{J}(\vect{x}_{k})\, \Delta\vect{x}_{k} = -\vect{F}(\vect{x}_{k}),
\qquad \vect{x}_{k+1} = \vect{x}_{k} + \Delta\vect{x}_{k},
\]

which replaces the scalar division by $f'$ with a linear solve against
$\mat{J}$. The convergence is quadratic near a root where $\mat{J}$ is
nonsingular, exactly as in the scalar case, with the conditioning of
$\mat{J}$ now setting how many digits the linear solve inside each step can
deliver. The method inherits the scalar method's fragility: a poor start can
diverge or cycle, a singular Jacobian stalls it, and production code
safeguards the step with a line search or a trust region that shortens
$\Delta\vect{x}_{k}$ when the full step does not reduce $\norm{\vect{F}}$.

\begin{example}
\textbf{A $2\times2$ system solved by vector Newton.} Find the intersection in
the first quadrant of the circle $x^{2} + y^{2} = 16$ and the parabola $y =
x^{2}/3$, that is the root of

\[
\vect{F}(x, y) = \begin{pmatrix} x^{2} + y^{2} - 16 \\ x^{2} - 3 y
\end{pmatrix}, \qquad
\mat{J}(x, y) = \begin{pmatrix} 2x & 2y \\ 2x & -3 \end{pmatrix}.
\]

From $\vect{x}_{0} = (1.5, 0.8)^{\transpose}$: $\vect{F}_{0} = (1.5^{2} +
0.8^{2} - 16,\, 1.5^{2} - 3(0.8))^{\transpose} = (-13.11, -0.15)^{\transpose}$
and $\mat{J}_{0} = \begin{psmallmatrix} 3 & 1.6 \\ 3 & -3 \end{psmallmatrix}$,
whose determinant is $-9 - 4.8 = -13.8$. Solving $\mat{J}_{0}\Delta\vect{x} =
-\vect{F}_{0} = (13.11, 0.15)^{\transpose}$ gives $\Delta\vect{x} =
\tfrac{1}{-13.8}\begin{psmallmatrix} -3 & -1.6 \\ -3 & 3 \end{psmallmatrix}
(13.11, 0.15)^{\transpose} = (2.867, 2.817)^{\transpose}$, so $\vect{x}_{1} =
(4.367, 3.617)^{\transpose}$, a large first step from the coarse start. The
second step re-evaluates and returns $\vect{x}_{2} = (3.13, 2.63)^{\transpose}$,
the third $\vect{x}_{3} = (2.73, 2.47)^{\transpose}$, and the iteration settles
to $\vect{x}^{*} = (2.708, 2.445)^{\transpose}$ with the error squaring once
the estimate is close, the quadratic rate the scalar method also enjoys.
Figure~\ref{fig:vol02ch16:vector-newton} draws the two curves and the
approach. The example exposes the one structural change from the scalar
method: every step is a linear solve against the Jacobian, so vector Newton is
the place where the root-finding of this section and the linear-algebra of the
next meet, and a system with an ill-conditioned Jacobian loses digits in the
step for exactly the reasons section 16.2 names.
\end{example}

\input{volumes/02-form/ch16-numerical-methods/figures/fig-vector-newton}

\begin{mastery}
\textbf{Newton fractals and the global-convergence question.} The
convergence theorems of this section are all \emph{local}: they promise
quadratic convergence once the iterate is close enough to a simple root, and
say nothing about which root a given start reaches, or whether it reaches one
at all. The global picture is far wilder than the local theory suggests.
Colour each starting point of the complex plane by which root of $z^{3} - 1 =
0$ the complex Newton iteration $z_{k+1} = z_{k} - (z_{k}^{3} - 1)/(3
z_{k}^{2})$ converges to, and the resulting map is a fractal: the three
basins of attraction interlock along a boundary that is self-similar at every
scale, so that arbitrarily close to a point that converges to one root sit
points that converge to each of the others. The boundary is the \engterm{Julia
set} of the Newton iteration, and its infinite intricacy means no finite
tolerance on the initial guess can guarantee which root a Newton solve will
find near that boundary. The engineering reading is not that Newton is
unreliable but that its reliability is a statement about a neighbourhood the
engineer must establish, not assume: a Newton solve reported without evidence
that the start lay in the target root's basin is a solve whose outcome the
fractal boundary can overturn. The safeguards of this section, the bracket
fallback in one dimension and the trust region in many, exist precisely to tame
the global behaviour the local theorems ignore, and the homotopy or
continuation methods of later volumes trace a path from an easy problem whose
basin is known to the hard one, so that the start is always inside the basin
by construction. The fractal is the visible form of the gap between local
convergence, which the mathematics guarantees, and global convergence, which
the engineer must engineer.
\end{mastery}
...
approximation of lower degree filters it. Interpolation fits the
points exactly, the Runge phenomenon is how that fit fails, and splines
are the piecewise remedy.
...
\subsection{The barycentric form}

The barycentric rewrite factors the shared part out of every cardinal
polynomial. Define the node polynomial $\ell(x) = \prod_{i=0}^{n}(x - x_{i})$
and the \engterm{barycentric weights}

\[
w_{j} = \frac{1}{\prod_{i \neq j}(x_{j} - x_{i})},
\]

each of which depends only on the nodes, not on the data. The cardinal
polynomial is then $\ell_{j}(x) = \ell(x)\, w_{j}/(x - x_{j})$, and after the
common $\ell(x)$ cancels between the numerator and a rewritten constant
denominator, the interpolant takes the \engterm{second (true) barycentric
form}

\[
p(x) = \frac{\displaystyle\sum_{j=0}^{n} \frac{w_{j}}{x - x_{j}}\, y_{j}}
{\displaystyle\sum_{j=0}^{n} \frac{w_{j}}{x - x_{j}}},
\qquad x \neq x_{j},
\]

with $p(x_{j}) = y_{j}$ taken as the limit at a node. Two properties earn the
form its place in every serious interpolation library. Evaluating $p$ at a new
point costs $O(n)$ once the weights are known, against the $O(n^{2})$ of the
raw Lagrange sum, and adding a data point updates the weights in $O(n)$ rather
than forcing a full recomputation, so the barycentric form has the Newton
form's incremental advantage without its ordering sensitivity. For the
Chebyshev nodes the weights have the closed form $w_{j} = (-1)^{j}\delta_{j}$
with $\delta_{j} = \tfrac{1}{2}$ at the two endpoints and $\delta_{j} = 1$
inside, so no products are formed at all and the evaluation is both fast and
backward stable for any node distribution that does not cluster
pathologically.

\begin{example}
\textbf{Barycentric evaluation of the three-point $\ln$ interpolant.} Return
to the nodes $x_{0} = 1, x_{1} = 2, x_{2} = 4$ with values $y = (0, 0.6931,
1.3863)$ from the divided-difference example below. The barycentric weights
are $w_{0} = 1/((1-2)(1-4)) = 1/3$, $w_{1} = 1/((2-1)(2-4)) = -1/2$, and
$w_{2} = 1/((4-1)(4-2)) = 1/6$. Evaluate at $x = 3$. The reciprocal distances
are $1/(3-1) = 0.5$, $1/(3-2) = 1$, $1/(3-4) = -1$, so the numerator is
$\tfrac{1}{3}(0.5)(0) + (-\tfrac{1}{2})(1)(0.6931) + \tfrac{1}{6}(-1)(1.3863)
= -0.34655 - 0.23105 = -0.57760$ and the denominator is $\tfrac{1}{3}(0.5) +
(-\tfrac{1}{2})(1) + \tfrac{1}{6}(-1) = 0.16667 - 0.5 - 0.16667 = -0.5$. The
quotient is $p(3) = (-0.57760)/(-0.5) = 1.1552$, the same value the Newton and
Lagrange forms returned to rounding, now from a computation whose cost grows
linearly in the node count and whose only subtractions are the node
differences fixed once at setup. The example shows the machinery a call to a
production interpolant runs: precompute the weights, then evaluate each query
point as one weighted-reciprocal sum over one weighted-reciprocal sum.
\end{example}

\begin{example}
\textbf{Clenshaw's recurrence for a Chebyshev series.} A smooth function is
often stored not as node values but as coefficients of a Chebyshev series
$p(x) = \sum_{k=0}^{n} a_{k} T_{k}(x)$, the natural representation once the
Runge-safe Chebyshev nodes are in hand. Evaluating that series by forming
each $T_{k}(x)$ and summing is both wasteful and numerically poor near the
endpoints, where the high-degree $T_{k}$ swing between $\pm 1$ and their
weighted sum cancels. \engterm{Clenshaw's recurrence} evaluates the series by
a single backward sweep that exploits the three-term recurrence $T_{k+1}(x) =
2 x\, T_{k}(x) - T_{k-1}(x)$. Set $b_{n+1} = b_{n+2} = 0$ and run downward,
$b_{k} = a_{k} + 2 x\, b_{k+1} - b_{k+2}$ for $k = n, n-1, \ldots, 1$; the
value is $p(x) = a_{0} + x\, b_{1} - b_{2}$. Take the four-term series with
$a = (1, 0.5, 0.25, 0.125)$ at $x = 0.6$, using $T_{0} = 1$, $T_{1} = x$,
$T_{2} = 2x^{2} - 1 = -0.28$, $T_{3} = 4x^{3} - 3x = -0.936$. The direct sum
is $1 + 0.5(0.6) + 0.25(-0.28) + 0.125(-0.936) = 1 + 0.3 - 0.07 - 0.117 =
1.113$. Clenshaw runs $b_{3} = a_{3} = 0.125$; $b_{2} = a_{2} + 1.2 b_{3} -
0 = 0.25 + 0.15 = 0.4$; $b_{1} = a_{1} + 1.2 b_{2} - b_{3} = 0.5 + 0.48 -
0.125 = 0.855$; then $p = a_{0} + x b_{1} - b_{2} = 1 + 0.6(0.855) - 0.4 =
1.113$, the same value in $n$ multiply-adds with no $T_{k}$ table formed and
no endpoint cancellation. Clenshaw is to a Chebyshev series what Horner is to
a monomial polynomial: the stable, minimal-operation backward recurrence that
every spectral and approximation library runs, and its backward stability is
the reason a Chebyshev representation of a function can be evaluated to full
precision across the whole interval where the monomial form loses digits at
the ends.
\end{example}
...
\subsection{Eigenvalues by iteration}

The eigenvalue problem $\mat{A}\vect{v} = \lambda \vect{v}$ appears wherever a
system's modes matter: the natural frequencies of a structure, the principal
components of a dataset, the stability of a linearised dynamics. The
production algorithm for a dense matrix is the shifted QR iteration behind
\texttt{numpy.linalg.eig} and LAPACK's \texttt{dgeev}, whose backward
stability the section on Householder QR already underwrites. The engineer
should be able to compute the single most important eigenvalue by hand,
because the method that does so, \engterm{power iteration}, is the seed of
every large-scale eigensolver and the exact algorithm behind the original
PageRank computation.

Power iteration repeatedly applies $\mat{A}$ to a starting vector and
renormalises: $\vect{v}_{k+1} = \mat{A}\vect{v}_{k}/\norm{\mat{A}\vect{v}_{k}}$.
Writing $\vect{v}_{0}$ in the eigenbasis $\vect{v}_{0} = \sum_{i} c_{i}
\vect{u}_{i}$ with eigenvalues ordered $\abs{\lambda_{1}} > \abs{\lambda_{2}}
\geq \cdots$, the $k$-th iterate is proportional to $\sum_{i} c_{i}
\lambda_{i}^{k}\vect{u}_{i} = \lambda_{1}^{k}\big(c_{1}\vect{u}_{1} + \sum_{i
\geq 2} c_{i}(\lambda_{i}/\lambda_{1})^{k}\vect{u}_{i}\big)$. Every term past
the first decays by the factor $(\lambda_{i}/\lambda_{1})^{k}$, so the iterate
aligns with the dominant eigenvector $\vect{u}_{1}$ at the linear rate
$\abs{\lambda_{2}/\lambda_{1}}$, and the \engterm{Rayleigh quotient}
$\vect{v}_{k}^{\transpose}\mat{A}\vect{v}_{k}/(\vect{v}_{k}^{\transpose}
\vect{v}_{k})$ converges to $\lambda_{1}$. The method's rate is governed by
the eigenvalue gap, the same way a fixed-point iteration's rate is governed by
$\abs{g'(x^{*})}$; a small gap makes it crawl.

\begin{example}
\textbf{Power iteration by hand, and the gap that sets its rate.} Take
$\mat{A} = \begin{psmallmatrix} 2 & 1 \\ 1 & 3 \end{psmallmatrix}$, whose exact
eigenvalues are $\tfrac{1}{2}(5 \pm \sqrt{5}) = 3.618$ and $1.382$, with ratio
$\lambda_{2}/\lambda_{1} = 0.382$. Start from $\vect{v}_{0} = (1, 0)^{\transpose}$.
First application: $\mat{A}\vect{v}_{0} = (2, 1)^{\transpose}$, norm
$\sqrt{5} = 2.236$, so $\vect{v}_{1} = (0.894, 0.447)^{\transpose}$; the
Rayleigh quotient is $\vect{v}_{0}^{\transpose}\mat{A}\vect{v}_{0} = 2$. Next:
$\mat{A}\vect{v}_{1} = (2.236, 2.236)^{\transpose}$, normalise to $\vect{v}_{2}
= (0.707, 0.707)^{\transpose}$; Rayleigh quotient $\vect{v}_{1}^{\transpose}
\mat{A}\vect{v}_{1} = 2(0.894)^{2} + 2(0.894)(0.447) + 3(0.447)^{2} = 1.6 +
0.8 + 0.6 = 3.0$. Next: $\mat{A}\vect{v}_{2} = (2.121, 2.828)^{\transpose}$,
normalise to $(0.6, 0.8)^{\transpose}$; Rayleigh quotient $= 3.52$. The
quotients $2.0, 3.0, 3.52, \ldots$ climb toward $3.618$, and the error falls
by roughly the factor $(0.382)^{2} = 0.146$ per step, because the Rayleigh
quotient of a symmetric matrix is accurate to second order in the eigenvector
error. The direction $\vect{v}_{k}$ tends to the dominant eigenvector
$(0.526, 0.851)^{\transpose}$. The example exposes the two working facts: the
dominant eigenpair costs one matrix-vector product per step, and the
convergence rate is the eigenvalue-gap ratio, so a clustered spectrum needs a
shift (the inverse-power and shifted-QR refinements) to separate the target
eigenvalue from its neighbours.
\end{example}

\begin{example}
\textbf{The conditioning of an eigenvalue, and why nonsymmetric matrices are
worse.} An eigenvalue carries its own condition number, distinct from the
matrix's condition number for a linear solve. For a simple eigenvalue
$\lambda$ with right eigenvector $\vect{v}$ and left eigenvector $\vect{w}$
(so $\vect{w}^{\transpose}\mat{A} = \lambda \vect{w}^{\transpose}$), a
perturbation $\mat{A} + \varepsilon\mat{E}$ shifts $\lambda$ by
$\varepsilon\, (\vect{w}^{\transpose}\mat{E}\vect{v})/(\vect{w}^{\transpose}
\vect{v}) + O(\varepsilon^{2})$, so the eigenvalue condition number is
$s(\lambda)^{-1} = \norm{\vect{w}}\norm{\vect{v}}/\abs{\vect{w}^{\transpose}
\vect{v}}$, the reciprocal of the cosine of the angle between the left and
right eigenvectors. For a symmetric matrix the left and right eigenvectors
coincide, that angle is zero, the cosine is one, and every eigenvalue is
perfectly conditioned: a perturbation of size $\varepsilon$ moves each
eigenvalue by at most $\varepsilon$. For a nonsymmetric matrix the two
eigenvectors can be nearly orthogonal, the cosine tiny, and the eigenvalue
wildly sensitive. Take $\mat{A} = \begin{psmallmatrix} 1 & 1000 \\ 0 & 1.001
\end{psmallmatrix}$, whose eigenvalues are the diagonal $1$ and $1.001$. The
right eigenvector for $\lambda = 1$ is $(1, 0)^{\transpose}$; the left
eigenvector is nearly $(0, 1)^{\transpose}$ because the large off-diagonal
couples the rows asymmetrically, so $\vect{w}^{\transpose}\vect{v} \approx
10^{-3}$ against $\norm{\vect{w}}\norm{\vect{v}} \approx 1$, an eigenvalue
condition number near $10^{3}$. A perturbation of $10^{-6}$ in the matrix
moves the eigenvalue by as much as $10^{-3}$, three decades of amplification
that a symmetric matrix would never show. The engineer who computes natural
frequencies (a symmetric mass-stiffness problem, well conditioned) reads the
eigenvalues at face value; the engineer who computes the poles of a
nonsymmetric state-space system (an unsymmetric $\mat{A}$) reports the
eigenvalue condition numbers alongside, because a backward-stable eigensolver
still returns eigenvalues whose forward error is the backward error times this
per-eigenvalue conditioning, exactly the master inequality of section 16.2
applied to the spectrum.
\end{example}

\begin{mastery}
\textbf{Randomised numerical linear algebra.} The classical factorisations
cost $O(mn^{2})$ and touch the whole matrix, which is prohibitive when
$\mat{A}$ is a terabyte-scale data matrix whose useful information sits in a
rank far below its dimensions. \engterm{Randomised numerical linear algebra}
computes an accurate low-rank factorisation at a cost that scales with the
target rank rather than the matrix dimension, by projecting the matrix onto a
small random subspace. To approximate the leading rank-$k$ subspace of an
$m \times n$ matrix $\mat{A}$, draw an $n \times (k + p)$ Gaussian random
matrix $\mat{\Omega}$ with a small oversampling $p$, form the sample matrix
$\mat{Y} = \mat{A}\mat{\Omega}$ (one pass over $\mat{A}$, cost $O(mnk)$),
orthonormalise its columns by a QR to get $\mat{Q}$, and project $\mat{B} =
\mat{Q}^{\transpose}\mat{A}$ down to a small $(k+p) \times n$ matrix whose SVD
is cheap. The product $\mat{Q}(\mat{U}_{B}\mat{\Sigma}\mat{V}^{\transpose})$ is
a near-optimal rank-$k$ approximation of $\mat{A}$, and the theory bounds its
error to within a modest factor of the Eckart-Young optimum with a failure
probability that falls exponentially in the oversampling $p$. The randomness
is not a heuristic; it is the mechanism that guarantees, with overwhelming
probability, that the random subspace captures the dominant directions, because
a Gaussian vector has a component along every direction. A power-iteration
refinement, replacing $\mat{Y} = \mat{A}\mat{\Omega}$ with $\mat{Y} =
(\mat{A}\mat{A}^{\transpose})^{q}\mat{A}\mat{\Omega}$, sharpens the decay of
the trailing singular values and tightens the bound when the spectrum decays
slowly, at $q$ extra passes. The randomised SVD is the working tool behind
large-scale principal-component analysis, spectral clustering, and model-order
reduction, and it turns a computation that classical dense methods could not
finish into a one-pass streaming algorithm; Volume VII develops the same
projection idea for feature learning. The durable point for this chapter is
that a controlled injection of randomness converts an $O(mn^{2})$ deterministic
factorisation into an $O(mnk)$ probabilistic one whose error the engineer can
still bound, a trade that pays whenever the matrix is large and its useful rank
is small.
\end{mastery}
...
\input{volumes/02-form/ch16-numerical-methods/figures/fig-cg-vs-gs}
...
\subsection{Applied vignette: solving for a receiver position from pseudoranges}

Satellite positioning turns the chapter's linear-algebra apparatus into a
device three billion people carry. A receiver on the ground estimates its
three-dimensional position $(x, y, z)$ and its own clock offset $b$ from the
travel times of signals broadcast by satellites at known positions
$(x_{k}, y_{k}, z_{k})$. The receiver clock is cheap and drifts, so the
measured range to each satellite is corrupted by a common unknown offset
$c\,b$ with $c$ the speed of light, giving the \engterm{pseudorange}

\[
\rho_{k} = \sqrt{(x - x_{k})^{2} + (y - y_{k})^{2} + (z - z_{k})^{2}} + c\,b
+ \varepsilon_{k},
\]

with $\varepsilon_{k}$ the measurement noise. Four unknowns $(x, y, z, b)$
require at least four satellites; a real receiver uses all it can see and
solves the overdetermined nonlinear least-squares problem for the position and
offset that best fit the pseudoranges. The problem exercises three strands of
the chapter at once: a nonlinear solve by Newton's method, a linear
least-squares step inside each Newton iteration, and a conditioning diagnosis
that the positioning field has named in its own vocabulary.

\paragraph{The Gauss-Newton iteration.}
Linearise each pseudorange about a current estimate $(x_{0}, y_{0}, z_{0},
b_{0})$. The gradient of the geometric range with respect to the position is
the unit vector $\hat{\vect{e}}_{k}$ pointing from the receiver to satellite
$k$, and the gradient with respect to the clock offset is $c$, so the
linearised residual for a position correction $\Delta\vect{p} = (\Delta x,
\Delta y, \Delta z, c\,\Delta b)$ is $r_{k} - \hat{\vect{e}}_{k} \cdot
(\Delta x, \Delta y, \Delta z) - c\,\Delta b$, where $r_{k}$ is the current
pseudorange residual. Stacking the satellites gives an overdetermined linear
system $\mat{G}\,\Delta\vect{p} = \vect{r}$, where the \engterm{geometry
matrix} $\mat{G}$ has one row per satellite, the first three entries being the
components of $-\hat{\vect{e}}_{k}$ and the fourth being $-1$. Each Newton (here
Gauss-Newton) step solves this least-squares system by Householder QR, updates
the estimate, and repeats; from a coarse initial guess the iteration reaches
centimetre-level agreement with the pseudoranges in three or four steps,
because the geometric range is nearly linear over the correction distances once
the estimate is within a few kilometres.

\paragraph{Conditioning is dilution of precision.}
The accuracy of the solve is set not by the algorithm but by the geometry
matrix's conditioning, and the positioning field gave the conditioning its own
name before numerical analysts standardised theirs. The covariance of the
estimated position, for pseudorange noise of standard deviation $\sigma$, is
$\sigma^{2}(\mat{G}^{\transpose}\mat{G})^{-1}$, and the scalar
\engterm{geometric dilution of precision} $\mathrm{GDOP} = \sqrt{\operatorname{tr}
((\mat{G}^{\transpose}\mat{G})^{-1})}$ multiplies the measurement noise into the
position-plus-clock error. When the visible satellites are spread across the
sky the rows of $\mat{G}$ point in many directions, $\mat{G}^{\transpose}
\mat{G}$ is well conditioned, and the GDOP is near its floor of about $2$; when
the satellites cluster in one part of the sky two rows become nearly parallel,
$\mat{G}^{\transpose}\mat{G}$ approaches singularity, and the GDOP climbs to
tens, stretching the same metre of ranging noise into tens of metres of
position error along the poorly-constrained direction.
Figure~\ref{fig:vol02ch16:gps-gdop} draws the two geometries. The GDOP is the
condition number of section 16.2 in field dress: a property of the problem
geometry that a better solver cannot improve, and the reason a receiver reports
a position-accuracy estimate that varies with the satellite configuration
overhead even when the ranging hardware is unchanged.

\input{volumes/02-form/ch16-numerical-methods/figures/fig-gps-gdop}

\paragraph{The numerical reading.}
A worked instance fixes the scale. Suppose four satellites give a
$\mat{G}^{\transpose}\mat{G}$ whose inverse has trace $9$, so $\mathrm{GDOP} =
3$; with pseudorange noise $\sigma = 3\,\text{m}$ (a plausible single-frequency
civilian figure), the position-plus-clock error standard deviation is $3 \times
3 = 9\,\text{m}$. Adding a fifth well-placed satellite that lowers the GDOP to
$2.2$ cuts the error to $6.6\,\text{m}$ with no change to the ranging hardware,
which is why a receiver's accuracy improves as more satellites rise. The
engineer who builds such a solver reports three things the chapter demands: the
algorithm (Gauss-Newton with a QR least-squares step, not the normal equations,
to avoid squaring $\kappa(\mat{G})$), the conditioning (the GDOP at the solve
epoch), and the resulting position uncertainty (the ranging noise times the
GDOP). The same three-part statement is the error budget of section 16.10
applied to a system in every pocket.

\subsection{Applied vignette: coefficient quantisation in a digital filter}

Every piece of audio a listener hears through a phone, every control loop
that stabilises a drone, every biomedical trace cleaned of mains hum, passes
through a \engterm{recursive digital filter} whose behaviour turns on the
placement of its poles relative to the unit circle. The filter's stability is
a root-location question, and the finite word length of the coefficients that
realise it is a conditioning question, so a sharp filter that is correct in
exact arithmetic can be unstable once its coefficients are stored in the
processor's floating-point or fixed-point format. The vignette carries the
chapter's coefficient-to-root conditioning of section 16.2 into a device the
reader owns.

\paragraph{Poles, stability, and the recurrence.}
An infinite-impulse-response (IIR) filter computes each output sample from
past inputs and past outputs by a recurrence $y_{n} = \sum_{k} b_{k}
x_{n-k} - \sum_{k=1}^{p} a_{k} y_{n-k}$, and its behaviour is fixed by the
roots of the denominator polynomial $A(z) = 1 + a_{1} z^{-1} + \cdots +
a_{p} z^{-p}$, the \engterm{poles} of the filter. The recurrence is stable,
meaning its response to a bounded input stays bounded, exactly when every
pole lies strictly inside the unit circle $\abs{z} < 1$; a pole on or outside
the circle produces an impulse response that fails to decay or grows without
bound. A selective filter (a narrow band-pass, a steep low-pass) achieves its
sharpness by placing poles very close to the unit circle, where the response
is large, and that is precisely where the design is most fragile.

\paragraph{Quantisation as a coefficient perturbation.}
The coefficients $a_{k}$ are computed in the design software to full
precision and then stored in the target's finite word length, whether a
$32$-bit float on a signal processor or a $16$-bit fixed-point word on a
low-power microcontroller. The stored coefficients are the design
coefficients perturbed by a rounding of relative size the format's unit
round-off, $\hat{a}_{k} = a_{k}(1 + \delta_{k})$ with $\abs{\delta_{k}} \leq
u$. The realised poles are the roots of the perturbed $\hat{A}(z)$, and the
map from the coefficients to the poles is the same coefficient-to-root map
that Wilkinson's polynomial showed can be catastrophically ill conditioned.
When two poles cluster near the unit circle, as they do in a sharp filter,
the coefficient-to-pole conditioning is large, and a rounding of the
coefficients at the level of $u$ can move a pole across the circle.
Figure~\ref{fig:vol02ch16:iir-quantization} plots a pole pair placed just
inside the circle in exact arithmetic and pushed outside by the coefficient
quantisation, and the decaying versus growing impulse response that follows.

\input{volumes/02-form/ch16-numerical-methods/figures/fig-iir-quantization}

\paragraph{The numerical reading and the fix.}
A worked scale fixes the danger. A second-order section with poles at radius
$r = 0.995$ and angle $\theta$ has denominator $A(z) = 1 - 2r\cos\theta\,
z^{-1} + r^{2} z^{-2}$, so the coefficient $a_{2} = r^{2} = 0.990025$; a
$16$-bit fixed-point word with a fractional resolution of $2^{-15} \approx 3
\times 10^{-5}$ can represent $a_{2}$ only to that step, and a rounding of one
step moves $r^{2}$ by $3 \times 10^{-5}$, hence $r$ by about $1.5 \times
10^{-5}$, which is safe for this single section but grows dangerous as the
pole radius climbs toward $r = 0.9997$ where the margin to the circle is
itself of order the quantisation step. The defence is structural and is
universal in filter deployment: never realise a high-order filter as one
direct-form recurrence, whose high-degree denominator has the Wilkinson
conditioning, but factor it into a \engterm{cascade of second-order sections}
(biquads), each with only two poles whose coefficient-to-root map is benign,
and quantise each section's four coefficients separately. The cascade trades
the ill-conditioned high-degree root problem for a product of well-conditioned
quadratic ones, exactly the reformulation-not-higher-precision move the
chapter's conditioning-versus-stability diagnostic prescribes. The engineer
who ships a filter states three things the chapter demands: the arithmetic
format of the coefficients, the pole radii and their margin to the unit
circle (the conditioning), and the realisation structure (the evidence that
the coefficient-to-pole map was kept well conditioned), and a filter reported
without them is a recurrence whose field stability the engineer cannot defend.
...
The deterministic rules place their nodes by a rule and pay a cost that grows
as $m^{d}$ for $m$ nodes per axis in $d$ dimensions, the \engterm{curse of
dimensionality} that makes a product Gauss rule useless past a handful of
dimensions. \engterm{Monte Carlo integration} escapes the curse by sampling.
To estimate $I = \int_{\Omega} f\, \dd V$ over a domain of volume $V$, draw $N$
points uniformly at random in $\Omega$ and average:

\[
\hat{I}_{N} = \frac{V}{N}\sum_{i=1}^{N} f(\vect{x}_{i}).
\]

The estimator is unbiased, and its error is set by the variance of the sample
mean: the standard error is $V\,\sigma_{f}/\sqrt{N}$, where $\sigma_{f}$ is the
standard deviation of $f$ over $\Omega$. The convergence is slow, $O(N^{-1/2})$,
and it does not improve with smoothness the way the deterministic rules do. Its
one decisive property is that the $N^{-1/2}$ rate holds in \emph{every}
dimension, because the central limit theorem that sets the rate does not know
the dimension of the sample space. A deterministic rule loses a factor per
dimension; Monte Carlo does not, so above roughly six dimensions Monte Carlo
and its low-discrepancy refinements are the only feasible quadrature.

\begin{example}
\textbf{Estimating $\pi$ by darts, and reading the error.} The area of the
unit quarter-disk is $\pi/4$, so the fraction of uniform random points in the
unit square that fall inside the quarter-circle $x^{2} + y^{2} \leq 1$
estimates $\pi/4$. This is Monte Carlo integration of the indicator function
$f(x, y) = \mathbf{1}[x^{2} + y^{2} \leq 1]$ over the unit square. With $N =
10^{4}$ points a typical run returns about $7854$ hits, giving $\hat\pi = 4
\times 7854/10^{4} = 3.1416$, close by luck; the honest report is the error
bar. The indicator has variance $p(1 - p)$ with $p = \pi/4 \approx 0.785$, so
$\sigma_{f} = \sqrt{0.785 \times 0.215} = 0.411$, and the standard error of
$\hat\pi = 4\hat{p}$ is $4\sigma_{f}/\sqrt{N} = 4(0.411)/100 = 0.0164$. The
estimate is $\hat\pi = 3.14 \pm 0.016$ at $N = 10^{4}$, and to cut the error
bar to $\pm 0.0016$, one more decimal digit, requires $N = 10^{6}$, a
hundredfold more work for a tenfold accuracy gain, the signature of the
$N^{-1/2}$ rate. The example makes the trade concrete: Monte Carlo buys
dimension-independence at the price of a punishing convergence rate, and the
engineer who reports a Monte Carlo result without its $\sigma/\sqrt{N}$
standard error has reported a number without its uncertainty, the numerical
form of the same omission the whole chapter warns against.
\end{example}

\input{volumes/02-form/ch16-numerical-methods/figures/fig-monte-carlo-convergence}

Figure~\ref{fig:vol02ch16:monte-carlo-convergence} ranks Monte Carlo against
the deterministic rules in one dimension, where the deterministic rules win
decisively, and states the property the one-dimensional plot cannot show: the
Monte Carlo slope is dimension-independent. Two refinements sharpen the rate
without giving up the dimension-independence. \engterm{Variance reduction}
(importance sampling, control variates, stratification, antithetic variates)
lowers the constant $\sigma_{f}$ by sampling where the integrand is large or by
subtracting a correlated quantity of known integral, and can cut the sample
count by orders of magnitude for a peaked integrand. \engterm{Quasi-Monte
Carlo} replaces the pseudorandom points with a low-discrepancy sequence (Sobol,
Halton) that fills the domain more evenly than random points do, improving the
rate to nearly $O(N^{-1})$ for integrands of bounded variation while keeping
the mild dimension dependence. High-dimensional financial and radiative-transfer
integrals are the working home of both refinements, and the engineer chooses
among plain Monte Carlo, variance reduction, and quasi-Monte Carlo by the
integrand's dimension, smoothness, and peaking.
...
\begin{example}
\textbf{Richardson extrapolation buys two orders from two evaluations.} The
centred difference $D(h) = (f(x+h) - f(x-h))/(2h)$ has an error expansion in
even powers, $D(h) = f'(x) + c_{2} h^{2} + c_{4} h^{4} + \cdots$, because the
odd Taylor terms cancel by symmetry. Two estimates at $h$ and $h/2$ then
eliminate the leading $h^{2}$ term the same way Romberg eliminates it in
quadrature: the combination $D_{R}(h) = (4 D(h/2) - D(h))/3 = f'(x) + O(h^{4})$
is a fourth-order estimate at the cost of one extra pair of evaluations. Take
$f(x) = e^{x}$ at $x = 0$, where $f'(0) = 1$. With $h = 0.2$: $D(0.2) =
(e^{0.2} - e^{-0.2})/0.4 = (1.221403 - 0.818731)/0.4 = 1.006681$, error
$6.68 \times 10^{-3}$. With $h = 0.1$: $D(0.1) = (1.105171 - 0.904837)/0.2 =
1.001668$, error $1.67 \times 10^{-3}$, a factor of four smaller as the
$O(h^{2})$ order promises. The Richardson combination is $D_{R} = (4(1.001668)
- 1.006681)/3 = (4.006672 - 1.006681)/3 = 1.0000$ (to the digits shown), error
about $3 \times 10^{-6}$, three decades better than either input at no new
function values beyond the two the pair already required. The extrapolation is
the derivative analogue of Romberg quadrature, and it is why a production
differentiation routine builds a small Richardson table rather than pushing a
single step size down into the round-off wall of
Figure~\ref{fig:vol02:ch16:error-vs-h}.
\end{example}

\begin{example}
\textbf{The complex step, and why it has no floor.} The finite difference is
trapped by the subtraction of nearly equal reals. The complex-step estimator
sidesteps the subtraction entirely. For a function $f$ analytic at a real $x$,
expand $f$ at an imaginary perturbation: $f(x + \mathrm{i}h) = f(x) +
\mathrm{i}h\,f'(x) - \tfrac{h^{2}}{2}f''(x) - \mathrm{i}\tfrac{h^{3}}{6}
f'''(x) + \cdots$. The imaginary part is $\Im[f(x + \mathrm{i}h)] = h\,f'(x) -
\tfrac{h^{3}}{6}f'''(x) + \cdots$, so

\[
f'(x) = \frac{\Im[f(x + \mathrm{i}h)]}{h} + O(h^{2}),
\]

a second-order estimate that never subtracts two nearly equal numbers, because
the derivative information rides in the imaginary channel and $f(x)$ sits in
the real channel. Take $f(x) = e^{x}\sin x$ at $x = 1.5$, whose exact
derivative is $e^{1.5}(\sin 1.5 + \cos 1.5) = 4.4817(0.99749 + 0.070737) =
4.7853$. A centred difference at $h = 10^{-8}$ already sits near its
round-off floor and returns about eight correct digits. The complex step at
$h = 10^{-8}$ evaluates $f(1.5 + \mathrm{i}\,10^{-8}) = e^{1.5 +
\mathrm{i}10^{-8}}\sin(1.5 + \mathrm{i}10^{-8})$ and divides the imaginary part
by $10^{-8}$, returning the derivative to full binary64 precision, and it holds
that precision as $h$ shrinks to $10^{-100}$ and below, where the finite
difference has long since collapsed to noise.
Figure~\ref{fig:vol02ch16:complex-step} contrasts the two against Richardson
extrapolation. The one requirement is that $f$ admit evaluation at a complex
argument, which every arithmetic expression and every analytic special
function does; a program that branches on the sign of its input, or calls a
real-only library routine, does not, and there the engineer falls back to the
Richardson difference or to automatic differentiation.
\end{example}

\input{volumes/02-form/ch16-numerical-methods/figures/fig-complex-step}
...
\input{volumes/02-form/ch16-numerical-methods/figures/fig-adaptive-quadrature}
...
Choosing among them means separating solvers worth running from
solvers that will silently lie, a judgment that rests on stability and
order of accuracy.
...
\input{volumes/02-form/ch16-numerical-methods/figures/fig-stiff-explicit-implicit}
...
0.05(3(-0.904837) - (-1)) = 0.904837 + 0.05(-1.714512) = 0.819111$, against
the exact $e^{-0.2} = 0.818731$, an error of $3.8 \times 10^{-4}$, the
...
predicted slope $f_{2}^{P} = -0.819111$ gives $y_{2}^{C} = y_{1} +
\tfrac{h}{2}(f_{1} + f_{2}^{P}) = 0.904837 + 0.05(-0.904837 - 0.819111) =
0.818640$, error $9.1 \times 10^{-5}$, several times better than the predictor
alone, and the predictor-corrector difference $\abs{y_{2}^{C} - y_{2}^{P}}
= 4.7 \times 10^{-4}$ is the local error estimate the step controller reads.
...
\begin{example}
\textbf{The CFL condition read off a step-size sweep.} The advection equation
$u_{t} + a\,u_{x} = 0$ carries a profile rightward at speed $a$ without
changing its shape. Discretise it by explicit upwind, $u_{i}^{n+1} = u_{i}^{n}
- \nu(u_{i}^{n} - u_{i-1}^{n})$ with the \engterm{Courant number} $\nu = a\,
\Delta t/\Delta x$. Take $a = 1$ on a grid with $\Delta x = 0.01$ and integrate
a smooth bump. At $\Delta t = 0.008$ the Courant number is $\nu = 0.8 \leq 1$,
the update coefficients $1 - \nu = 0.2$ and $\nu = 0.8$ are both nonnegative, so
each new value is a convex average of two old ones, bounded between them, and
the scheme is stable: the bump advects across the grid holding its shape to the
scheme's first-order accuracy. At $\Delta t = 0.012$ the Courant number is
$\nu = 1.2 > 1$, the coefficient $1 - \nu = -0.2$ turns negative, the new value
is no longer a bounded average, and node-to-node oscillations appear and grow
by the factor $\abs{1 - \nu} + \nu = 1.4$ per step, so after twenty steps the
error is amplified by $1.4^{20} \approx 800$ and the solution has diverged. The
threshold is exactly $\nu = 1$, the Courant-Friedrichs-Lewy condition. The
stability boundary is not a round-off effect and does not soften with a finer
grid: refining $\Delta x$ while holding $\Delta t$ makes $\nu$ larger and the
instability worse, the reverse of the intuition that a finer grid is safer.
Figure~\ref{fig:vol02ch16:cfl-condition} reads the condition off the stencil as
a statement about which grid points carry the information the answer needs
\cite{text:v2ch16-courant-friedrichs-lewy-1928}.
\end{example}

\input{volumes/02-form/ch16-numerical-methods/figures/fig-cfl-condition}
...
\input{volumes/02-form/ch16-numerical-methods/figures/fig-error-budget}
...
diagonal or near-diagonal in that basis. The working arithmetic of both is what a reader who calls
\texttt{numpy.fft.fft} or builds a Chebyshev solver should be able to
defend.
...
\input{volumes/02-form/ch16-numerical-methods/figures/fig-fft-butterfly}
...
\input{volumes/02-form/ch16-numerical-methods/figures/fig-spectral-convergence}
...
\input{volumes/02-form/ch16-numerical-methods/figures/fig-peclet-instability}
...
\input{volumes/02-form/ch16-numerical-methods/figures/fig-mms-convergence}
...
\input{volumes/02-form/ch16-numerical-methods/figures/fig-conv-diff-budget}
...
simulation, design, diagnosis, and judgment.
...
\begin{exercise}
Reread the Patriot Dhahran case study in section 16.12.2. State
...
\begin{exercise}
Reread the Ariane 5 Flight 501 case study in section 16.12.3. The
