...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-skin-depth.tex}
...
is $\delta = c\sqrt{m_e\varepsilon_0/(n_e e^{2})} = 5.31\times 10^{12}/\sqrt{n_e}$
...
far higher temperature and $\Gamma \approx 2\times 10^{-7}$, weaker still, which
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-wave-dispersion.tex}
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-two-stream.tex}
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-gradb-drift.tex}
...
10^{8})] \approx 4\times 10^{-14}\,\si{\watt}$. Multiplied by the electron
density $10^{20}\,\si{\per\meter\cubed}$ this is $\sim 4\times 10^{6}\,\si{\watt
...
$\nu_{ei} \approx 2.9\times 10^{-6}\,n_e\ln\Lambda\,[T_e/\text{eV}]^{-3/2}\,
\si{\per\second}$. For the case-study tokamak at $n_e = 10^{20}\,\si{\per\meter
\cubed}$, $T_e = 12\,\si{\kilo\electronvolt} = 1.2\times 10^{4}\,\text{eV}$,
and $\ln\Lambda = 17$, this is $\nu_{ei} \approx 2.9\times 10^{-6}(10^{20})(17)
...
$\nu_{ei} \approx 2.9\times 10^{-6}(10^{17})(10)/1 \approx 2.9\times
10^{12}\,\si{\per\second}$ and, with $v_{te} \approx 4.2\times 10^{5}\,\si{
\meter\per\second}$, a mean free path of $\lambda_{\mathrm{mfp}} \approx
1.4\times 10^{-7}\,\si{\meter}$, a fraction of a micron. The lamp electron
collides constantly, the plasma is collisional, and a fluid description with
local transport coefficients applies, the opposite extreme from the core. The
single group $n_e T_e^{-3/2}$ carries the plasma from one regime to the other.

\input{volumes/04-energy/ch13-plasma-physics/figures/fig-safety-factor.tex}
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-runaway.tex}
...
10^{-15}) \approx 1.89\times 10^{-38}$, so $E_D \approx 3.7\times 10^{2}\,\si{
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-magnetic-stress.tex}
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-pressure-balance.tex}
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-beta-troyon.tex}
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-timescale-ordering.tex}
...
(1.58\times 10^{13})] \approx 3.35\times 10^{-16}\,\si{\meter\squared}$, so
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-ionospheric-refraction.tex}
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-paschen-curve.tex}
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-fusion-gain.tex}
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-divertor-flux.tex}
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-ccp-reactor.tex}
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-etch-bias.tex}
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-hall-thruster.tex}
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-hall-performance.tex}
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-magnetron-sputter.tex}
...
\input{volumes/04-energy/ch13-plasma-physics/figures/fig-dust-charging.tex}
...
10^{-26})} \approx 6.96\times 10^{4}$, and $V_{s}^{3/2} = 350^{1.5} \approx
6.55\times 10^{3}$, so the bracket is $\tfrac49(8.85\times 10^{-12})(6.96
\times 10^{4})(6.55\times 10^{3})/5 \approx 3.6\times 10^{-4}\,\si{\meter
\squared}$ and $s \approx 1.9\times 10^{-2}\,\si{\meter}$. The full
$1.9\,\si{\centi\meter}$ result overstates the true sheath because the
Child-Langmuir form ignores the finite ion entry velocity and the RF
modulation of the sheath; the working sheath is a few millimetres, still
many Debye lengths thick and consistent with the $90\,\si{\micro\meter}$
...
10^{-34})^{2}])^{3/2} \approx (2.7\times 10^{22})^{3/2} \approx 4.5\times
10^{33}\,\si{\per\meter\cubed}$. The atmospheric neutral density is $n_{n}
\approx p/(k_{B}T) = 10^{5}/[(1.38\times 10^{-23})(1.5\times 10^{4})]
\approx 4.8\times 10^{23}\,\si{\per\meter\cubed}$. With $n_{i} = n_{e}$, the
Saha relation $n_{e}^{2}/n_{n} = (4.5\times 10^{33})(5.0\times 10^{-6})
\approx 2.3\times 10^{28}$ gives $n_{e} = \sqrt{(2.3\times 10^{28})(4.8\times
10^{23})} \approx 3.3\times 10^{25}\,\si{\per\meter\cubed}$, an ionisation
fraction of order $n_{e}/n_{n}\approx 7\%$. A few percent ionisation at
$15{,}000\,\si{\kelvin}$ is enough to make the arc a good conductor and to
carry the hundreds of amperes a cutting torch draws, which is why thermal
arcs are genuine equilibrium plasmas while a fluorescent lamp at the same
ionisation fraction but far lower temperature is a non-equilibrium one.
...
\approx 1.4\,\si{\meter\squared\per\second}$. The ion gyroradius is $\rho_i =
...
10^{-3}$ and $D_{\mathrm{gB}} \approx 3\times 10^{-3}\,\si{\meter\squared\per
\second}$. The confinement time it implies is $\tau_E \sim a^{2}/D_{\mathrm{gB}}
\approx 4/(3\times 10^{-3}) \approx 1.3\times 10^{3}\,\si{\second}$, far
...
overstates confinement by orders of magnitude because the turbulence is not a
