...
Six of the book's ten recurring problem archetypes make their first
...
\begin{itemize}[leftmargin=*]
  ...
  \item \engterm{Balance}: dimensional balance as the first form, in
    Chapter 2. Every later balance archetype (mass, momentum, energy,
    charge, information, population) builds on the same
    accounting-equation discipline.
  ...
  \item \engterm{Interface}: the place where one quantity's
    representation is translated into another's. Introduced in
    Chapter 3 as the calibration handoff between the world and an
    instrument's indication; recurring in Chapter 5 as the
    sensor-to-system handoff. The interface archetype recurs across
    later volumes whenever a quantity crosses between subsystems.
  \item \engterm{Transport}: the chain by which a signal moves from
    its physical source through the measurement system to the
    engineer's record. Introduced in Chapter 5 in the form of
    sensor signal transport; recurring in Volume IV (heat and mass
    transfer), Volume VII (network packet routing), Volume IX
    (system flows), and Volume XII (supply chains and infrastructure
    flows).
\end{itemize}

The remaining four archetypes (stability, optimisation, control,
ethics) arrive in later volumes, in the chapters where the
