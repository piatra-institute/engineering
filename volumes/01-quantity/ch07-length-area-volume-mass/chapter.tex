...
\subsubsection*{Worked example: sag and tension on a steel tape over
a span}

At the opposite end of the length ladder from the gauge-block stack,
the humble steel tape measure carries two systematic corrections that
the working surveyor and site engineer must apply on any span long
enough to matter. A $30 \,\si{\meter}$ steel tape suspended between
two supports and measuring a horizontal distance reads long, because
the tape follows a catenary and its arc length exceeds the chord it
spans, and it reads short or long depending on whether the field
tension differs from the tension at which the tape was standardised.

The sag correction runs first. A tape of mass per unit length
$w = 0.020 \,\si{\kilogram\per\meter}$ (a typical light steel tape)
pulled to a field tension $P = 50 \,\si{\newton}$ over an unsupported
span $L = 30 \,\si{\meter}$ hangs in a shallow catenary whose
excess arc length over the chord is, to the working second-order
approximation,
\[
C_{\mathrm{sag}} = -\frac{w^{2} g^{2} L^{3}}{24 P^{2}}
= -\frac{(0.020)^{2} (9.81)^{2} (30)^{3}}{24 \times (50)^{2}}
\approx -0.017 \,\si{\meter},
\]
a $17 \,\si{\milli\meter}$ correction, always negative (the tape
arc is longer than the straight-line distance, so the distance read
off the tape overstates the true horizontal length). The correction
scales with the cube of the span and the inverse square of the
tension: halving the tension quadruples the sag error, and doubling
the span multiplies it eightfold.

The tension correction runs second. A steel tape is standardised at
a stated reference tension, commonly $P_{0} = 50 \,\si{\newton}$
under support conditions printed on the tape. If the field tension
$P$ differs, the tape stretches elastically by
\[
C_{\mathrm{tension}} = \frac{(P - P_{0}) L}{A E},
\]
with $A$ the tape's cross-sectional area and $E$ the elastic modulus
of steel ($E \approx 200 \,\text{GPa}$). For a tape of section
$A = 3.0 \,\si{\milli\meter} \times 0.25 \,\si{\milli\meter} =
0.75 \,\si{\milli\meter\squared} = 7.5 \times 10^{-7} \,\si{\meter
\squared}$ pulled to $P = 80 \,\si{\newton}$ against a standardising
$P_{0} = 50 \,\si{\newton}$, the stretch over $30 \,\si{\meter}$ is
$(30)(30)/(7.5 \times 10^{-7} \times 2.0 \times 10^{11}) = 900 /
(1.5 \times 10^{5}) = 6.0 \times 10^{-3} \,\si{\meter}$, a positive
$6 \,\si{\milli\meter}$ (the higher field tension stretches the tape,
so the graduations spread and the tape reads short unless the stretch
is added back).

The two corrections combine additively with the thermal correction
of the failure-modes section. On the worked span the sag correction
($-17 \,\si{\milli\meter}$) dominates the tension correction
($+6 \,\si{\milli\meter}$); a naive reading that applied neither
would overstate the $30 \,\si{\meter}$ distance by about
$11 \,\si{\milli\meter}$ net, or $3.7 \times 10^{-4}$ relative, far
outside the tolerance of any survey traverse or steel-fabrication
layout. The working mitigation is to support the tape at intermediate
points (which cuts the span $L$ in each sag term and reduces the
cubed contribution sharply), to apply the standardising tension with
a spring balance rather than by feel, and to record the tape
temperature for the thermal correction. The discipline is the same
one the gauge-block stack teaches at the millimetre scale: a length
instrument carries systematic corrections that the raw reading does
not expose, and the defensible measurement is the corrected one with
its corrections recorded.

\subsubsection*{Worked example: a precision injection-mould cavity at
operating temperature}

A second working case ties the gauge-block stack above to the
production-tooling world. A two-cavity injection mould tool is being
machined for a polycarbonate optical housing. The cavity dimension
along the length axis is specified at $200 \,\si{\milli\meter}$
$\pm\,15 \,\si{\micro\meter}$, and the part is moulded in a tool
that runs at $200 \,\si{\degreeCelsius}$ steady-state. The tool
steel is a hardened P20 grade with linear expansion coefficient
$\alpha_{\mathrm{steel}} \approx 1.1 \times 10^{-5} \,\si{\per
\kelvin}$. The dimensional standard for the drawing is the ISO 1
reference temperature of $20 \,\si{\degreeCelsius}$
\cite{std:iso-1-2022}. The CNC tool-room machining the cavity, and
the CMM verifying it after machining, both operate at $20 \,\si
{\degreeCelsius}$. The questions the tooling engineer must answer
read in order: what target dimension does the machinist cut to,
what target dimension does the CMM check against, what residual
geometric error remains when the tool reaches process temperature,
and which uncertainty contributions threaten the $15 \,\si{\micro
\meter}$ tolerance.

The thermal-expansion analysis runs first. From $20 \,\si
{\degreeCelsius}$ to $200 \,\si{\degreeCelsius}$ is a temperature
step $\Delta T = 180 \,\si{\kelvin}$. The linear strain on the
cavity dimension is $\alpha_{\mathrm{steel}} \Delta T = 1.1 \times
10^{-5} \times 180 = 1.98 \times 10^{-3}$, or about $2 \,\si
{\milli\meter}$ per $\si{\meter}$. Over the $200 \,\si{\milli\meter}$
cavity dimension the steel expands by $200 \times 1.98 \times
10^{-3} = 0.396 \,\si{\milli\meter}$, about $400 \,\si{\micro\meter}$
from $20$ to $200 \,\si{\degreeCelsius}$. The tolerance on the cold
cavity is $15 \,\si{\micro\meter}$; the thermal expansion alone is
twenty-six times the tolerance. The tool cavity must be cut
undersized at room temperature so that, at operating temperature,
the cavity is the correct nominal dimension. The compensation is
the negative of the expansion: the cavity is machined at
$200 - 0.396 = 199.604 \,\si{\milli\meter}$ at $20 \,\si
{\degreeCelsius}$, with the same $\pm\,15 \,\si{\micro\meter}$
tolerance band carried unchanged.

A second compensation runs at the part level. The moulded
polycarbonate contracts on cooling from the melt at about
$300 \,\si{\degreeCelsius}$ to the ambient at $25 \,\si
{\degreeCelsius}$, with linear shrinkage typically $0.5\,\%$ to
$0.7\,\%$ for unfilled polycarbonate
\cite{ch07t3-text:rosato-injection-molding-2012}. The part-shrinkage
allowance is added to the room-temperature target: at a nominal
shrinkage of $0.6\,\%$, the cavity at room temperature must be
$200 \times 1.006 = 201.2 \,\si{\milli\meter}$ to deliver a
$200 \,\si{\milli\meter}$ part after cooling. The two compensations
combine: cavity dimension at $20 \,\si{\degreeCelsius}$, before the
tool is heated, is the part target multiplied by the part-shrinkage
allowance and reduced by the tool-steel thermal expansion at
process temperature, or
\[
L_{\mathrm{machine}} = L_{\mathrm{part}} \cdot (1 + s_{\mathrm{shrink}})
\cdot (1 - \alpha_{\mathrm{steel}} \Delta T)
\approx 200 \times 1.006 \times (1 - 1.98 \times 10^{-3})
\approx 200.81 \,\si{\milli\meter},
\]
to the same order as the $\pm\,15 \,\si{\micro\meter}$ tolerance.
The two corrections work in opposite directions; the part-shrinkage
allowance is roughly three times the magnitude of the tool-steel
thermal expansion, so the room-temperature cavity reads larger than
the nominal part dimension, not smaller.

The CMM verification at $20 \,\si{\degreeCelsius}$ checks against
$L_{\mathrm{machine}}$, not against the nominal $200 \,\si{\milli
\meter}$. A CMM specification for a precision tool-room machine is
of the form $A + B L$ with $A = 1.5 \,\si{\micro\meter}$ and
$B = 4 \times 10^{-6}$ at the $k = 2$ expanded level, so the CMM
maximum permissible error on a $200 \,\si{\milli\meter}$ measurement
is $1.5 + 4 \times 10^{-6} \times 200000 = 1.5 + 0.8 \approx
2.3 \,\si{\micro\meter}$ at $k = 2$. Converted to a standard
uncertainty (divide by $2$) and combined with a temperature-control
contribution (the CMM laboratory is held to $20 \pm 0.5 \,\si
{\degreeCelsius}$, giving a residual length uncertainty of
$200 \,\si{\milli\meter} \times 1.1 \times 10^{-5} \times
0.5 / \sqrt{3} \approx 0.64 \,\si{\micro\meter}$ from a
uniform-distribution rule), the combined standard uncertainty on
the CMM verification is $\sqrt{1.15^{2} + 0.64^{2}} \approx
1.32 \,\si{\micro\meter}$, or about $2.6 \,\si{\micro\meter}$ at
$k = 2$. The CMM verification is roughly six times tighter than
the $15 \,\si{\micro\meter}$ tolerance band, leaving room for the
machining process variability and for the shrinkage-allowance
uncertainty (typically $\pm 0.05\,\%$ about the nominal $0.6\,\%$,
or $\pm 10 \,\si{\micro\meter}$ on $200 \,\si{\milli\meter}$, by far
the dominant contribution in the cold-tool measurement chain).

The CMM in the tool room is itself calibrated against a length bar
traceable, through a national metrology institute (NIST, NPL, PTB
under their respective accreditation schemes
\cite{ch07t3-std:iso-iec-17025-2017}) to the SI metre. The
traceability path runs cavity to length bar to gauge block to laser
interferometer to iodine-stabilised laser to the SI definition of
the metre. Each link carries its calibration certificate; the
calibration laboratory's accreditation under ISO/IEC 17025
\cite{ch07t3-std:iso-iec-17025-2017} carries forward the
chain-traceability claim. The lesson for the working tooling
engineer is the same one the gauge-block stack above carries: the
dimensional uncertainty budget is built from the machine, the
environment, and the standard, and the standard's traceability is
itself the chain whose audit determines whether the $15 \,\si
{\micro\meter}$ tolerance can be defended.

A common failure mode in this case is omitting one of the two
compensations. A tool-room that cuts to the nominal $200 \,\si
{\milli\meter}$ and ignores both shrinkage and thermal expansion
delivers parts about $0.8 \,\si{\milli\meter}$ undersized at room
temperature, or $1.2 \,\si{\milli\meter}$ undersized at operating
temperature, both far outside any plausible tolerance band. A
tool-room that compensates for shrinkage but not for thermal
expansion delivers parts about $0.4 \,\si{\milli\meter}$ oversized
at the cold tool and on-nominal at the hot tool, often only caught
when the first-shot part is measured. The discipline records both
allowances on the tooling drawing, signs them off with the moulding
engineer's process-temperature specification, and verifies the
cavity at the CMM against the compensated dimension, not against
the nominal part dimension. The case sits inside the dimensional
test-conditions framework of ISO 10791-7 \cite{ch07t3-std:iso-10791-7-2014},
which fixes test-piece geometry and tolerance structure for
machining-centre qualification at the $20 \,\si{\degreeCelsius}$
reference.

\subsection*{Historical sketch: the metre's three definitions and
the 2019 kilogram redefinition}
\addcontentsline{toc}{subsection}{Historical sketch: the metre's
three definitions and the 2019 kilogram redefinition}

The two units this chapter measures most often, the metre and the
kilogram, carry the longest definition history of any SI base unit.
The metre has been redefined three times in roughly two centuries;
the kilogram waited 130 years for its first redefinition, in 2019.
Both histories share the same structural pattern: an artefact
becomes the unit, a better physical realisation eventually exists,
the artefact is retired, and the unit is tied to a constant of
nature whose value is fixed by definition. The transition is each
time delayed by the absence of a better realisation and accelerated
by the achievement of one.

The metre's first definition (1799) was a prototype platinum bar,
deposited at the Archives nationales (Paris) on 22 June 1799 after
the survey of the Paris meridian by Delambre and M\'echain. The bar
was a single artefact whose length was, by definition, one metre.
The 1st CGPM, meeting in Paris in September 1889, replaced the
Archives bar with the International Prototype Metre, a
platinum-iridium X-section bar held at the BIPM (Pavillon de
Breteuil, S\`evres) along with the International Prototype Kilogram
\cite{ch07t3-web:bipm-cgpm-1889-prototype}. The relative standard
uncertainty achievable with the artefact metre was of order
$10^{-4}$ before bath-comparison protocols were in routine use and
of order $10^{-7}$ after, set by the precision of the line scratches
on the bar and by the comparator microscope reading.

\input{volumes/01-quantity/ch07-length-area-volume-mass/figures/fig-metre-definition-timeline.tex}

The metre's second definition (1960) replaced the artefact bar with
a wavelength of light. The 11th CGPM, in Resolution 6, defined the
metre as $1{,}650{,}763.73$ wavelengths in vacuum of the orange-red
line corresponding to the transition between the $2p_{10}$ and
$5d_{5}$ energy levels of the krypton-86 atom
\cite{ch07t3-web:bipm-cgpm-1960-krypton-metre}. The motivation was
that no artefact was more stable than a quantum transition, and the
relative standard uncertainty of the krypton wavelength realisation
reached about $10^{-8}$. The realisation required a hot-cathode
discharge lamp filled with isotopically pure krypton-86; only a few
metrology laboratories worldwide ran them, and the lamp's lifetime
and the broadening of the line by Doppler and pressure effects
limited the reproducibility.

The metre's third definition (1983) replaced the wavelength with a
constant. The 17th CGPM, in Resolution 1, fixed the speed of light
at $c = 299{,}792{,}458 \,\si{\meter\per\second}$ exactly, and
defined the metre as the distance light travels in
$1/299{,}792{,}458$ of a second
\cite{ch07t3-web:bipm-cgpm-1983-speed-of-light-metre}. The metre
became a derived unit, dependent on the second, which had been
defined since 1967 in terms of the cesium-133 hyperfine transition.
The motivation was that the krypton-86 line had been outperformed
by laser interferometry: a stabilised laser whose frequency was
measured against a cesium clock realised the metre at relative
uncertainty of order $10^{-11}$ once the speed of light was fixed,
two to three decades better than the krypton wavelength. The
iodine-stabilised helium-neon laser at $633 \,\si{\nano\meter}$
became the working \engterm{mise en pratique} of the metre, with a
frequency known to within a few parts in $10^{11}$
\cite{web:bipm-mise-en-pratique-kilogram-2019}.
\Cref{fig:vol01:ch07:metre-timeline} maps the three definitions and
their uncertainty floors on a common timeline.

The kilogram's history runs in parallel until 2019. The 1st CGPM
(1889) sanctioned the IPK, a platinum-iridium cylinder of $39 \,\si
{\milli\meter}$ height and diameter held under nested bell jars at
the BIPM
\cite{ch07t3-web:bipm-cgpm-1889-prototype,paper:girard-ipk-1994}.
The IPK served as the world's reference mass for 130 years. The
periodic verifications recorded apparent drifts among the artefacts
of order tens of micrograms over decades, but the IPK's primacy
made the absolute drift unmeasurable: by definition, the IPK was
exactly one kilogram. The 26th CGPM, in Resolution 1 (Versailles,
16 November 2018, taking effect 20 May 2019), fixed the Planck
constant at $h = 6.626\,070\,15 \times 10^{-34} \,\si{\joule\second}$
exactly, redefining the kilogram in terms of $h$, the second, and
the metre. The redefinition required the development of the Kibble
balance to a relative uncertainty of $10^{-8}$, work that took four
decades from Kibble's 1975 proposal at NPL to the CODATA
adjustment that supported the redefinition
\cite{paper:stock-kibble-2018,std:bipm-si2019,%
web:bipm-mise-en-pratique-kilogram-2019}.

The structural lesson behind both histories is that a unit defined
by an artefact carries the artefact's drift as a definitional
unmeasurable. The metre escaped that condition in 1960 and reached
its present form in 1983; the kilogram escaped in 2019. The working
engineer rarely meets the primary realisations directly, but every
calibration certificate in the tool room or the metrology
laboratory ties through a chain to one of these definitions, and
the certificate's uncertainty floor is bounded by the floor of the
primary realisation. The 1983 metre definition makes a $10^{-11}$
length realisation achievable, and the 2019 kilogram definition
makes a $10^{-8}$ mass realisation achievable. A working measurement
at $10^{-3}$ relative uncertainty sits eight to five decades above
the definitional floor; the floor is rarely the binding constraint
in practical work, and that comfortable distance is the product of
the definitional work above.
...
\paragraph{Scanner selection: structured-light, laser-line,
photogrammetry.}
Three classes of capture instrument deliver the point clouds the
mesh-integration pipeline above consumes. A working engineer picks
the class by surface texture, working volume, and target
uncertainty, not by instrument prestige.

\engterm{Structured-light scanners} project a sequence of fringe
patterns (typically sinusoidal-phase or binary-Gray code) onto the
part and read the deformed pattern with one or two calibrated
cameras at a known stereo baseline. The mid-range working unit at
the time of writing is the Artec Eva (around $19{,}800 \,\$ $ list
price as of 2025), with lateral resolution about $0.2 \,\si{\milli
\meter}$ and depth noise about $0.1 \,\si{\milli\meter}$ at
$0.5 \,\si{\meter}$ working distance, covering a working volume
roughly the size of a shoebox to a small chair. Structured-light
units fail on transparent and on highly specular surfaces (the
pattern is not visibly returned), and on objects with no surface
texture for the alignment software to register adjacent scans
against.

\engterm{Laser-line scanners and time-of-flight units} project a
laser line (or sweep a point) and read the diffuse return from the
surface. The FARO Focus S 350 (around $30{,}000 \,\$ $ for the
S350 unit as of 2024) is a working terrestrial unit with maximum
range $350 \,\si{\meter}$ and range noise around $0.3 \,\si
{\milli\meter}$ at $10 \,\si{\meter}$. Laser-line scanners are
preferred for larger working volumes (room-scale and outdoor) but
their per-point uncertainty is larger than structured-light at
benchtop scale, and their range noise is range-dependent.

\engterm{Photogrammetry} reconstructs the surface from a set of
overlapping photographs taken from different viewpoints. A working
setup uses Agisoft Metashape Professional (\$3{,}499 perpetual
licence as of 2025) plus a DSLR or mirrorless camera with a
calibration target. The achievable uncertainty depends on the
camera, the lens, the number of viewpoints (typically forty to
two hundred for a benchtop object), and the calibration target's
quality; an instrumented setup against a calibration cube reaches
relative volume uncertainty of order $0.5\,\%$ to $1\,\%$ on a
benchtop object. Photogrammetry's advantage is that the capture
hardware is whatever camera the engineer already owns; its cost is
the calibration and processing discipline. The choice across these
three classes is, as always, the chapter-5 selection step (state
measurand, identify candidate principles, compare specifications
against tolerance, document the choice).

\paragraph{Mesh-cleaning workflow.}
The raw point cloud from any of these instruments is processed
into a triangular mesh by software (Artec Studio, Geomagic Wrap,
or open-source MeshLab and PyMeshLab). The intermediate stages are
worth listing because each is a discrete uncertainty contribution
the working engineer can audit:

\begin{itemize}[leftmargin=*]
  \item \engterm{Outlier removal}. Stray points from
    specular returns, edge effects, or background scattering are
    deleted, typically by statistical outlier filter (drop a point
    if its mean distance to the $k$ nearest neighbours exceeds the
    cluster mean by more than two standard deviations). The filter
    is logged with its threshold; an aggressive threshold deletes
    real surface detail, a permissive one leaves noise in the mesh.
  \item \engterm{Surface reconstruction}. The cleaned point cloud
    is meshed by Poisson reconstruction (a smooth-surface global
    fit) or by ball-pivoting (a local triangulation). Poisson is
    preferred for smooth organic shapes; ball-pivoting is preferred
    when sharp edges must be preserved.
  \item \engterm{Hole filling and decimation}. Boundary holes are
    closed (flat fill, smooth fill, or context-aware fill); the
    mesh is decimated to a working triangle count by edge-collapse.
    Decimation below the lateral resolution of the scanner is
    waste; decimation above it discards measurement information.
  \item \engterm{Watertight check}. The Euler characteristic
    $V - E + F = 2$ for a closed genus-zero surface is verified
    along with the count of boundary loops, which must be zero. A
    non-watertight mesh's volume reading is unreliable; the volume
    algorithm relies on the closed-surface divergence-theorem
    identity given earlier.
\end{itemize}

The mesh-cleaning workflow can introduce a fifth uncertainty
contribution beyond depth-noise, mesh-repair, thermal-expansion, and
scale-calibration above: the \engterm{reconstruction bias} of the
chosen meshing algorithm. Poisson reconstruction's smooth-surface
prior tends to read concave features at the bottom of a hole as
slightly less concave than they are, biasing the recovered volume
high by an amount that scales with the smoothing parameter; a
ball-pivoting reconstruction's local-triangulation bias is in the
opposite direction. A working uncertainty audit runs both
reconstructions on the same point cloud and reports the spread as a
reconstruction-bias contribution to the volume budget; on
the bracket of the worked example, the spread is typically of order
$0.2\,\%$ to $0.4\,\%$ of $V_{0}$, comparable to the depth-noise
contribution.

\paragraph{When water-displacement is the wrong method: a porous
sculpture.}
A counter-case to the worked example above runs in the opposite
direction. A craft studio asked the editors to measure the volume
of an unfired clay sculpture (roughly $250 \,\si{\milli\liter}$
nominal) for an insurance valuation tied to displaced volume of
gallery space. The first attempt used water displacement: the
sculpture was immersed in a graduated cylinder of measured water
volume, and the rise was read. The first reading was $295 \,\si
{\milli\liter}$, a $20\,\%$ overshoot against the dimensional
estimate from callipers. Within three minutes of immersion, the
reading fell back to $260 \,\si{\milli\liter}$ and the water
surface around the sculpture acquired a haze. The clay was
absorbing water; the sculpture's apparent volume increased while it
swelled, and the water level fell as the clay's pore network filled.
After ten minutes the reading stabilised near $278 \,\si{\milli
\liter}$, but the sculpture's mass had increased by $35 \,\si{\gram}$
from the absorbed water, signalling that what was being measured
was no longer the sculpture's volume but the sculpture plus absorbed
water minus the air the water displaced from the pores.

Two routes recover the volume measurement. The first is to coat
the sculpture in a thin paraffin film (about $20 \,\si{\micro\meter}$,
applied as a melted wax dip) before immersion; the paraffin layer
adds a known volume that is subtracted, and prevents water from
entering the pores during the displacement reading. The paraffin
route gave a volume of $252 \,\si{\milli\liter}$ on the sculpture
above, within $1\,\%$ of the calliper estimate. The second route
skips water-displacement entirely and uses a structured-light scan,
which has no contact with the porous surface and is not perturbed
by absorption. The scan-derived volume on the same sculpture was
$254 \,\si{\milli\liter}$, agreeing with the paraffin-coated
displacement to within their combined uncertainty. The lesson is
the selection-step lesson from earlier: water-displacement assumes
the object is not absorbent and does not change shape during the
reading; an object that violates either assumption requires either
a contact-free method (3D scanning, geometric reconstruction) or a
modified-protocol displacement (paraffin, latex film, or another
coating that the object does not absorb). The discipline is to
identify the violated assumption before the measurement, not after.

\paragraph{Volume of a porous ceramic by three methods.}
A second worked comparison sets three independent volume methods
against each other on the same artefact, a sintered alumina specimen
of nominal volume $50 \,\si{\centi\meter\cubed}$ produced to a
calibration tolerance of $\pm 0.05 \,\si{\centi\meter\cubed}$ for an
in-house metrology check. The three methods are water displacement,
helium gas pycnometry, and industrial computed tomography. They are
expected to disagree, and the magnitude and direction of the
disagreement is the working diagnostic.

\engterm{Water displacement} reads the rise of water in a
$100 \,\si{\milli\liter}$ graduated cylinder when the specimen is
submerged with a thin nylon hanger. The reading is $50.3 \,\si{
\centi\meter\cubed}$, with a per-reading standard uncertainty of
about $0.3 \,\si{\centi\meter\cubed}$ from the cylinder graduation
and the meniscus. The displacement reading includes the volume of
the open pores at the specimen's surface, which fill with water
during the immersion, only if the immersion time is long enough; for
the ten-second reading recorded here, the surface-connected pores
are still mostly air-filled, and the reading approximates the
\engterm{bulk} (or \engterm{envelope}) volume that an outline-tracing
method would also report.

\engterm{Gas pycnometry} uses a Micromeritics AccuPyc II 1340 helium
pycnometer (list price approximately $30{,}000 \,\$ $ as of 2024)
to read the volume the specimen occupies that helium cannot enter
\cite{ch07t3b-web:micromeritics-accupyc-1340}. The instrument
seats the specimen in a calibrated reference cell, equilibrates
helium at a controlled fill pressure (typically near
$1.2 \,\text{atm}$), and uses the Boyle-Mariotte gas law to compute
the displaced volume from the pressure change when a second
calibrated expansion cell is opened. Helium permeates open pores
down to several Angstroms; the pycnometer reading is the
\engterm{skeletal} (or \engterm{true}) volume excluding open
porosity. The reading on the alumina specimen is $50.1 \,\si{
\centi\meter\cubed}$, with a standard uncertainty of about
$0.01 \,\si{\centi\meter\cubed}$ from the manufacturer's calibration
certificate. The protocol is laid down in ASTM B311
\cite{ch07t3b-std:astm-b311-2022} for powder-metallurgy materials,
the working standard the pycnometer's calibration ties to.

\engterm{Industrial computed tomography} uses a Zeiss METROTOM 6
(a CT-CMM hybrid; list price approximately $1{,}000{,}000 \,\$ $
as of 2023) at $4 \,\si{\micro\meter}$ voxel resolution
\cite{ch07t3b-web:zeiss-metrotom-6}. The X-ray attenuation
reconstruction segments the specimen from the surrounding air at a
material-density threshold; the segmented voxel sum, multiplied by
the calibrated voxel volume, reports the specimen's
\engterm{closed-pore-aware} volume, which counts the closed
internal pores as part of the specimen volume but excludes the
surface-connected open pores. The reading is $50.05 \,\si{
\centi\meter\cubed}$ with a standard uncertainty of about
$0.05 \,\si{\centi\meter\cubed}$ from the volumetric-accuracy
specification of $\pm 0.1\,\%$ per VDI/VDE 2630 Part 1.3
\cite{ch07t3b-std:vdi-vde-2630-1-3} applied to the calibration
artefact.

The three numbers ($50.3$, $50.1$, $50.05 \,\si{\centi\meter
\cubed}$) disagree by amounts well outside their individual
standard uncertainties, and the disagreement pattern is the
discriminator. The water-displacement reading is the largest because
the bulk-envelope volume includes both open and closed porosity
plus the thin water film on the outer surface; the pycnometer
reading is intermediate because helium fills the open pores and
excludes their volume from the skeletal reading; the CT reading is
the lowest because the segmentation counts the closed internal
pores as solid (they cannot be distinguished from solid material at
the chosen threshold without sub-voxel reconstruction). The
combined picture lets the engineer extract the porosity structure:
the open-pore fraction is
$(V_{\mathrm{disp}} - V_{\mathrm{pyc}})/V_{\mathrm{disp}}
\approx 0.4\,\%$, the closed-pore fraction is
$(V_{\mathrm{pyc}} - V_{\mathrm{CT}})/V_{\mathrm{pyc}}
\approx 0.1\,\%$, and the bulk porosity totals about $0.5\,\%$,
within the expected range for the sinter cycle. The lesson runs
parallel to the cross-checking discipline of section 7.5: the
disagreement is the information; the convergence on a single
number would have hidden the porosity structure entirely.

\input{volumes/01-quantity/ch07-length-area-volume-mass/figures/fig-porosity-volume-methods.tex}

\Cref{fig:vol01:ch07:porosity-volume-methods} plots the three
readings on a common axis zoomed to the disagreement, with the two
gaps annotated as the open-pore and closed-pore fractions. The
figure makes the ordering visible at a glance: bulk above skeletal
above closed-pore-aware, each step down excluding one more class of
void from the reported volume. A reader who reaches for a single
volume method and takes its number as the volume of the specimen has
chosen, without stating it, which porosity class the number counts
as solid.

\paragraph{Cast-turbine-blade internal-cavity volume by CT.}
A working application of the Zeiss METROTOM 6 that has no
non-destructive alternative is the internal-cavity volume of a
hollow investment-cast turbine blade. The blade is a vacuum-cast
nickel-superalloy aerofoil with a serpentine internal cooling
passage whose dimensions affect blade thermal performance and
service life. The cavity volume specification is
$2.50 \pm 0.04 \,\si{\centi\meter\cubed}$ (a $1.6\,\%$ tolerance);
the casting-process variation that the foundry wants to bound is
about half that.

The CT scan acquires roughly $2000$ projections at $225 \,\si
{\kilo\volt}$ tube voltage, reconstructs to a $4 \,\si{\micro
\meter}$ voxel grid (about $9 \times 10^{9}$ voxels in the scan
volume), and segments the internal void from the surrounding nickel
alloy at the air-metal density boundary. The cavity-volume readout
is $2.487 \,\si{\centi\meter\cubed}$ with a combined standard
uncertainty of about $0.005 \,\si{\centi\meter\cubed}$ ($0.2\,\%$).
The blade passes the specification with margin, and the foundry
plots the cavity volume across a $200$-blade production lot to
monitor casting drift. Two destructive alternatives exist (cut the
blade open and measure the cavity directly, or fill the cavity with
a low-viscosity wax and weigh the wax), but both destroy the part;
the CT scan reports the cavity volume on a deliverable blade and
adds the cavity-volume record to the part's quality file. The
selection step from chapter 5 plays out cleanly: state the
measurand (internal-cavity volume on a deliverable blade), identify
the candidate principles (destructive section, wax-fill destructive
mass, CT), compare specifications against tolerance (only CT is
non-destructive, and its $0.2\,\%$ uncertainty meets the $1.6\,\%$
tolerance with margin), document the choice. The
\repopath{volumes/01-quantity/ch07-length-area-volume-mass/code/ct_cavity_volume.py}
script reads a representative voxel-occupancy file and reports the
integrated cavity volume by the same threshold-and-count step the
metrology software implements.

\paragraph{CT-CMM hybrid versus tactile CMM: when to use each.}
The Zeiss METROTOM 6 above and the
\engterm{Zeiss PRISMO Ultra} traditional bridge-type CMM (list
price approximately $400{,}000 \,\$ $ as of 2023)
\cite{ch07t3b-web:zeiss-prismo-ultra} are the two ends of an
engineering decision a metrology laboratory makes per workpiece. The
PRISMO Ultra carries a VAST gold tactile scanning probe with maximum
permissible error of $0.5 + L/500 \,\si{\micro\meter}$ at the
$k = 2$ expanded level under ISO 10360-2:2009
\cite{ch07t3b-std:iso-10360-2-2009}; that floor is roughly five
times tighter than the METROTOM 6's volumetric specification on
external geometry. A tactile CMM measures only what the probe can
physically reach: external surfaces, accessible internal cylinders,
and through-holes whose probe access is engineered into the part.
A CT-CMM hybrid measures every voxel in the scan volume, including
internal cavities and assembled subcomponents that the tactile
probe cannot reach. The selection rule is geometric: a part with no
internal features and tolerances tighter than about
$\pm 5 \,\si{\micro\meter}$ goes to the PRISMO Ultra; a part with
internal features, hidden assemblies, or tolerances looser than
about $\pm 20 \,\si{\micro\meter}$ goes to the METROTOM 6 when CT
access is justified; the overlap range gets both, and the two
measurement records are reconciled by the cross-checking discipline
of section 7.5.

The ISO 10360-2 acceptance test for either machine is the
discipline guard that makes the per-measurement specifications
defensible. The test procedure measures five calibrated reference
gauge blocks of different lengths at seven positions and three
orientations across the working volume, $35$ measurements per
length, for a total of $175$ measurements per acceptance run. The
working acceptance criterion is that every measured length agree
with its calibration value within the maximum permissible error
$A + L/K$ ($\si{\micro\meter}$, with $A$ and $K$ from the machine
specification). A working tool-room runs the acceptance test
annually as the formal recalibration and after any service event
that touches the linear scales, the bridge, or the probe
electronics. The test certificate, signed by an accredited
calibration laboratory under ISO/IEC 17025
\cite{ch07t3-std:iso-iec-17025-2017}, carries the per-axis
deviations the CMM exhibits across its working volume.

The temperature-compensated stage is the working precondition for
the specification above to hold. The METROTOM 6 specification of
$\pm 0.1\,\%$ volumetric accuracy assumes the stage is held to
$\pm 0.01 \,\si{\degreeCelsius}$ stability over the scan time
(typically thirty minutes to several hours per part), enforced by
a closed-loop thermal control with a calibrated platinum-resistance
thermometer on the stage and a chiller feeding a circulating-fluid
jacket. A laboratory at $\pm 0.5 \,\si{\degreeCelsius}$ stability,
typical for a clean machine-shop quality room rather than a CT-CMM
metrology suite, would inflate the per-voxel positional uncertainty
on a $300 \,\si{\milli\meter}$ part by about
$300 \times 1.1 \times 10^{-5} \times 0.5 \approx 1.65 \,\si{\micro
\meter}$, comparable to half a voxel and twenty percent of the
working uncertainty budget. The thermal-control discipline matches
the chapter's standing rule: the dimensional reference temperature
is $20 \,\si{\degreeCelsius}$ per ISO 1 \cite{std:iso-1-2022}, and
the stage stability is the practical means by which that reference
is realised at the working scan.
...
\paragraph{Worked example: density-based separation of two aluminium
alloys, and where it fails.}
A scrap-sorting line must separate two aluminium alloys by density
alone: wrought 6061 at a handbook density of
$2700 \,\si{\kilogram\per\meter\cubed}$ and cast A356 at
$2680 \,\si{\kilogram\per\meter\cubed}$
\cite{text:ashby2017}. The two differ by
$20 \,\si{\kilogram\per\meter\cubed}$, or $0.74\,\%$. The question
is whether a density measurement can resolve the two, and the answer
runs entirely through the uncertainty budget the measurement can
achieve.

The separation is resolvable only if the density measurement's
combined standard uncertainty is small enough that the two
populations do not overlap at the decision threshold. Treating the
two handbook values as the population means and demanding a
separation of at least $r = 3$ combined standard uncertainties
between them, the measurement must reach
\[
u(\rho) \leq \frac{|\rho_{6061} - \rho_{\mathrm{A356}}|}{3}
= \frac{20}{3} \approx 6.7 \,\si{\kilogram\per\meter\cubed},
\]
or $u(\rho)/\rho \leq 6.7 / 2690 \approx 0.25\,\%$ relative. That
target forecloses the household methods at once. A kitchen-scale mass
at $1\,\%$ and a graduated-cylinder volume at $1\,\%$ combine in
quadrature to about $1.4\,\%$ relative density uncertainty, five to
six times too coarse; the two alloy populations would overlap so
badly that a density reading of $2690 \,\si{\kilogram\per\meter
\cubed}$ could belong to either. The target is reached only by an
analytical-balance mass ($\sim 0.01\,\%$) and a hydrostatic-weighing
volume ($\sim 0.1$ to $0.3\,\%$), combining to about $0.1$ to
$0.3\,\%$, at or just inside the $0.25\,\%$ requirement. The
engineering conclusion is a decision, not a number: density-based
sorting of these two alloys is defensible only with laboratory-grade
instruments, and a production line that needs the separation on
every part reaches instead for X-ray fluorescence or optical-emission
spectroscopy, which read composition directly and do not ride on a
$0.74\,\%$ density gap. The density discriminator that cleanly
separates aluminium from steel, a $190\,\%$ gap, cannot separate two
aluminium alloys across a sub-percent gap; the same physical quantity
is a decisive tool at one scale of difference and a useless one at
another.

\begin{mastery}
\textbf{Hydrostatic weighing as a primary volume and density
method.} The household displacement methods read volume from a
graduated cylinder; the laboratory alternative reads it from two mass
measurements and Archimedes' principle, reaching an uncertainty the
graduation cannot. A sample is weighed in air, giving apparent mass
$m_{\mathrm{air}}$, then weighed while fully submerged from a fine
wire in a liquid of known density $\rho_{L}$, giving a smaller
apparent mass $m_{\mathrm{liq}}$. The buoyant force equals the weight
of the displaced liquid, so the sample volume is
\[
V = \frac{m_{\mathrm{air}} - m_{\mathrm{liq}}}{\rho_{L}},
\]
and the sample density follows directly as
\[
\rho_{s} = \frac{m_{\mathrm{air}}}{m_{\mathrm{air}} -
m_{\mathrm{liq}}}\,\rho_{L},
\]
in which the two masses enter only through their difference and their
ratio, and the volume never has to be measured geometrically. The
method's uncertainty is dominated by the liquid density $\rho_{L}$
and by the surface-tension pull on the suspension wire at the liquid
surface, not by a graduation. Water's density is known to
$\sim 10^{-5}$ relative as a function of temperature, so a
temperature-controlled water bath and an analytical balance deliver
$u(\rho_{s})/\rho_{s}$ of order $0.1\,\%$, an order of magnitude
tighter than a graduated cylinder. The wire correction and the
degassing of surface bubbles are the two disciplines that separate a
defensible hydrostatic measurement from a careless one; both are
codified in the density-determination-kit procedures that ship with
laboratory balances, and both trace to the same wetting and
bubble-adhesion assumptions the displacement method carries in the
volume section. The method is the working realisation of the
$0.25\,\%$ target the alloy-sorting example above demands, and it is
the natural third method for the chapter's project when a reader has
access to a fine-resolution balance.
\end{mastery}
...
\paragraph{BET nitrogen adsorption for porous solids.}
The dominant working method for porous-solid specific surface area
is the \engterm{Brunauer-Emmett-Teller} (BET) nitrogen-adsorption
technique, named for the 1938 derivation that generalised the
single-layer Langmuir isotherm to multilayer adsorption
\cite{ch07t3b-paper:brunauer-emmett-teller-1938}. The specimen is
outgassed under vacuum at elevated temperature (typically
$200 \,\si{\degreeCelsius}$ to $300 \,\si{\degreeCelsius}$ for hours
to clear adsorbed water and contaminants), cooled to liquid-nitrogen
temperature ($77 \,\si{\kelvin}$), and dosed with nitrogen gas at
controlled relative pressure $p/p_{0}$ between roughly $0.05$ and
$0.30$. The instrument records the volume of nitrogen adsorbed at
each pressure step. Fitting the BET isotherm to the adsorbed-volume
vs $p/p_{0}$ data extracts the monolayer capacity $V_{m}$; the
specific surface area follows from $V_{m}$ and the cross-sectional
area of a nitrogen molecule ($0.162 \,\si{\nano\meter\squared}$ by
convention):
\[
A_{\mathrm{BET}} = \frac{V_{m} N_{A} \sigma_{N_{2}}}{V_{\mathrm{molar}}},
\]
with $N_{A}$ Avogadro's number and $V_{\mathrm{molar}}$ the molar
volume of nitrogen at STP. The method standards are ISO 9277:2010
\cite{ch07t3b-std:iso-9277-2010} for general solids and
ASTM C1274-12(2018) \cite{ch07t3b-std:astm-c1274-2018} for advanced
ceramics; both fix the outgassing protocol, the pressure-range
window for the BET fit, and the reporting requirements.

The working laboratory instrument is the
\engterm{Micromeritics TriStar II Plus} (list price approximately
$40{,}000 \,\$ $ as of 2024)
\cite{ch07t3b-web:micromeritics-tristar-ii}, a three-station
adsorption analyser that measures specific surface areas from
$0.01$ to $3000 \,\si{\meter\squared\per\gram}$, covering most
porous materials an engineer is likely to meet from
non-porous metal powders at the low end to high-surface-area
catalysts at the high end. The accuracy is set by the dosing-volume
calibration and the pressure-transducer linearity; a working
typical relative standard uncertainty on the BET surface area is
$2\,\%$ to $5\,\%$, dominated by the outgassing thoroughness and
the specimen mass measurement.

\paragraph{Activated carbon: a thousand-fold surface-area lift.}
The BET method's working application is the surface-area
characterisation of activated carbon, the canonical adsorbent. Raw
charcoal has a BET specific surface area of $1$ to $3 \,\si{
\meter\squared\per\gram}$, scaling with the macroscopic surface area
of the powder grains. Steam-activated or chemically activated
carbon, after thermal treatment in the $800 \,\si{\degreeCelsius}$
to $1000 \,\si{\degreeCelsius}$ range that opens a network of
micropores of order $1$ to $5 \,\si{\nano\meter}$ diameter, reaches
$800$ to $1500 \,\si{\meter\squared\per\gram}$. The thousand-fold
surface-area lift is what makes activated carbon a working
adsorbent for water-treatment, gas-mask, and pharmaceutical
applications. The same specimen mass of one gram presents an
adsorbing surface area equivalent to roughly a tenth of a football
pitch.

A worked sanity check: a $10 \,\si{\milli\gram}$ activated-carbon
sample with BET area $1200 \,\si{\meter\squared\per\gram}$ presents
$12 \,\si{\meter\squared}$ of internal surface, on which a nitrogen
monolayer of about $1.4 \times 10^{-9} \,\si{\mole\per\meter
\squared}$ amounts to $1.7 \times 10^{-8} \,\si{\mole}$ of nitrogen,
or about $0.4 \,\si{\milli\liter}$ at STP. The dosing apparatus must
deliver and read that millilitre-scale gas volume with relative
uncertainty matched to the $2\,\%$ to $5\,\%$ target. The
discipline parallels the volume measurements above: the technique
imposes its own minimum-quantity requirement, and a measurement
below that threshold has no defensible uncertainty budget. The
working pre-check is the same one a balance laboratory runs (the
minimum sample mass for the chosen tolerance) recast in adsorbed
gas volume.
...
\subsection{The micro-balance and the minimum-sample-mass rule}

A working analytical balance at the chapter's lower limit reaches
mass scales where the calibration chain above ceases to be a textbook
abstraction. The
\engterm{Mettler Toledo XPR2U} ultra-micro-balance (list price
approximately $35{,}000 \,\$ $ as of 2024) reports
$0.1 \,\si{\micro\gram}$ readability at a $2.1 \,\si{\gram}$ capacity,
with a manufacturer's repeatability standard deviation of
$0.15 \,\si{\micro\gram}$ at typical load
\cite{ch07t3b-web:mettler-xpr2u-datasheet}. The instrument occupies a
draught-shield enclosure, sits on an isolated stone slab, and reads
across a span of seven orders of magnitude from readability to
capacity. The cell uses electromagnetic force-restoration against a
permanent-magnet flux; the servo current is the working mass
indication.

The instrument's smallest defensible measurement is set by the
\engterm{minimum-sample-mass} discipline of USP General Chapter
$\langle$41$\rangle$ \cite{ch07t3b-std:usp-41-2024}: the minimum mass
at which the balance achieves a chosen relative repeatability target.
For the $0.1\,\%$ tolerance of compendial pharmaceutical work, the
minimum-sample mass is roughly $1000$ times the repeatability
standard deviation (three sigma divided by the target). The XPR2U
at $0.15 \,\si{\micro\gram}$ repeatability has a minimum-sample mass
of about $0.15 \,\si{\milli\gram}$ at the $0.1\,\%$ tolerance, or
about $15 \,\si{\micro\gram}$ at the slacker $1\,\%$ tolerance of a
research-laboratory measurement. Below the minimum-sample mass, the
displayed reading carries no defensible uncertainty budget against
the chosen tolerance; the discipline is to weigh on a smaller-range
instrument (which the XPR2U is not), to scale up the sample, or to
report against the slacker tolerance the actual mass supports. The
USP rule is the formal version of the working laboratory heuristic
that the balance readability should be at most about $0.1\,\%$ of
the measured value; the rule is enforced by audit in any GMP
pharmaceutical environment.

A working failure mode at the micro-balance scale is the static
charge on a polymer sample. A dry polypropylene weigh-boat brushed
against a lab-coat sleeve picks up surface charge of order
$10^{-8} \,\si{\coulomb}$. Placed on the XPR2U pan that charge
induces an image charge in the metal pan; the resulting Coulomb
force at a separation of a few millimetres produces an apparent
mass shift of about $10 \,\si{\milli\gram}$, ten times the
$1 \,\si{\milli\gram}$ analytical-class budget and a hundred
thousand times the XPR2U readability. The reading drifts over thirty
seconds as the charge equilibrates with the room (humidity-driven
discharge through the air-borne ion population) or as the pan
discharges through its grounded chassis. The working mitigation is
a piezo-driven static-eliminator electrode that injects a balanced
ion stream into the weighing chamber; the Mettler Toledo
\engterm{StaticDetect} accessory does this automatically and reports
the residual charge as a per-measurement diagnostic. Without the
static-eliminating discipline, the analytical-balance measurement
at the micro-balance scale is not reproducible at the instrument's
nominal class; with it, the measurement is reproducible to within
the manufacturer's repeatability specification.

\subsection{Mass flow and totalising: the Coriolis meter}

A separate working measurement chain reports not the mass on a
balance pan but the \engterm{mass flow rate} of a fluid through a
pipe, and the \engterm{integrated mass} (the totaliser output) the
flow delivers over a stated interval. The dominant working
instrument is the \engterm{Coriolis flowmeter}, named for the
fictitious force the fluid experiences as it traverses an
oscillating curved tube. Fluid moving through a tube vibrating at
its mechanical resonance experiences a Coriolis acceleration that
twists the tube by an angle proportional to the mass flow rate
through the tube. Reading the twist angle (typically by optical or
inductive pickoffs at two points along the tube) gives the mass
flow rate directly, independent of the fluid's composition, density,
or viscosity. The
\engterm{Emerson Micro Motion CMF200} (list price approximately
$15{,}000 \,\$ $ as of 2023)
\cite{ch07t3b-web:emerson-cmf200} is the working benchmark: liquid
mass-flow accuracy of $\pm 0.05\,\%$ of rate down to a turn-down
ratio of about $20{:}1$ on the nominal range, with the additional
output of fluid density to $\pm 0.0005 \,\si{\gram\per\centi\meter
\cubed}$ from the tube's resonance frequency shift.

The Coriolis meter's totaliser output is the time integral of mass
flow rate over a stated interval, and the totaliser's calibration
is by mass-balance reference. The standard protocol fills a
calibrated reference tank from the flowmeter under known conditions,
weighs the delivered mass on a working reference scale traceable to
the chapter's primary realisation, and compares the totaliser
indication against the weighed mass. The mass-flow versus
mass-totalising distinction matters because the two outputs have
different uncertainty propagation: the per-reading mass-flow
uncertainty is the instantaneous accuracy specification
($\pm 0.05\,\%$ of rate), while the totaliser uncertainty accumulates
the integration-window contribution (the clock stability of the
sampling, typically negligible at $10^{-6}$ over an hour) and the
zero-flow drift contribution (the residual indication when the
tube is full but the fluid is static, typically a few grams per hour
on a $50 \,\si{\milli\meter}$ tube). A working custody-transfer
application that bills product by integrated mass over a delivery
window of an hour reads the totaliser at the start and the end, and
reports the difference with the combined uncertainty from the
$\pm 0.05\,\%$-of-rate per-reading floor and the zero-flow drift
ceiling. The two uncertainties bracket the achievable accuracy: a
high-rate delivery is dominated by the per-reading floor, a
low-rate-with-long-window delivery by the zero-flow drift. The
Coriolis-meter discipline tracks both and reports the dominant
...
self-fulfilling. By the 1875 Metre Convention (signed 20 May
1875 in Paris by representatives of $17$ states) and the
1st CGPM in 1889 (the first General Conference on Weights and
Measures), the IPK was, by definition, the unit of mass; no
procedural mechanism existed to declare a definitional change in
the SI kilogram on the basis of a verification result. The path
out of the impasse required four decades of metrological
research into electrical analogues of mass, originating with
Bryan Kibble's proposal at the National Physical Laboratory
(Teddington, UK) in $1975$ for a watt balance using moving-coil
weighing against an electrically-measured force. The instrument
class was later renamed the \engterm{Kibble balance} in 2016
following Kibble's death in 2016.

Two independent realisation routes were developed in parallel.
NIST (Gaithersburg, MD) built NIST-1 (1980-1985, mechanical
$10^{-6}$), NIST-2 (1996-2000, $10^{-7}$), NIST-3 (2002-2010,
$3 \times 10^{-8}$), and NIST-4 (2012-present, $1.3 \times
10^{-8}$). NPL built Watt Balance Mark I (1985-2005), Mark II
(2007-2011), then transferred the technology to the National
Research Council of Canada (NRC Ottawa) as part of NPL's withdrawal
from the kilogram-determination programme. METAS (Bern,
Switzerland), LNE-CNAM (Paris), and the BIPM operate additional
Kibble balances; the Korea Research Institute of Standards and
Science (KRISS) joined the programme in 2014.

The XRCD (X-Ray Crystal Density) route used isotopically
enriched silicon-$28$ ($> 99.99$\% by atom fraction, separated
by centrifugation at the Central Design Bureau of Machine
Building in Saint Petersburg) to determine Avogadro's number by
counting atoms in a polished single-crystal sphere. PTB
(Braunschweig, Germany) led the International Avogadro
Coordination (IAC), which delivered four high-purity spheres
(designated AVO28-S5, S8, S15, S33) characterised at NMIJ
(Tsukuba, Japan), INRIM (Turin, Italy), and PTB. The IAC's
final pre-redefinition $N_{A}$ value was $6.022\,140\,758\,
\times 10^{23}\,\si{\per\mol}$ at $1.2 \times 10^{-8}$ relative
uncertainty (Bartl et al., 2017).

The CGPM-recommended consensus value of the Planck constant at
the time of the redefinition was $h = 6.626\,070\,15 \times
10^{-34}\,\si{\joule\second}$, derived from a CODATA
least-squares adjustment combining the Kibble-balance and XRCD
results: NIST-4 contributed $h = 6.626\,069\,83(60) \times
10^{-34}\,\si{\joule\second}$ ($9.1 \times 10^{-8}$ relative
uncertainty), NPL/NRC contributed $h = 6.626\,070\,113(70) \times
10^{-34}\,\si{\joule\second}$ ($1.1 \times 10^{-8}$), IAC-XRCD
contributed $h = 6.626\,070\,151(73) \times 10^{-34}\,\si{
\joule\second}$ ($1.1 \times 10^{-8}$, derived from $N_{A}$
through Planck-Avogadro consistency). The CGPM Resolution 1
(2018), adopted at the $26$th CGPM in Versailles on 16 November
2018, fixed the Planck constant and redefined the kilogram,
ampere, kelvin, and mole accordingly; the change took effect on
20 May 2019 (World Metrology Day, the anniversary of the
Convention) \cite{paper:stock-kibble-2018,std:bipm-si2019,%
web:bipm-mise-en-pratique-kilogram-2019}.
...
\input{volumes/01-quantity/ch07-length-area-volume-mass/figures/fig-buoyancy-correction.tex}

\Cref{fig:vol01:ch07:buoyancy-correction} shows why the correction
depends on both densities rather than on the sample alone. A balance
weighs the sample against reference weights of a different density,
and each body loses apparent weight by the weight of the air it
displaces. The in-vacuum mass follows from the in-air indication as
$m = m_{\mathrm{ind}}\,[1 + \rho_{a}(1/\rho_{s} - 1/\rho_{r})]$, with
$\rho_{a}$ the air density, $\rho_{s}$ the sample density, and
$\rho_{r}$ the reference-weight density.

\paragraph{Worked example: buoyancy correction on a foam and on a
steel block.}
A working analytical balance is calibrated against steel reference
weights of density $\rho_{r} = 8000 \,\si{\kilogram\per\meter\cubed}$
in laboratory air of density $\rho_{a} = 1.20 \,\si{\kilogram\per
\meter\cubed}$ (near $20 \,\si{\degreeCelsius}$, $101.3 \,\text{kPa}$,
$50\,\%$ relative humidity). Two samples of identical indicated mass
$m_{\mathrm{ind}} = 50.000 \,\si{\gram}$ are weighed: a
polystyrene-foam block of density $\rho_{s} = 30 \,\si{\kilogram
\per\meter\cubed}$ and a steel offcut of density $\rho_{s} = 7850 \,
\si{\kilogram\per\meter\cubed}$.

The foam correction runs first. The bracketed factor is
$1 + 1.20\,(1/30 - 1/8000) = 1 + 1.20 \times (0.03333 - 0.000125)
= 1 + 1.20 \times 0.03321 = 1 + 0.03985$. The true mass is
$50.000 \times 1.03985 = 51.99 \,\si{\gram}$, a correction of nearly
$2 \,\si{\gram}$, or $4\,\%$. The foam displaces
$50 / 30 \times 10^{-3} \approx 1.67 \,\si{\liter}$ of air, which at
$1.20 \,\si{\gram\per\liter}$ is $2.0 \,\si{\gram}$ of buoyant lift,
and the balance reads the foam that much light. A reader who reports
the raw $50.000 \,\si{\gram}$ has made a $4\,\%$ mass error on a
balance whose readability is $0.1 \,\si{\milli\gram}$, five orders
of magnitude finer than the error.

The steel correction runs second, on the same indicated mass. The
bracketed factor is $1 + 1.20\,(1/7850 - 1/8000) = 1 + 1.20 \times
(1.2739 \times 10^{-4} - 1.25 \times 10^{-4}) = 1 + 1.20 \times
2.39 \times 10^{-6} = 1 + 2.9 \times 10^{-6}$. The true mass is
$50.000 \times 1.0000029 = 50.00014 \,\si{\gram}$, a correction of
$0.14 \,\si{\milli\gram}$, comparable to the balance readability and
negligible for most steel work. The lesson reads off the two cases:
the correction is set by the difference of the reciprocal densities,
so it collapses toward zero when the sample density approaches the
reference density and grows without bound as the sample density
falls. The convention that fixes $\rho_{r}$, the reference air
density, and the correction algebra is OIML R 111 for weights of the
E and F accuracy classes, the working standard behind any
buoyancy-corrected analytical measurement.
...
\engterm{Viscosity} is a fluid's resistance to shear deformation.
The SI unit of dynamic viscosity is the pascal second, $\si{
\pascal\second}$; the older unit, the poise, equals $0.1\,\si{
\pascal\second}$. Working benchmark values: water at $20\,\si{
\degreeCelsius}$ at $1.002\,\si{\milli\pascal\second}$; air at
$20\,\si{\degreeCelsius}$ at $1.81 \times 10^{-5}\,\si{\pascal
\second}$; honey at $20\,\si{\degreeCelsius}$ at $\sim 10\,\si{
\pascal\second}$ (varying with floral source and water content);
SAE 10W-30 motor oil at $100\,\si{\degreeCelsius}$ at $\sim
0.011\,\si{\pascal\second}$ ($11\,\text{cSt}$ kinematic);
asphalt binder at $135\,\si{\degreeCelsius}$ at $\sim 0.5\,\si{
\pascal\second}$ (the Superpave PG specification's working
condition). Any quoted value must carry temperature, grade, and
source.

The working instruments measure viscosity by the response of a
probe to fluid drag. \engterm{Rotational viscometers} drive a
spindle in the fluid at known angular velocity and measure the
torque required to maintain the velocity; the ratio of torque to
velocity, divided by a geometric factor, yields the viscosity.
Working instruments: Brookfield DV-II+ Pro (the workhorse
\$$3000$ industrial unit, $\pm 1\%$ of full scale, working from
$1\,\si{\milli\pascal\second}$ to $6 \times 10^{6}\,\si{\milli
\pascal\second}$ depending on spindle); Anton Paar RheolabQC
(\$$15{,}000$ research-grade, $\pm 0.5\%$); Anton Paar MCR 502
WESP rheometer (\$$50{,}000$-$80{,}000$, the workhorse research
rheometer with cone-and-plate, parallel-plate, concentric-cylinder
geometries and oscillatory and rotational modes). The
cone-and-plate geometry (typical cone angle $1\,\si{\degree}$ to
$4\,\si{\degree}$, diameter $25$-$60\,\si{\milli\meter}$) produces
uniform shear rate across the sample, the working choice for
non-Newtonian rheology characterisation.

\engterm{Falling-ball viscometers} (Haake Falling Ball
Viscometer, $\$5000$, working range $0.6$-$7 \times 10^{4}\,\si{
\milli\pascal\second}$) drop a calibrated sphere through the
fluid in a vertical tube tilted $10\,\si{\degree}$ from
horizontal; the rolling-ball variant reduces wall effects and
extends the working range. Terminal velocity, after the ball has
reached steady state, gives the viscosity through Stokes's law
for laminar flow: $\eta = (2/9)(\rho_{b} - \rho_{f})g r^{2}/v$ for
the falling-ball case at low Reynolds.

\engterm{Capillary viscometers} (Cannon-Fenske routine viscometer
sizes 25-500, $\$200$ each, NIST-traceable; SI Analytics
ViscoSystem AVS 470 automated capillary tower at $\$25{,}000$
for laboratory production work) drain a known volume of fluid
through a narrow tube under gravity. The flow time gives the
kinematic viscosity through the Hagen-Poiseuille relation:
$\nu = K t$, where $K$ is the viscometer constant calibrated at
$25\,\si{\degreeCelsius}$ traceable to NIST SRM 2300 (general-
purpose viscosity reference standards, polyalphaolefin and
polyisobutylene blends). The IP 71 / ASTM D445 standard
specifies the procedure for petroleum-product kinematic
viscosity determination, the working reference for oil-industry
quality control.

\engterm{Vibrating-element viscometers} (SVM 3001 Stabinger
viscometer at $\$25{,}000$, working over $-60$ to $+135\,\si{
\degreeCelsius}$; A\&D SV-10 tuning-fork viscometer at $\$8000$
for $0.3$-$10{,}000\,\si{\milli\pascal\second}$) measure
viscosity through the resonant response of a vibrating element
immersed in the fluid; the damping shift gives the viscosity.
Faster than rotational for routine measurements but with
...
\paragraph{Nanoindentation: hardness at the nanometre scale.}
The microhardness load family extends downward through
\engterm{ultra-microhardness} ($0.001$ to $0.1\,\text{kgf}$,
indent depths $\sim 1$-$10\,\si{\micro\meter}$) into
\engterm{nanoindentation} (loads $0.1\,\si{\micro\newton}$ to
$500\,\si{\milli\newton}$, indent depths $\sim 1\,\si{\nano\meter}$
to $10\,\si{\micro\meter}$). At nanometre depths, the
optical-reticle measurement of indent size becomes impossible
($d$ falls below the optical diffraction limit); the working
method shifts to \engterm{instrumented indentation} per ISO
14577-1:2015, which measures load and displacement continuously
during indentation and unloading, extracting hardness and
elastic modulus from the unloading slope (Oliver-Pharr method).

The working nanoindentation instruments. \engterm{Hysitron} (now
part of Bruker since 2017) TI 980 TriboIndenter at \$$300{,}000$
delivers load from $1\,\si{\micro\newton}$ to $200\,\si{\milli
\newton}$ with $1\,\si{\nano\newton}$ resolution and depth
measurement to $0.04\,\si{\nano\meter}$ resolution through a
capacitive transducer; the working application is thin-film
hardness on coatings $5$-$500\,\si{\nano\meter}$ thick (DLC, TiN,
hardened-steel surface treatments). \engterm{Anton Paar} UNHT$^3$
(Ultra Nanoindentation Tester) at \$$200{,}000$ offers
$0.1\,\si{\micro\newton}$ load resolution at $0.003\,\si{\nano
\meter}$ depth resolution, the working choice for soft-matter
nanoindentation (biological tissues, polymer films, gels).
\engterm{Micro Materials} NanoTest Vantage at \$$150{,}000$
combines nanoindentation with the working temperature range of
$-100$ to $+1000\,\si{\degreeCelsius}$, the choice for
high-temperature thin-film characterisation.

The Oliver-Pharr extraction. The load-displacement curve of an
instrumented indentation has three regions: loading
(elastic-plastic, monotonically increasing), hold at peak load
(creep and load-control transients), and unloading (initially
elastic). The unloading-curve initial slope $S = \dd P/\dd h$
gives the contact stiffness; with the indenter's elastic
properties known (diamond Berkovich, $E = 1140\,\si{\giga\pascal}$,
$\nu = 0.07$), the working reduced modulus is
$E_{r} = (\sqrt{\pi}/2) S/\sqrt{A_{c}}$, where $A_{c}$ is the
projected contact area at peak load (a function of contact depth
through the indenter's calibrated area function). The sample
modulus $E_{s}$ extracts as $1/E_{r} = (1 - \nu_{s}^{2})/E_{s} +
(1 - \nu_{i}^{2})/E_{i}$. The Berkovich-indenter geometry (a
three-sided pyramid with face angle $65.27\,\si{\degree}$ from
vertical) is the working choice because its self-similarity
makes the contact-area function depth-independent at large
loads; the cube-corner geometry (face angle $35.26\,\si{\degree}$)
is used when sharper plastic deformation is wanted (fracture-
toughness measurements, scratch testing).

\engterm{Pile-up and sink-in} at the indent edges modify the
contact area from its ideal geometric value. The Hay-Pharr
correction factor accounts for the deviation: for elastic-plastic
materials with substantial work-hardening capacity, pile-up
underestimates the contact area by $5$-$30\%$, leading to an
overestimate of hardness and modulus. The Anton Paar UNHT
software's "scanning probe imaging" mode images the residual
indent immediately after unloading at $\sim 1\,\si{\nano\meter}$
lateral resolution, allowing the contact area to be measured
directly when the Hay-Pharr correction is uncertain. The
discipline applies the chapter's compatibility-ratio test: an
Oliver-Pharr hardness that disagrees with an SPM-measured
hardness by more than the combined uncertainty triggers
investigation of the pile-up correction.

The hardness tier across the load range. HV10 ($10\,\text{kgf}$,
depth $\sim 100\,\si{\micro\meter}$): material-class
characterisation, weld certification, casting QC. HV1 (depth
$\sim 30\,\si{\micro\meter}$): microhardness profile on
case-hardened parts, the working choice for CHD per ISO 2639.
HV0.1 (depth $\sim 10\,\si{\micro\meter}$): fine microhardness
profile, single-grain or single-phase hardness in polycrystalline
alloys. HV0.01 to HV0.001 (depths $\sim 3\,\si{\micro\meter}$ to
$1\,\si{\micro\meter}$): ultra-microhardness, thin-film
characterisation. Nanoindentation (loads below $10\,\si{\milli
\newton}$, depths below $1\,\si{\micro\meter}$): research-grade
characterisation of nanostructured materials, individual lamellae
in pearlite, individual grains below $100\,\si{\nano\meter}$
size, biological-tissue micromechanics. Each step down in load
multiplies the working uncertainty (from $\sim 1\%$ at HV10 to
$\sim 5\%$ at nanoindentation), requires instrumented
indentation with displacement measurement rather than optical
diagonal measurement, and trades depth of probed material for
spatial resolution.
...
intended geometric features. The working parameters per
ISO 21920-2:2021 include the arithmetic mean roughness $R_{a}$
(the mean absolute deviation from the mean line over the
evaluation length), the root-mean-square roughness $R_{q}$ (the
quadratic-mean form, related to $R_{a}$ by $R_{q} = R_{a}
\sqrt{\pi/2}$ for a Gaussian-distributed surface), the maximum
peak-to-valley height $R_{z}$ (the average over the evaluation
length of the largest peak-to-valley height per sampling length),
$R_{p}$ and $R_{v}$ (mean peak height and mean valley depth), $R_{sk}$
(skewness, distinguishing bearing surfaces with valleys for
lubrication from sealing surfaces with peaks to be flattened),
and $R_{ku}$ (kurtosis, distinguishing platykurtic
plateau-like surfaces from leptokurtic spiky surfaces). Machined
and polished surfaces span seven orders of magnitude in $R_{a}$:
sand-cast surfaces at $\sim 25\,\si{\micro\meter}$ $R_{a}$; rough-
turned at $\sim 6\,\si{\micro\meter}$; finished-turned at
$\sim 1.6\,\si{\micro\meter}$; finely ground at $\sim 0.4\,\si{
\micro\meter}$; lapped at $\sim 0.05\,\si{\micro\meter}$;
optical-quality polished at $\sim 1\,\si{\nano\meter}$;
super-polished mirror at $\sim 0.05\,\si{\nano\meter}$ atomic-scale.

\engterm{Contact stylus profilometers} drag a diamond stylus
(typical tip radius $2\,\si{\micro\meter}$ for industrial work,
$0.4$ to $1\,\si{\micro\meter}$ for fine-finish work) across the
surface, recording vertical displacement against horizontal
position through an LVDT or inductive transducer. Working
instruments: Mitutoyo Surftest SJ-410 (\$$15{,}000$, vertical
resolution $0.6\,\si{\nano\meter}$, traverse $25\,\si{\milli
\meter}$); Taylor Hobson Form Talysurf PGI 1240 (\$$80{,}000$,
$0.4\,\si{\nano\meter}$ resolution, $240\,\si{\milli\meter}$
traverse for form and roughness combined); Bruker DektakXT
(\$$60{,}000$, $0.1\,\si{\nano\meter}$ resolution, the working
choice for stepped-substrate semiconductor metrology). The stylus
load is $0.7$ to $4\,\si{\milli\newton}$ to avoid damaging
soft surfaces; even at this low load, the stylus may plough
through soft polymer or biological surfaces, which restricts
contact stylus to hard surfaces.

\engterm{Optical profilometers} use interferometric or confocal
methods to measure surface height without contact. Coherence-
scanning interferometry (CSI, also called white-light
interferometry, WLI) modulates a broad-spectrum source through a
Mirau or Michelson interferometric objective; the fringe-envelope
peak position at each pixel gives the height. Working instruments:
Bruker Contour Elite I (\$$120{,}000$, $0.01\,\si{\nano\meter}$
vertical resolution, $\sim 1\,\si{\micro\meter}$ lateral
through a $5\times$ objective); Zygo NewView 9000 (\$$200{,}000$,
$0.001\,\si{\nano\meter}$ vertical, the working choice for
optical-surface metrology in semiconductor and aerospace);
Polytec TopMap Pro.Surf (\$$90{,}000$, the working choice for
large-area scanning of machined surfaces). Phase-shifting
interferometry (PSI) modulates phase rather than coherence and
delivers $\sim 0.01\,\si{\nano\meter}$ vertical resolution at
the cost of $\lambda/4$ ambiguity, working best on smooth
surfaces ($R_{q} < \lambda/8$). Confocal microscopy uses a
pinhole to reject out-of-focus light, scanning the focal plane in
$z$ to build a height map at $\sim 100\,\si{\nano\meter}$
vertical resolution.

\engterm{Atomic force microscopes (AFMs)} drag a sharp silicon
or silicon-nitride tip ($\sim 10\,\si{\nano\meter}$ tip radius)
across the surface in contact, tapping, or non-contact mode,
detecting tip deflection through a laser-and-quadrant-photodiode
optical lever. Working instruments: Bruker Dimension Icon
(\$$300{,}000$, the workhorse research AFM with $0.01\,\si{\nano
\meter}$ vertical and $0.1\,\si{\nano\meter}$ lateral); Asylum
Cypher S (\$$200{,}000$, $0.005\,\si{\nano\meter}$ vertical, the
working choice for high-resolution AFM); Park Systems NX20
(\$$250{,}000$, large-sample working choice with $200\,\si{
\milli\meter}$ wafer chuck). The working uncertainty is
dominated by tip convolution: features narrower than the tip
radius cannot be resolved laterally, but vertical accuracy
remains at the angstrom level.

\engterm{Selecting the right technique.} For surfaces with
$R_{a} > 100\,\si{\nano\meter}$ on hard materials, contact stylus
is the cheapest and most established option. For soft surfaces or
finer roughness ($R_{a} = 1$ to $100\,\si{\nano\meter}$), optical
profilometry is the working choice; CSI handles the largest
height range, PSI gives the highest vertical resolution on
smooth surfaces. For nanometre-scale or sub-nanometre roughness,
AFM is required. Cross-checking two techniques on the same
surface (a stylus profile plus an AFM scan on a representative
patch) is the chapter's compatibility-ratio discipline applied
at the surface-metrology scale.
...
profile is sampled (the \engterm{evaluation length}, typically $5
\times \lambda_{c}$ where $\lambda_{c}$ is the long-wavelength
cutoff) and to the Gaussian or robust filter used to separate
roughness from waviness and form. ISO 21920-3 specifies
$\lambda_{c}$ values of $0.08$, $0.25$, $0.8$, $2.5$, and $8\,
\si{\milli\meter}$ corresponding to working roughness ranges
($R_{a}$ up to $0.1$, $0.5$, $2$, $10$, and $80\,\si{\micro
\meter}$ respectively). Two laboratories measuring "the same
surface" report different $R_{a}$ values when their evaluation
lengths and filters differ; the discipline is to specify the
evaluation length, cutoff, filter type, and governing standard
alongside the roughness value
...
\subsection{The null-corrector test setup}

The forensic detail in the OSBI report
\cite{acc:nasa-hubble-optical-systems-1990} repays a closer reading.
The Hubble primary mirror was a $2.4 \,\si{\meter}$ hyperboloid
figured at Perkin-Elmer Corporation's optical-fabrication facility
in Danbury, Connecticut, between 1979 and 1981. The primary's
design conic constant was $\kappa = -1.002\,98$, the value that
combines with the secondary mirror's hyperboloid to produce a
diffraction-limited image at the focal plane. Measuring a $2.4 \,
\si{\meter}$ aspheric mirror to a fraction of a wavelength of
visible light required a \engterm{null corrector}, a custom
optical-test instrument that converts the aspheric wavefront from
the test surface into a spherical wavefront that a reference
interferometer can read against a sphere of known radius. The
mirror is then polished until the interferometer reads null, that
is, until the residual wavefront error against the corrector's
output sphere is below tolerance.

Perkin-Elmer built two null correctors for the Hubble primary. The
\engterm{reflective null corrector} (RNC) was the primary fabrication
test, a high-precision two-mirror instrument whose geometry was
assembled against a metering rod that defined the axial spacings of
its internal optics. The \engterm{refractive null corrector} (RfNC)
was a smaller-aperture refractive instrument built and used in
parallel as a cross-check. The two correctors disagreed at the
$0.4$-wave rms level on the figure of the test mirror during the
1981 fabrication-acceptance measurements
\cite{acc:nasa-hubble-optical-systems-1990,ch07t3-text:chaisson-hubble-wars-1994}.

The geometric heart of the failure sat at the metering rod inside
the RNC. The field lens, a refractive element whose axial position
sets the corrector's null-output figure, was located by reference to
a measurement target on a fine-machined metering rod. The rod had
two features near its end: a polished end cap, and a recessed
target feature inset by approximately $1.3 \,\si{\milli\meter}$.
The assembly drawing called for the lens to be set against the
target feature; the assembly process set it against the end cap.
The error in the metering-rod reading was $1.3 \,\si{\milli\meter}$
of axial misposition of the field lens, which propagated into the
RNC's output reference figure as a conic-constant error.

Perkin-Elmer's project optical lead at the time judged the RNC's
design pedigree higher than the RfNC's and ruled the RNC the
authoritative instrument. The OSBI report records that the RfNC's
measurement of the test surface was set aside under a documented
schedule pressure: the $1986$ Challenger accident and the
subsequent shuttle stand-down compressed the optical-fabrication
schedule against an already-tight 1985 launch target, and the
fabrication management treated the auxiliary-test disagreement as
an instrument-calibration discrepancy to be resolved later, not as
a measurement-process failure to be investigated before sign-off
\cite{acc:nasa-hubble-optical-systems-1990,ch07t3-text:smith-hubble-history-1989}.
The mirror was shipped to Lockheed (Sunnyvale, CA) in 1984, the
telescope was integrated, and the launch slipped to STS-31 on 24
April 1990 only because the Challenger accident grounded the shuttle
fleet from January 1986 through September 1988.

The post-flight measurement of the as-built mirror's conic
constant, derived from the on-orbit imaging by the Hubble
Independent Optical Review Panel, gave $\kappa_{\mathrm{as-built}} =
-1.013\,90$ against the design $\kappa_{\mathrm{design}} = -1.002\,98$,
a difference of $1.092 \times 10^{-2}$ on a dimensionless conic
constant. The corresponding figure error at the mirror's outer
edge was about $2 \,\si{\micro\meter}$ too flat against the design
hyperboloid, equivalent to $0.4$-wave rms wavefront error at the
$632.8 \,\si{\nano\meter}$ helium-neon test wavelength
\cite{acc:nasa-hubble-optical-systems-1990}. The geometric chain
from the $1.3 \,\si{\milli\meter}$ metering-rod misposition to the
$2 \,\si{\micro\meter}$ outer-edge figure error runs through the
RNC's $f$-number, the field lens's refractive power, and the
mirror's $f/2.3$ figure, with each stage's optical-design sensitivity
recorded in the corrector's optical prescription.
...
\subsection{On-orbit diagnosis and the COSTAR retrofit}

The on-orbit failure manifested at first light on 20 May 1990,
when the Wide Field/Planetary Camera (WF/PC) returned its first
imaging frames and the focus refused to converge. Within weeks
the imaging anomaly was characterised as spherical aberration, the
wavefront defect produced by a hyperboloid whose conic constant
differs from its design value. NASA convened the Hubble Space
Telescope Optical Systems Board of Investigation (OSBI), chaired
by Lew Allen Jr., on 2 July 1990; the board's report was completed
in November 1990 \cite{acc:nasa-hubble-optical-systems-1990}. The
board's diagnostic chain ran from on-orbit imaging through
phase-retrieval analysis of out-of-focus star images to a
quantitative reconstruction of the mirror's figure error, and from
there back to the metering-rod misposition in the RNC. The
phase-retrieval analysis, conducted at NASA Goddard, the Space
Telescope Science Institute (Baltimore), and JPL, was a length
measurement at the optical-wavelength scale derived from imaging
data that had not been designed for that purpose. The on-orbit
diagnosis succeeded because the on-orbit imaging was an independent
measurement of the mirror's figure, made on a different chain from
the RNC's pre-launch test.

The COSTAR retrofit, designed at Ball Aerospace (Boulder, CO) and
built between 1991 and 1993, was a corrective optic that intercepted
the light from the primary mirror before it reached the first
generation of axial instruments (the Faint Object Camera, the Faint
Object Spectrograph, and the Goddard High Resolution Spectrograph)
and applied the conjugate of the primary's figure error to recover
diffraction-limited imaging at those instruments' input ports. The
Wide Field/Planetary Camera 2, also installed on STS-61, carried
its own internal correction in its relay optics rather than
receiving COSTAR's light \cite{ch07t3-text:chaisson-hubble-wars-1994}.

The recovery's cost is recorded in NASA's lifecycle-cost summary
for the Hubble programme \cite{ch07t3-web:nasa-hubble-program-cost-2010}.
The COSTAR hardware itself cost approximately \$$48$ million
(1991--1993 dollars); the STS-61 servicing mission in December
1993, which delivered COSTAR and WF/PC2 and replaced the solar
arrays, cost approximately \$$629$ million; and the cumulative
servicing-mission programme through STS-125 (Servicing Mission 4,
May 2009) cost over \$$2$ billion across five missions. The full
Hubble lifecycle cost, including construction, launch, operations,
and servicing, is reported by NASA as approximately \$$10$ billion
through 2010 and is expected to exceed \$$15$ billion across the
full extended-mission period through the late 2020s
\cite{ch07t3-web:nasa-hubble-program-cost-2010}. The pre-launch
optical-fabrication contract with Perkin-Elmer was approximately
\$$70$ million in 1980 dollars, an order of magnitude smaller than
the COSTAR-and-STS-61 recovery, and roughly two orders of magnitude
smaller than the lifetime programme. A working figure for the
recovery cost of a single failed length-measurement chain is
therefore not the primary fabrication cost but the integrated cost
of the on-orbit diagnosis, the corrective hardware design and
build, the servicing operation, and the operational opportunity
cost during the three-and-a-half-year imaging delay.

Perkin-Elmer's quality-process response to the OSBI report
restructured the optical-fabrication line for subsequent precision
optics. The two changes the OSBI recommended, and that Perkin-Elmer
implemented, were the \engterm{dual-null-corrector confirmation}
requirement (independent verification of any null-corrector
fabrication test by a second null corrector built and assembled by
an independent team, with formal anomaly-resolution before any
fabrication sign-off can be granted) and the \engterm{Hubble
Independent Quality Assurance} (HIQA) review group, an internal
audit structure with sign-off authority on optical-test procedures
separate from the fabrication management chain
\cite{acc:nasa-hubble-optical-systems-1990,ch07t3-text:chaisson-hubble-wars-1994}.
The dual-null-corrector procedure became standard for subsequent
large-mirror programmes, including the James Webb Space Telescope
primary-mirror-segment qualification (Ball Aerospace and Tinsley,
2003--2011) and the LSST (now Vera Rubin Observatory) primary-
secondary mirror (Steward Mirror Lab, 2008--2015), each of which
documented independent-verification protocols traceable in
structure to the OSBI recommendations.

The post-flight institutional change is the operational form of
the principle below. A reference instrument that cannot be checked
against an independent reference is a single-point failure of the
measurement system, and the design of the measurement system
recognises that fact by building in the independent check before
the measurement is taken, not after the artefact is in orbit.
