...
first three apply Bernoulli plus continuity to the standard
configurations of aerodynamics and internal flow; the next group adds
the momentum theorem (jet impingement, the nozzle reaction force, the
sudden expansion) and the rotating-frame pressure balance (the forced
vortex); a third group works the superposition machinery harder (the
Rankine half-body, the thin-airfoil camber line as a vortex sheet, the
unsteady oscillating column, and the free-streamline contraction of a
jet); a fourth set carries the free-surface and suction-limit cases
(the sluice gate and the siphon) and a short angular-momentum vignette
on the lawn sprinkler; the section closes with three end-to-end studies,
a Venturi metering station, a water-hammer penstock, and a finite-wing
induced-drag analysis, in which the relations of the chapter are strung
together into a sized result. A reader who follows every step here can
apply the inviscid toolkit to any configuration that meets the five
Bernoulli assumptions, and can recognise from the momentum-theorem
examples how the analysis continues when one of them fails.
